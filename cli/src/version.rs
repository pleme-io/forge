...
/// [`From<BumpLevel> for Arc<std::ffi::OsStr>`] routes through
/// [`BumpLevel::as_str`] composed with [`std::ffi::OsStr::new`]
/// composed with [`std::sync::Arc::<std::ffi::OsStr>::from`] so a
/// downstream consumer that takes a shared-owned
/// [`Arc<std::ffi::OsStr>`] via [`Into<Arc<std::ffi::OsStr>>`] (a
/// cross-thread cached-OS-string slot typed as
/// [`Arc<std::ffi::OsStr>`] to share a canonical [`BumpLevel`] label
/// across worker threads via atomic refcount, a validated-input
/// newtype wrapper whose canonical OS-string label field is stored as
/// [`Arc<std::ffi::OsStr>`] to hand cheap [`std::sync::Arc::clone`]s
/// to sibling structures, a serde container that opts into
/// `#[serde(from = "Arc<std::ffi::OsStr>")]` at the shared-owned
/// OS-string frontier, a dashmap-style keyed-table OS-string-value
/// slot whose readers want an [`Arc`] clone rather than a per-lookup
/// allocation, a per-request-arena OS-string slot cheap-cloned across
/// worker threads without per-clone allocation) reads the canonical
/// lowercase label (`"patch"`, `"minor"`, `"major"`) as a shared-
/// owned [`Arc<std::ffi::OsStr>`] with a single allocation for the
...
/// `Arc::<std::ffi::OsStr>::from(std::ffi::OsStr::new(level.as_str()))`
/// restatement.
///
/// The by-value shared-owned emit peer of the by-value owned
/// [`From<BumpLevel> for std::ffi::OsString`] surface (line 1829),
/// the by-value borrowed [`From<BumpLevel> for &'static
/// std::ffi::OsStr`] surface (line 2040), the by-value borrowed/owned
/// [`From<BumpLevel> for std::borrow::Cow<'static, std::ffi::OsStr>`]
/// surface (line 2271), and the by-value shrunk-owned
/// [`From<BumpLevel> for Box<std::ffi::OsStr>`] surface (line 2528)
/// above — all five are OS-string emit surfaces at the same
/// canonical-label oracle, differing only on receiver-side shape:
...
/// allocation with an atomic-refcount header preceding the label bytes
/// for consumers that will hand cheap [`Arc::clone`]s across worker
/// threads. All five route through the same [`BumpLevel::as_str`]
/// canonical-label oracle.
///
/// Structural mirror of [`From<BumpLevel> for Arc<str>`] (line 6418)
/// and [`From<BumpLevel> for Arc<[u8]>`] (line 4767) at the UTF-8 and
/// byte-slice frontiers respectively — the same by-value shared-owned
/// emit surface at the same one-oracle discipline, projected onto the
/// OS-string frontier this time; all siblings route through the
/// standard library `From<&T> for Arc<T>` impl at their respective
/// frontier constructor ([`std::sync::Arc::<str>::from`] for the
/// UTF-8 sibling routing through the `&str` view,
/// [`std::sync::Arc::<[u8]>::from`] for the byte-slice sibling routing
/// through the `.as_bytes()` view,
...
/// emit peer carries no refcount header and the [`std::ffi::OsString`]
/// emit peer carries a growth-header for future extension.
///
/// Trio-closing peer at the third ordered typed sum of the by-value
/// shared-owned OS-string emit trio:
/// [`From<crate::retry::PerAttemptRegion> for
/// std::sync::Arc<std::ffi::OsStr>`] (commit aa7a860) opened the trio
/// at the per-attempt-region ladder;
/// [`From<crate::probe_outcome::AdmissionTier> for
/// std::sync::Arc<std::ffi::OsStr>`] (commit 99f9460) carried the
...
/// (4a4a9d7 → ab2fdcd → 3280cd3), the [`From<T> for
/// std::borrow::Cow<'static, std::ffi::OsStr>`] closure order at the
/// borrowed/owned-frontier OS-string emit sibling
/// (24f6110 → 4e94fc5 → f305c9b), and the [`From<T> for Arc<str>`]
/// and [`From<T> for Arc<[u8]>`] closure orders at the shared-owned
/// UTF-8 and byte-slice emit siblings one and two frontiers below.
/// After this commit the shared-owned OS-string emit frontier joins
/// the closed-across-all-three-typed-sums set alongside the closed
/// shared-owned UTF-8 (`Arc<str>`) and byte-slice (`Arc<[u8]>`) emit
/// trios one and two frontiers below — sharpens the case for a lifted
/// derive-macro that generates the shared-owned emit cross-product
/// across UTF-8 / byte-slice / OS-string / filesystem-path frontiers
/// at every oracle-label-annotated typed sum (the shared-owned
/// OS-string emit surface now has three identical trio-parallel impls
/// to templatize against).
///
/// The identity `Arc::<std::ffi::OsStr>::from(level).as_ref() ==
/// std::ffi::OsStr::new(level.as_str())` at every
/// [`BumpLevel::ALL`] variant is pinned by
/// [`tests::test_bump_level_into_arc_os_str_agrees_with_os_str_new_as_str`];
/// the identity carried through a generic
/// `impl Into<Arc<std::ffi::OsStr>>` consumer at every variant is
/// pinned by
/// [`tests::test_bump_level_into_arc_os_str_carries_through_generic_consumer`];
...
/// [`tests::test_bump_level_into_arc_os_str_shares_label_across_clones`].
///
/// THEORY.md §V.4 typed primitives: the by-value shared-owned OS-
/// string emit surface is a typed-primitive site on [`BumpLevel`]
/// itself (one `From<BumpLevel> for std::sync::Arc<std::ffi::OsStr>`
/// impl routing through [`BumpLevel::as_str`],
...
impl From<BumpLevel> for std::sync::Arc<std::ffi::OsStr> {
    fn from(level: BumpLevel) -> std::sync::Arc<std::ffi::OsStr> {
        std::sync::Arc::<std::ffi::OsStr>::from(std::ffi::OsStr::new(level.as_str()))
    }
}
...
/// [`From<BumpLevel> for std::rc::Rc<std::ffi::OsStr>`] routes
/// through [`BumpLevel::as_str`] composed with
/// [`std::ffi::OsStr::new`] composed with
/// [`std::rc::Rc::<std::ffi::OsStr>::from`] at the shared canonical-
/// label oracle so a downstream consumer that takes a thread-local
/// shared-owned [`Rc<std::ffi::OsStr>`] via
/// [`Into<Rc<std::ffi::OsStr>>`] (a same-thread cached-OS-string slot
/// typed as [`Rc<std::ffi::OsStr>`] to share a canonical [`BumpLevel`]
/// label allocation within one worker via non-atomic refcount, a
/// validated-input newtype wrapper whose canonical OS-string label
/// field is stored as [`Rc<std::ffi::OsStr>`] to hand cheap
/// [`std::rc::Rc::clone`]s to sibling structures on the same thread, a
/// serde container that opts into `#[serde(from = "Rc<std::ffi::OsStr>")]`
/// at the thread-local shared-owned OS-string frontier, a single-
/// threaded arena-keyed OS-string-value table whose readers want an
/// [`Rc`] clone rather than a per-lookup allocation, a graph-walk
/// visitor that clones OS-string labels across nodes without needing
/// [`Send`] / [`Sync`]) reads the canonical lowercase label
/// (`"patch"`, `"minor"`, `"major"`) as a thread-local shared-owned
...
/// per-consumer
/// `Rc::<std::ffi::OsStr>::from(std::ffi::OsStr::new(level.as_str()))`
/// restatement at every downstream site.
///
/// The by-value thread-local shared-owned emit peer of the by-value
/// owned [`From<BumpLevel> for std::ffi::OsString`] surface
/// (line 1829), the by-value borrowed
/// [`From<BumpLevel> for &'static std::ffi::OsStr`] surface
/// (line 2040), the by-value borrowed/owned
/// [`From<BumpLevel> for std::borrow::Cow<'static, std::ffi::OsStr>`]
/// surface (line 2271), the by-value shrunk-owned
/// [`From<BumpLevel> for Box<std::ffi::OsStr>`] surface (line 2528),
/// and the by-value atomic-shared-owned
/// [`From<BumpLevel> for std::sync::Arc<std::ffi::OsStr>`] surface
/// (line 2775) above — all six are OS-string emit surfaces at the same
/// canonical-label oracle, differing only on receiver-side shape:
/// [`From<T> for std::ffi::OsString`] returns a growth-header-carrying
/// resizable owned buffer, [`From<T> for &'static std::ffi::OsStr`]
/// returns a zero-copy `'static`-lived borrow into the static-string
...
/// (line 6944) and [`From<BumpLevel> for std::rc::Rc<[u8]>`]
/// (line 5159) at the UTF-8 and byte-slice frontiers respectively —
...
///
/// Trio-closing peer at the third ordered typed sum of the by-value
/// thread-local shared-owned OS-string emit trio:
/// [`From<crate::retry::PerAttemptRegion> for std::rc::Rc<std::ffi::OsStr>`]
/// (commit d484f80 at `retry.rs:5283`) opened the trio at the per-
/// attempt-region ladder;
/// [`From<crate::probe_outcome::AdmissionTier> for std::rc::Rc<std::ffi::OsStr>`]
/// (commit 2df36e3 at `probe_outcome.rs:9688`) carried the mid-trio
/// slot at the admission-tier ladder; this impl closes the trio at the
/// version-bump-magnitude ladder, matching the
/// [`From<T> for Arc<std::ffi::OsStr>`] closure order at the atomic-
/// shared-owned OS-string emit sibling one ownership shape above
/// (aa7a860 → 99f9460 → 6fc3e70), the
/// [`From<T> for Box<std::ffi::OsStr>`] closure order at the shrunk-
/// owned OS-string emit sibling one ownership shape below
/// (4a4a9d7 → ab2fdcd → 3280cd3), and the
/// [`From<T> for Arc<std::path::Path>`] closure order at the atomic-
/// shared-owned filesystem-path emit sibling one frontier above
/// (97a2228 → 0b378c1 → 54c3104). After this commit the thread-local
/// shared-owned OS-string emit surface joins the closed-across-all-
/// three-typed-sums set alongside the closed thread-local shared-owned
/// UTF-8 (`Rc<str>`) and byte-slice (`Rc<[u8]>`) emit trios one and
/// two frontiers below and the closed atomic-shared-owned
/// (`Arc<std::ffi::OsStr>`) emit trio one ownership shape above —
/// sharpens the case for a lifted derive-macro that generates the
/// thread-local shared-owned emit cross-product across UTF-8 /
/// byte-slice / OS-string / filesystem-path frontiers at every
/// oracle-label-annotated typed sum (the thread-local shared-owned
/// OS-string emit surface now has three identical trio-parallel impls
/// to templatize against, matching the shared-owned UTF-8, byte-slice,
/// and OS-string emit siblings' three-impl coverage).
///
/// The impl body picks [`std::rc::Rc::<std::ffi::OsStr>::from`] on
/// [`std::ffi::OsStr::new`]`(level.as_str())` rather than a
...
///
/// The identity `Rc::<std::ffi::OsStr>::from(level).as_ref() ==
/// std::ffi::OsStr::new(level.as_str())` at every
/// [`BumpLevel::ALL`] variant is pinned by
/// [`tests::test_bump_level_into_rc_os_str_agrees_with_os_str_new_as_str`];
/// the identity carried through a generic
/// `impl Into<Rc<std::ffi::OsStr>>` consumer at every variant is
/// pinned by
/// [`tests::test_bump_level_into_rc_os_str_carries_through_generic_consumer`];
...
/// canonical label is named at one site ([`BumpLevel::as_str`]) and
/// every OS-string emit surface — [`From<T> for std::ffi::OsString`],
/// [`From<T> for &'static std::ffi::OsStr`],
/// [`From<T> for std::borrow::Cow<'static, std::ffi::OsStr>`],
...
impl From<BumpLevel> for std::rc::Rc<std::ffi::OsStr> {
    fn from(level: BumpLevel) -> std::rc::Rc<std::ffi::OsStr> {
        std::rc::Rc::<std::ffi::OsStr>::from(std::ffi::OsStr::new(level.as_str()))
    }
}

/// [`From<BumpLevel> for std::rc::Rc<std::path::Path>`] routes
/// through [`BumpLevel::as_str`] composed with
/// [`std::path::Path::new`] composed with
/// [`std::rc::Rc::<std::path::Path>::from`] at the shared canonical-
/// label oracle so a downstream consumer that takes a thread-local
/// shared-owned [`Rc<std::path::Path>`] via
/// [`Into<Rc<std::path::Path>>`] (a same-thread cached-filesystem-
/// path slot typed as [`Rc<std::path::Path>`] to share a canonical
/// [`BumpLevel`] label allocation within one worker via non-atomic
/// refcount, a validated-input newtype wrapper whose canonical
/// filesystem-path label field is stored as [`Rc<std::path::Path>`]
/// to hand cheap [`std::rc::Rc::clone`]s to sibling structures on
/// the same thread, a serde container that opts into
/// `#[serde(from = "Rc<std::path::Path>")]` at the thread-local
/// shared-owned filesystem-path frontier, a single-threaded arena-
/// keyed table whose filesystem-path-value slot readers want an
/// [`Rc`] clone rather than a per-lookup allocation, a graph-walk
/// visitor that clones filesystem-path labels across nodes without
/// needing [`Send`] / [`Sync`]) reads the canonical lowercase label
/// (`"patch"`, `"minor"`, `"major"`) as a thread-local shared-owned
...
/// atomic [`std::sync::Arc::clone`] — through ONE composition rather
/// than a per-consumer
/// `Rc::<std::path::Path>::from(std::path::Path::new(level.as_str()))`
/// restatement at every downstream site.
///
/// The by-value thread-local shared-owned emit peer of the by-value
/// owned [`From<BumpLevel> for std::path::PathBuf`] surface
/// (line 1944), the by-value borrowed
/// [`From<BumpLevel> for &'static std::path::Path`] surface
/// (line 2141), the by-value borrowed/owned
/// [`From<BumpLevel> for std::borrow::Cow<'static, std::path::Path>`]
/// surface (line 2411), the by-value shrunk-owned
/// [`From<BumpLevel> for Box<std::path::Path>`] surface (line 2646),
/// and the by-value atomic-shared-owned
/// [`From<BumpLevel> for std::sync::Arc<std::path::Path>`] surface
/// (line 2913) above — all six are filesystem-path emit surfaces at
/// the same canonical-label oracle, differing only on receiver-side
/// shape: [`From<T> for std::path::PathBuf`] returns a growth-
/// header-carrying resizable owned buffer,
/// [`From<T> for &'static std::path::Path`] returns a zero-copy
/// `'static`-lived borrow into the static-string constant table,
/// [`From<T> for Cow<'static, std::path::Path>`] returns either
/// uniformly at the borrowed/owned frontier,
/// [`From<T> for Box<std::path::Path>`] returns a shrunk immutable
/// heap allocation with no refcount header,
/// [`From<T> for Arc<std::path::Path>`] returns a shared-owned
/// immutable heap allocation with an atomic-refcount header
/// preceding the label bytes for consumers that will hand cheap
/// [`Arc::clone`]s across worker threads, this
/// [`From<T> for Rc<std::path::Path>`] returns a thread-local
/// shared-owned immutable heap allocation with a non-atomic-
/// refcount header for consumers whose upstream will never cross a
/// thread boundary. All six route through the same
/// [`BumpLevel::as_str`] canonical-label oracle.
///
/// Structural mirror of [`From<BumpLevel> for std::rc::Rc<str>`]
/// (line 7102), [`From<BumpLevel> for std::rc::Rc<[u8]>`]
/// (line 5317), and [`From<BumpLevel> for std::rc::Rc<std::ffi::OsStr>`]
/// directly above (line 3071) at the UTF-8, byte-slice, and
/// OS-string frontiers respectively — the same by-value thread-local
/// shared-owned emit surface at the same one-oracle discipline,
/// projected onto the filesystem-path frontier this time; all
/// siblings route through the standard library `From<&T> for Rc<T>`
/// impl at their respective frontier constructor
...
///
/// Trio-closing peer at the third ordered typed sum of the by-value
/// thread-local shared-owned filesystem-path emit trio opened at the
/// per-attempt-region ladder by
/// [`From<crate::retry::PerAttemptRegion> for std::rc::Rc<std::path::Path>`]
/// (commit af6b09a) and mid-slotted at the admission-tier ladder by
/// [`From<crate::probe_outcome::AdmissionTier> for std::rc::Rc<std::path::Path>`]
/// (commit 949d2e3); this impl closes the trio at the version-bump-
/// magnitude ladder, matching the
/// [`From<T> for std::rc::Rc<std::ffi::OsStr>`] closure order at the
/// thread-local shared-owned OS-string emit sibling one frontier
/// above (d484f80 → 2df36e3 → 91e16b4), the
/// [`From<T> for std::sync::Arc<std::path::Path>`] closure order at
/// the atomic-shared-owned filesystem-path emit sibling one
/// ownership shape above (97a2228 → 0b378c1 → 54c3104), and the
/// [`From<T> for Box<std::path::Path>`] closure order at the shrunk-
/// owned filesystem-path emit sibling one ownership shape below
/// (7aa68e4 → a903972 → 41ec18a). After this commit the thread-local
/// shared-owned emit axis spans all four frontiers (UTF-8, byte-
/// slice, OS-string, filesystem-path) across all three ordered typed
/// sums on the ladder set against ONE canonical-label oracle each,
/// matching the closed atomic-shared-owned tier's coverage
/// (`Arc<str>`, `Arc<[u8]>`, `Arc<std::ffi::OsStr>`,
/// `Arc<std::path::Path>` at every ladder). Sharpens the case for a
/// lifted derive-macro that generates the thread-local shared-owned
/// emit cross-product across UTF-8 / byte-slice / OS-string /
/// filesystem-path frontiers at every oracle-label-annotated typed
/// sum — the thread-local shared-owned filesystem-path emit surface
/// now has three identical trio-parallel impls to templatize
/// against, matching the three-impl coverage the thread-local
/// shared-owned UTF-8, byte-slice, and OS-string emit siblings
/// already carry; the macro is not in this commit.
///
/// The impl body picks [`std::rc::Rc::<std::path::Path>::from`] on
/// [`std::path::Path::new`]`(level.as_str())` rather than a
/// [`Box::<std::path::Path>::from`]-then-[`Rc::from`] round trip
/// (which would allocate twice — box the filesystem-path slice, then
/// rewrap into an [`Rc<std::path::Path>`]) or a
/// [`std::path::PathBuf`]-then-[`Rc::from`] round trip (which would
/// allocate a growth-header-carrying resizable buffer, then rewrap):
/// the direct [`std::rc::Rc::<std::path::Path>::from`] path
/// allocates once from the `'static` label slice viewed through
/// [`std::path::Path::new`], including the non-atomic-refcount
/// header, so the receiver pays a single allocation for exactly the
/// label's length plus the header.
///
/// The identity `Rc::<std::path::Path>::from(level).as_ref() ==
/// std::path::Path::new(level.as_str())` at every
/// [`BumpLevel::ALL`] variant is pinned by
/// [`tests::test_bump_level_into_rc_path_agrees_with_path_new_as_str`];
/// the identity carried through a generic
/// `impl Into<Rc<std::path::Path>>` consumer at every variant is
/// pinned by
/// [`tests::test_bump_level_into_rc_path_carries_through_generic_consumer`];
/// the thread-local shared-owned receiver contract —
/// [`std::rc::Rc::clone`] reads the same canonical label,
/// [`std::rc::Rc::ptr_eq`] holds after the clone, and
/// [`std::rc::Rc::strong_count`] lifts to at least two after the
/// clone — at every variant is pinned by
/// [`tests::test_bump_level_into_rc_path_shares_label_across_clones`].
///
/// THEORY.md §V.4 typed primitives: the by-value thread-local
/// shared-owned filesystem-path emit surface is a typed-primitive
/// site on [`BumpLevel`] itself (one
/// `From<BumpLevel> for std::rc::Rc<std::path::Path>` impl routing
/// through [`BumpLevel::as_str`], [`std::path::Path::new`], and
...
/// [`From<T> for std::rc::Rc<std::path::Path>`] — reads through it.
impl From<BumpLevel> for std::rc::Rc<std::path::Path> {
    fn from(level: BumpLevel) -> std::rc::Rc<std::path::Path> {
        std::rc::Rc::<std::path::Path>::from(std::path::Path::new(level.as_str()))
    }
}
...
/// [`TryFrom<std::ffi::CString> for BumpLevel`] routes through the
/// by-reference [`TryFrom<&std::ffi::CStr>`] parse peer directly above on
/// [`std::ffi::CString::as_c_str`] of the caller-owned NUL-terminated
/// C-string buffer, so a downstream consumer bound by
/// `impl TryFrom<std::ffi::CString>` (a `serde` container that opts into
/// `#[serde(try_from = "CString")]` on a wrapper field, a generic
/// try-conversion helper `fn parse_c_field<T: TryFrom<CString>>` that owns
/// the input NUL-terminated C-string, a validated-input newtype builder
/// that consumes an owned [`std::ffi::CString`] and returns a validated
/// [`BumpLevel`], a [`std::ffi::CString::from_vec_with_nul`] receiver
/// that owns the returned [`std::ffi::CString`] over a FFI-produced
/// buffer, a [`std::ffi::CString::new`] terminus that owns the
/// freshly-allocated NUL-terminated buffer, a `libc::getenv` +
/// [`std::ffi::CStr::from_ptr`] + [`std::ffi::CStr::to_owned`] compose
/// that owns the process-environment slot's NUL-terminated payload)
/// recovers a [`BumpLevel`] value from a canonical lowercase-label owned
/// NUL-terminated C-string through the same one-oracle grammar the
...
/// [`TryFrom<&std::ffi::OsStr>`], [`TryFrom<std::ffi::OsString>`],
/// [`TryFrom<&std::path::Path>`], [`TryFrom<std::path::PathBuf>`], and
/// [`TryFrom<&std::ffi::CStr>`] parse peers already read.
///
/// The by-value owned-buffer NUL-terminated C-string parse peer of
/// [`TryFrom<&std::ffi::CStr> for BumpLevel`] directly above — both are
/// parse surfaces of the NUL-terminated C-string frontier, differing
/// only on the input C-string ownership: [`TryFrom<&std::ffi::CStr>`]
/// takes a borrowed [`&std::ffi::CStr`] view for consumers that already
/// hold a borrow (a `*const c_char` FFI return surfaced through
/// [`std::ffi::CStr::from_ptr`], a `getenv(3)` reader that keeps the
/// process-environment slot borrowed, a `syslog(3)` / `journald` bridge
/// that reads a canonical NUL-terminated label off a C-emitted log frame
/// without allocating), this [`TryFrom<std::ffi::CString>`] takes an
/// owned [`std::ffi::CString`] for consumers that own the input
/// NUL-terminated C-string buffer (a `serde` `try_from = "CString"`
/// container, a C-string-consuming builder, a [`std::ffi::CString::new`]
/// receiver, a [`std::ffi::CString::from_vec_with_nul`] receiver). Both
/// delegate through the shared [`FromStr`] parse oracle: the borrowed
/// peer through [`std::ffi::CStr::to_str`] + [`TryFrom<&str>`] (which
/// itself composes through [`FromStr`]), this owned peer through the
/// borrowed peer via [`std::ffi::CString::as_c_str`] at the boundary —
/// the same canonical grammar lifted to the owned-buffer input layer.
///
/// Structural mirror of [`TryFrom<String> for BumpLevel`] at the UTF-8
/// frontier, [`TryFrom<Vec<u8>> for BumpLevel`] at the byte-slice
/// frontier, and [`TryFrom<std::ffi::OsString> for BumpLevel`] at the
/// OS-string frontier — the same by-value owned-input parse surface at
/// the same one-oracle discipline, projected onto the NUL-terminated
/// C-string frontier.
///
/// Closing peer of the by-value owned-buffer NUL-terminated C-string
/// parse trio at the version-bump-magnitude ladder:
/// [`TryFrom<std::ffi::CString>`] for [`crate::retry::PerAttemptRegion`]
/// (commit e73daa7) opened the trio at the first ordered typed sum;
/// [`TryFrom<std::ffi::CString>`] for
/// [`crate::probe_outcome::AdmissionTier`] (commit 77c0cdd) mid-slotted
/// the trio at the second ordered typed sum; this impl closes the trio
/// at the third and last ordered typed sum, matching the
/// [`TryFrom<&std::ffi::CStr>`] borrowed-view parse closing order
/// (8777e9f → 95ddf71 → 5073623) at the borrowed-view counterpart, the
/// [`TryFrom<String>`] UTF-8 owned-input parse closing order at the
/// UTF-8 frontier's owned-input counterpart, the [`TryFrom<Vec<u8>>`]
/// byte-slice owned-input parse closing order at the byte-slice
/// frontier's owned-input counterpart, the [`TryFrom<std::ffi::OsString>`]
/// OS-string owned-input parse closing order at the OS-string
/// frontier's owned-input counterpart, and the [`From<BumpLevel> for
/// std::ffi::CString`] owned-emit closing order (020317a → 1e8ed3c →
/// 6cff423) at the NUL-terminated C-string frontier's emit-side
/// owned-buffer dual. After this commit the by-value owned-buffer
/// NUL-terminated C-string parse axis spans all three ordered typed
/// sums on the ladder set through ONE
/// [`std::ffi::CString::as_c_str`] + [`TryFrom<&std::ffi::CStr>`]
/// composition each.
///
/// The natural bridge to any downstream site that types its
/// NUL-terminated C-string parse contract as
/// `impl TryFrom<std::ffi::CString>` rather than
/// [`TryFrom<&std::ffi::CStr>`] — the NUL-terminated C-string frontier's
/// `serde` `try_from` container attribute at the owned-buffer layer
/// (`#[serde(try_from = "CString")]` — which keys off
/// [`TryFrom<std::ffi::CString>`], not [`TryFrom<&std::ffi::CStr>`]), a
/// validated-input newtype builder whose canonical parse contract is
/// stated as `TryFrom<std::ffi::CString>` (consumes the caller's input
/// NUL-terminated C-string end-to-end), a
/// [`std::ffi::CString::from_vec_with_nul`] terminus that owns the
/// returned [`std::ffi::CString`] and hands it to a typed parser without
/// a borrow-then-clone round trip. The [`FromStr`] impl carries the
/// load-bearing match body against the canonical grammar; this
/// [`TryFrom<std::ffi::CString>`] impl delegates through the
/// [`TryFrom<&std::ffi::CStr>`] parse peer (which itself composes
/// [`std::ffi::CStr::to_str`] with [`TryFrom<&str>`] which composes
/// [`FromStr`]), so the parse-oracle discipline is preserved end-to-end
...
/// [`TryFrom<&std::ffi::CStr>`] peer directly above is strict at,
/// inherited through the delegation: non-UTF-8 NUL-terminated C-string
/// byte sequences reject at [`std::ffi::CStr::to_str`] with the standard
/// "invalid UTF-8 in bump level CStr input" diagnostic surfaced through
/// [`anyhow::Error`], and valid-UTF-8 NUL-terminated C-string sequences
/// that decode to a non-canonical label (`""`, `"Patch"`, `"Minor"`,
/// `"Major"`, `"PATCH"`, `" patch"`, `"patch "`, `"pat"`) reject at the
/// underlying [`FromStr`] impl — the same canonical-only strictness the
/// NUL-terminated C-string frontier already carries at the by-reference
/// peer, now lifted to the owned-buffer input layer at ONE composition
/// through the borrowed [`TryFrom<&std::ffi::CStr>`] peer.
///
/// The impl body picks [`std::ffi::CString::as_c_str`] rather than
/// [`std::ffi::CString::into_string`] or an intermediate `to_str` +
/// [`str::parse`] restatement: [`std::ffi::CString::as_c_str`] yields a
/// borrowed [`&std::ffi::CStr`] view of the caller-owned buffer without
/// allocation, so the parse-side receiver pays the by-reference
/// [`TryFrom<&std::ffi::CStr>`] cost, not the [`String`]-allocation cost
/// of a [`std::ffi::CString::into_string`] round trip. The owned
/// [`std::ffi::CString`] input is dropped at return, freeing its buffer
/// exactly once — the same discipline the sibling [`TryFrom<Vec<u8>>`],
/// [`TryFrom<String>`], and [`TryFrom<std::ffi::OsString>`] owned-buffer
/// parse peers apply at the [`Vec::as_slice`], [`String::as_str`], and
/// [`std::ffi::OsString::as_os_str`] boundaries at the byte-slice,
/// UTF-8, and OS-string frontiers.
///
/// The identity `BumpLevel::try_from(std::ffi::CString::new(
/// level.as_str()).unwrap()).unwrap() == level` at every
/// [`BumpLevel::ALL`] variant is pinned by
/// [`tests::test_bump_level_try_from_cstring_agrees_with_from_str`];
/// the identity carried through a generic `impl TryFrom<std::ffi::CString>`
/// consumer at every variant is pinned by
/// [`tests::test_bump_level_try_from_cstring_carries_through_generic_consumer`];
/// the strict-rejection contract on non-UTF-8 owned NUL-terminated
/// C-string input is pinned by
/// [`tests::test_bump_level_try_from_cstring_rejects_non_utf8_input`];
/// the strict-rejection contract on valid-UTF-8 non-canonical owned
/// NUL-terminated C-string input is pinned by
/// [`tests::test_bump_level_try_from_cstring_rejects_non_canonical_input`].
///
/// THEORY.md §V.4 typed primitives: the by-value owned-buffer
/// NUL-terminated C-string try-conversion surface is a typed-primitive
/// site on [`BumpLevel`] itself (one `TryFrom<std::ffi::CString>` impl
/// routing through the borrowed [`TryFrom<&std::ffi::CStr>`] parse peer
/// on [`std::ffi::CString::as_c_str`]), not a per-consumer
/// `BumpLevel::try_from(c.as_c_str())` bridge at every downstream site
/// that types its parse contract as `impl TryFrom<std::ffi::CString>`
/// rather than [`TryFrom<&std::ffi::CStr>`] or [`std::str::FromStr`].
...
/// [`TryFrom<&std::ffi::CStr>`], this [`TryFrom<std::ffi::CString>`] —
/// reads through it.
impl TryFrom<std::ffi::CString> for BumpLevel {
    type Error = anyhow::Error;

    fn try_from(c: std::ffi::CString) -> Result<Self, Self::Error> {
        <Self as std::convert::TryFrom<&std::ffi::CStr>>::try_from(c.as_c_str())
    }
}
...
/// [`TryFrom<Arc<std::ffi::CStr>> for BumpLevel`] routes through
/// [`std::sync::Arc::<std::ffi::CStr>::as_ref`] on the caller-supplied
/// shared-owned NUL-terminated C-string and the by-reference
/// NUL-terminated C-string parse peer [`TryFrom<&std::ffi::CStr>`]
/// (which itself composes [`std::ffi::CStr::to_str`] with
/// [`TryFrom<&str>`] which itself delegates through
/// [`<BumpLevel as std::str::FromStr>::from_str`]), so a downstream
/// consumer bound by `impl TryFrom<std::sync::Arc<std::ffi::CStr>>` (a
/// shared-owned NUL-terminated C-string wrapper handed to sibling
/// threads through an atomic-refcount header, a
/// [`Box<std::ffi::CStr>::into`] shared-buffer input that trades
/// exclusive shrunk-owned single-owner semantics for a shared
/// reader-count of the same immutable NUL-terminated C-string, a
/// [`std::ffi::CStr::from_ptr`] + [`std::ffi::CStr::to_owned`] +
/// [`std::ffi::CString::into_boxed_c_str`] +
/// [`std::sync::Arc::<std::ffi::CStr>::from`] compose that shares the
/// FFI-returned NUL-terminated buffer across threads without a per-
/// reader clone of the label bytes, a generic try-conversion helper
/// `fn parse<T: TryFrom<std::sync::Arc<std::ffi::CStr>>>` that composes
/// with shared-owned NUL-terminated C-string inputs) recovers a
/// [`BumpLevel`] value from a shared-owned canonical NUL-terminated
/// C-string label through the same one-oracle grammar the sibling
/// [`TryFrom<&std::ffi::CStr>`], [`TryFrom<std::ffi::CString>`],
/// [`TryFrom<Box<std::ffi::CStr>>`],
/// [`TryFrom<std::sync::Arc<std::ffi::OsStr>>`],
/// [`TryFrom<std::sync::Arc<std::path::Path>>`], and (one frontier
/// below) [`TryFrom<std::sync::Arc<str>>`],
/// [`TryFrom<std::sync::Arc<[u8]>>`] shared-owned parse peers already
/// read at neighboring NUL-terminated-C-string, OS-string, filesystem-
/// path, UTF-8, and byte-slice frontiers.
///
/// The by-value shared-owned NUL-terminated C-string parse peer of
/// [`TryFrom<Box<std::ffi::CStr>>`] above — both are shrunk/shared
/// immutable-heap NUL-terminated C-string parse surfaces of the label-
/// axis conversion set, differing on ownership discipline:
/// [`TryFrom<Box<std::ffi::CStr>>`] consumes a single-owner shrunk
/// NUL-terminated buffer with exclusive ownership semantics,
/// this [`TryFrom<std::sync::Arc<std::ffi::CStr>>`] consumes a
/// shared-owner atomic-refcount NUL-terminated buffer that may have
/// surviving clones outside the receiver. Both route through the shared
/// [`<BumpLevel as std::str::FromStr>::from_str`] canonical-label
/// parse oracle: the shrunk-owned peer through
/// [`std::boxed::Box::<std::ffi::CStr>::as_ref`] composed with
/// [`TryFrom<&std::ffi::CStr>`], this shared-owned peer through
/// [`std::sync::Arc::<std::ffi::CStr>::as_ref`] composed with
/// [`TryFrom<&std::ffi::CStr>`] — the same canonical grammar lifted to
/// the shared-owned NUL-terminated C-string layer.
///
/// The two-stage strictness discipline (UTF-8 validity at the
/// NUL-terminated C-string decode frontier gated by
/// [`std::ffi::CStr::to_str`], canonical-label grammar at the parse
/// frontier gated by [`std::str::FromStr`]) is inherited unchanged from
/// the by-reference [`TryFrom<&std::ffi::CStr>`] peer via
/// [`std::sync::Arc::<std::ffi::CStr>::as_ref`] which yields a
/// `&std::ffi::CStr` view of the shared inner payload without cloning
/// the atomic refcount or reallocating the label buffer.
///
/// The impl body picks [`std::sync::Arc::<std::ffi::CStr>::as_ref`]
/// rather than an intermediate clone-into-`CString` +
/// `TryFrom<CString>` restatement: the [`AsRef`] view yields a borrowed
/// `&std::ffi::CStr` window over the shared heap allocation without a
/// clone of the label bytes into a resizable [`std::ffi::CString`]
/// first, so the receiver pays no clone and no capacity-vs-length
/// metadata rebuild on the fast path — the borrow-then-drop discipline
/// the sibling [`TryFrom<std::ffi::CString>`] and
/// [`TryFrom<Box<std::ffi::CStr>>`] peers apply at the resizable-buffer
/// and shrunk-owned axes, lifted to the shared-owned axis via
/// [`std::sync::Arc::<std::ffi::CStr>::as_ref`].
///
/// Trio-closing peer of the shared-owned NUL-terminated C-string parse
/// trio at the version-bump-magnitude ladder:
/// [`TryFrom<std::sync::Arc<std::ffi::CStr>>`] for
/// [`crate::retry::PerAttemptRegion`] (commit d0e9b09) opened the trio
/// at the first ordered typed sum;
/// [`TryFrom<std::sync::Arc<std::ffi::CStr>>`] for
/// [`crate::probe_outcome::AdmissionTier`] (commit 57e7b7b) mid-slotted
/// the trio at the second ordered typed sum; this impl closes the trio
/// at the third and last ordered typed sum, matching the
/// [`TryFrom<&std::ffi::CStr>`] borrowed-view NUL-terminated C-string
/// parse closing order (8777e9f → 95ddf71 → 5073623) at the borrowed-
/// view counterpart, the [`TryFrom<std::ffi::CString>`] owned-buffer
/// NUL-terminated C-string parse closing order (e73daa7 → 77c0cdd →
/// 20357c7) at the owned-buffer counterpart, the
/// [`TryFrom<Box<std::ffi::CStr>>`] shrunk-owned NUL-terminated
/// C-string parse closing order (1076700 → 16320c3 → 3a2edcd) at the
/// shrunk-owned counterpart, the
/// [`TryFrom<std::sync::Arc<std::ffi::OsStr>>`] OS-string shared-owned
/// parse closing order (478313f → 7f411bc → 140d9c2) at the OS-string
/// sibling one frontier above, and the
/// [`TryFrom<std::sync::Arc<std::path::Path>>`] filesystem-path shared-
/// owned parse closing order (b9f5ef1 → aec69b1 → ee4a862) at the
/// filesystem-path sibling. After this commit the shared-owned NUL-
/// terminated C-string parse axis spans all three ordered typed sums
/// on the ladder set through ONE
/// [`std::sync::Arc::<std::ffi::CStr>::as_ref`] +
/// [`TryFrom<&std::ffi::CStr>`] composition each.
///
/// The identity `BumpLevel::try_from(
/// std::sync::Arc::<std::ffi::CStr>::from(std::ffi::CString::new(
/// level.as_str()).unwrap().into_boxed_c_str())).unwrap() == level` at
/// every [`BumpLevel::ALL`] variant is pinned by
/// [`tests::test_bump_level_try_from_arc_cstr_agrees_with_from_str`];
/// the identity carried through a generic
/// `impl TryFrom<std::sync::Arc<std::ffi::CStr>>` consumer at every
/// variant is pinned by
/// [`tests::test_bump_level_try_from_arc_cstr_carries_through_generic_consumer`];
/// the strict-rejection contract on non-UTF-8 shared-owned
/// NUL-terminated C-string input is pinned by
/// [`tests::test_bump_level_try_from_arc_cstr_rejects_non_utf8_input`];
/// the strict-rejection contract on valid-UTF-8 non-canonical shared-
/// owned NUL-terminated C-string input is pinned by
/// [`tests::test_bump_level_try_from_arc_cstr_rejects_non_canonical_input`].
///
/// THEORY.md §V.4 typed primitives: the shared-owned NUL-terminated
/// C-string try-conversion parse surface is a typed-primitive site on
/// [`BumpLevel`] itself (one
/// `TryFrom<std::sync::Arc<std::ffi::CStr>>` impl routing through
/// [`std::sync::Arc::<std::ffi::CStr>::as_ref`] and the by-reference
/// [`TryFrom<&std::ffi::CStr>`] peer), not a per-consumer
/// `BumpLevel::try_from(shared.as_ref())` bridge at every downstream
/// site that types its parse contract as
/// `impl TryFrom<std::sync::Arc<std::ffi::CStr>>`.
/// THEORY.md §VI.1 one-oracle: the canonical label grammar is named
/// at one site ([`BumpLevel::as_str`]), inverted at one site
/// ([`<BumpLevel as std::str::FromStr>::from_str`]), and every parse
/// surface — including this shared-owned NUL-terminated C-string
/// peer — reads through it.
impl TryFrom<std::sync::Arc<std::ffi::CStr>> for BumpLevel {
    type Error = anyhow::Error;

    fn try_from(c: std::sync::Arc<std::ffi::CStr>) -> Result<Self, Self::Error> {
        <Self as std::convert::TryFrom<&std::ffi::CStr>>::try_from(c.as_ref())
    }
}
...
/// [`TryFrom<Rc<std::path::Path>> for BumpLevel`] routes through
/// [`std::rc::Rc::<std::path::Path>::as_ref`] on the caller-supplied
/// thread-local shared-owned filesystem path and the by-reference
/// filesystem-path parse peer [`TryFrom<&std::path::Path>`] (which
/// itself composes [`std::path::Path::as_os_str`] with
/// [`std::ffi::OsStr::to_str`] and [`TryFrom<&str>`], which delegates
/// through [`<BumpLevel as std::str::FromStr>::from_str`]), so a
/// downstream consumer bound by `impl TryFrom<Rc<std::path::Path>>` (a
/// non-`Sync` release-manifest filesystem-walk renderer that stashes
/// canonical bump labels in an [`Rc`] cell for cheap in-thread clone, a
/// [`Box<std::path::Path>::into`] shared-buffer input that trades
/// exclusive shrunk-owned single-owner semantics for a shared reader-
/// count of the same immutable filesystem path within a single thread,
/// a generic try-conversion helper
/// `fn parse<T: TryFrom<Rc<std::path::Path>>>` that composes with
/// thread-local shared-owned filesystem-path inputs) recovers a
/// [`BumpLevel`] value from a thread-local shared-owned canonical
/// filesystem-path label (`"patch"`, `"minor"`, `"major"`) through the
/// same one-oracle grammar the sibling [`TryFrom<&std::path::Path>`],
/// [`TryFrom<std::path::PathBuf>`],
/// [`TryFrom<std::borrow::Cow<'_, std::path::Path>>`],
/// [`TryFrom<Box<std::path::Path>>`],
/// [`TryFrom<std::sync::Arc<std::path::Path>>`], and (one frontier
/// below) [`TryFrom<std::rc::Rc<std::ffi::OsStr>>`] shared-owned parse
/// peers read.
///
/// The thread-local shared-owned filesystem-path parse peer of
/// [`TryFrom<std::sync::Arc<std::path::Path>>`] above and the
/// filesystem-path frontier peer of
/// [`TryFrom<std::rc::Rc<std::ffi::OsStr>>`] directly above — all three
/// route through the shared
/// [`<BumpLevel as std::str::FromStr>::from_str`] canonical-label parse
/// oracle, differing on refcount discipline and frontier:
...
///
/// The impl body picks [`std::rc::Rc::<std::path::Path>::as_ref`]
/// rather than [`std::rc::Rc::<std::path::Path>::to_path_buf`] or an
/// intermediate owned [`std::path::PathBuf`] +
/// [`TryFrom<std::path::PathBuf>`] restatement: the [`AsRef`] view
/// yields a borrowed `&std::path::Path` window over the shared heap
/// allocation without cloning the single-threaded refcount header,
/// converting the shared-owned receiver into a resizable
/// [`std::path::PathBuf`], or forking the underlying immutable buffer
/// — the receiver pays no refcount increment on the fast path, only
/// the eventual drop-decrement when the
/// [`std::rc::Rc<std::path::Path>`] receiver falls out of scope. The
/// same borrow-then-drop discipline the sibling
/// [`TryFrom<Box<std::path::Path>>`], [`TryFrom<std::path::PathBuf>`],
/// [`TryFrom<Cow<'_, std::path::Path>>`], and
/// [`TryFrom<std::sync::Arc<std::path::Path>>`] peers apply at the
/// shrunk-owned, resizable-buffer, borrowed/owned-frontier, and
/// atomic-shared-owned axes, lifted to the thread-local shared-owned
/// filesystem-path axis via
/// [`std::rc::Rc::<std::path::Path>::as_ref`].
///
/// Trio-closing slot in the thread-local shared-owned filesystem-path
/// parse trio at the bump-level ladder — the opening peer at
/// [`crate::retry::PerAttemptRegion`] was carried at 4497115
/// (`TryFrom<Rc<std::path::Path>> for PerAttemptRegion` — opens
/// thread-local shared-owned filesystem-path parse trio), the mid-trio
/// peer at [`crate::probe_outcome::AdmissionTier`] followed at c973b49
/// (`TryFrom<Rc<std::path::Path>> for AdmissionTier` — mid-trio
/// thread-local shared-owned filesystem-path parse peer), and this
/// impl carries the closing slot at the third ordered typed sum. After
/// this commit the thread-local shared-owned filesystem-path parse
/// axis spans all three ordered typed sums on the ladder set through
/// ONE [`std::rc::Rc::<std::path::Path>::as_ref`] +
/// [`TryFrom<&std::path::Path>`] composition each — matching the
/// atomic-shared-owned filesystem-path parse closing order
/// (b9f5ef1 → aec69b1 → ee4a862), the shrunk-owned filesystem-path
/// parse closing order (315c145 → 657434c → 336e453), the borrowed/
/// owned-frontier filesystem-path parse closing order
/// (47c471f → b52ed1e → 87eda90), the borrowed-view filesystem-path
/// parse closing order (dba4c6b → 321b2d8 → 7863a1d), the owned-
/// buffer filesystem-path parse closing order
/// (33e4e48 → 2855792 → b1945f4), and (one frontier below) the
/// thread-local shared-owned OS-string parse closing order
/// (63f6f55 → 2adee36 → b27263d) already read.
///
/// The parser inherits the two-stage strict-rejection discipline of
/// the underlying [`TryFrom<&std::path::Path>`] impl: non-Unicode
/// [`std::path::Path`] input reaches the [`std::ffi::OsStr::to_str`]
...
/// returns an [`anyhow::Error`] before the [`FromStr`] canonical-
/// grammar gate; valid-Unicode but non-canonical input (empty,
/// UpperCamel `"Patch"`/`"Minor"`/`"Major"`, whitespace padding,
/// uppercase `"PATCH"`, truncated `"pat"`) all reach the [`FromStr`]
/// canonical-grammar gate and return an [`anyhow::Error`] there.
///
/// The identity
/// `BumpLevel::try_from(std::rc::Rc::<std::path::Path>::from(
/// std::path::PathBuf::from(level.as_str()).into_boxed_path()))
/// .unwrap() == level` at every [`BumpLevel::ALL`] variant is pinned
/// by [`tests::test_bump_level_try_from_rc_path_agrees_with_from_str`];
/// the identity carried through a generic
/// `impl TryFrom<Rc<std::path::Path>>` consumer at every variant is
/// pinned by
/// [`tests::test_bump_level_try_from_rc_path_carries_through_generic_consumer`];
/// the Unicode-decode strict-rejection contract at the thread-local
/// shared-owned filesystem-path boundary is pinned (Unix-only, per the
/// same platform constraint the sibling
/// [`TryFrom<std::sync::Arc<std::path::Path>>`] non-Unicode pin reads)
/// by
/// [`tests::test_bump_level_try_from_rc_path_rejects_non_unicode_input`];
/// the FromStr-gate strict-rejection contract on valid-Unicode non-
/// canonical thread-local shared-owned filesystem-path input is pinned
/// by
/// [`tests::test_bump_level_try_from_rc_path_rejects_non_canonical_input`].
///
/// THEORY.md §V.4 typed primitives: the thread-local shared-owned
/// filesystem-path try-conversion parse surface is a typed-primitive
/// site on [`BumpLevel`] itself (one
/// [`TryFrom<std::rc::Rc<std::path::Path>>`] impl routing through
/// [`std::rc::Rc::<std::path::Path>::as_ref`] and the by-reference
/// [`TryFrom<&std::path::Path>`] peer), not a per-consumer
/// `.as_ref().as_os_str().to_str().parse()` composition at every
/// downstream site that types its parse contract as
/// `impl TryFrom<Rc<std::path::Path>>`. THEORY.md §VI.1 one-oracle:
/// the canonical label grammar is named at one site
/// ([`BumpLevel::as_str`]), inverted at one site
/// ([`<BumpLevel as std::str::FromStr>::from_str`]), and every parse
/// surface — including this thread-local shared-owned filesystem-path
/// peer — reads through it.
impl TryFrom<std::rc::Rc<std::path::Path>> for BumpLevel {
    type Error = anyhow::Error;
...
mod tests {
    ...
    /// At every [`BumpLevel`] variant enumerated by
    /// [`BumpLevel::ALL`],
    /// `std::sync::Arc::<std::ffi::OsStr>::from(level)` (the
    /// [`From<BumpLevel> for Arc<std::ffi::OsStr>`] impl body)
    /// yields the same OS-string view as
    /// `std::ffi::OsStr::new(level.as_str())` — the composition through
    /// the canonical-label oracle and the OS-string constructor. Pins
    /// the agreement identity that the by-value shared-owned OS-string
    /// emit surface reads the same canonical label the borrowed-view
    /// [`AsRef<std::ffi::OsStr>`], the by-value owned
    /// [`From<BumpLevel> for std::ffi::OsString`], the by-value
    /// borrowed [`From<BumpLevel> for &'static std::ffi::OsStr`], the
    /// by-value borrowed/owned-frontier
    /// [`From<BumpLevel> for Cow<'static, std::ffi::OsStr>`], and the
    /// by-value shrunk-owned
    /// [`From<BumpLevel> for Box<std::ffi::OsStr>`] surfaces already
    /// read at the OS-string frontier. Structural mirror of
    /// [`test_bump_level_into_arc_str_agrees_with_as_str`] at the
    /// UTF-8 sibling frontier and
    /// [`test_bump_level_into_arc_bytes_agrees_with_as_str_as_bytes`]
    /// at the byte-slice sibling frontier — projected onto the OS-
    /// string frontier this time. Structural mirror of
    /// `test_per_attempt_region_into_arc_os_str_agrees_with_os_str_new_as_str`
    /// (commit aa7a860) at the trio-opener per-attempt-region ladder
    /// and
    /// `test_admission_tier_into_arc_os_str_agrees_with_os_str_new_as_str`
    /// (commit 99f9460) at the mid-trio admission-tier ladder —
    /// closing the shared-owned OS-string emit agreement pin at the
    /// third and last ordered typed sum.
    #[test]
    fn test_bump_level_into_arc_os_str_agrees_with_os_str_new_as_str() {
        for level in BumpLevel::ALL {
            let shared: std::sync::Arc<std::ffi::OsStr> =
                std::sync::Arc::<std::ffi::OsStr>::from(level);
            assert_eq!(
                shared.as_ref(),
                std::ffi::OsStr::new(level.as_str()),
                "From<BumpLevel> for Arc<OsStr> must agree with OsStr::new(as_str()) at {level:?}",
            );
        }
    }

    /// The [`From<BumpLevel> for Arc<std::ffi::OsStr>`] identity
    /// carries through a generic `impl Into<Arc<std::ffi::OsStr>>`
    /// consumer at every [`BumpLevel::ALL`] variant. A tiny generic
    /// function
    /// `fn read<T: Into<Arc<std::ffi::OsStr>>>(t: T) ->
    /// Arc<std::ffi::OsStr> { t.into() }` — the shape of an actual
    /// downstream consumer (a cross-thread cached-OS-string slot typed
    /// as [`Arc<std::ffi::OsStr>`] to share a canonical OS-string label
    /// allocation across worker threads via atomic refcount, a
    /// validated-input newtype wrapper that stores canonical OS-string
    /// labels as [`Arc<std::ffi::OsStr>`] to hand cheap
    ...
    /// lookup allocation) reads the canonical lowercase label directly
    /// from a [`BumpLevel`] value as a shared-owned immutable heap-
    /// owned [`Arc<std::ffi::OsStr>`]. The structural witness that a
    /// [`BumpLevel`] is genuinely usable at
    /// `impl Into<Arc<std::ffi::OsStr>>` call sites — a regression
    /// that drifted the [`From`] impl signature (returning
    /// [`Box<std::ffi::OsStr>`] instead of [`Arc<std::ffi::OsStr>`],
    /// returning an [`std::ffi::OsString`] instead of the shared-owned
    /// handle, requiring `&BumpLevel` and losing the by-value
    /// semantics, or dropping to a
    /// [`Box<std::ffi::OsStr>`]-then-[`Arc::from`] composition that
    /// would allocate twice) fails here at compile time or at the
    /// assertion instead of at every downstream generic call site.
    /// Structural mirror of
    /// `test_per_attempt_region_into_arc_os_str_carries_through_generic_consumer`
    /// (commit aa7a860) at the trio-opener per-attempt-region ladder
    /// and
    /// `test_admission_tier_into_arc_os_str_carries_through_generic_consumer`
    /// (commit 99f9460) at the mid-trio admission-tier ladder.
    #[test]
    fn test_bump_level_into_arc_os_str_carries_through_generic_consumer() {
        ...
        for level in BumpLevel::ALL {
            assert_eq!(
                read(level).as_ref(),
                std::ffi::OsStr::new(level.as_str()),
                "generic Into<Arc<OsStr>> consumer must read canonical label at {level:?}",
            );
        }
    }

    /// The [`From<BumpLevel> for Arc<std::ffi::OsStr>`] shared-owned
    ...
    /// same allocation (identity of the underlying OS-string pointer
    ...
    /// [`test_bump_level_into_arc_str_shares_label_across_clones`]
    ...
    /// byte-slice, and OS-string frontiers. Structural mirror of
    /// `test_per_attempt_region_into_arc_os_str_shares_label_across_clones`
    /// (commit aa7a860) at the trio-opener per-attempt-region ladder
    /// and
    /// `test_admission_tier_into_arc_os_str_shares_label_across_clones`
    /// (commit 99f9460) at the mid-trio admission-tier ladder —
    /// closing the shared-owned OS-string clone-identity pin at the
    /// third and last ordered typed sum.
    #[test]
    fn test_bump_level_into_arc_os_str_shares_label_across_clones() {
        for level in BumpLevel::ALL {
            let shared: std::sync::Arc<std::ffi::OsStr> =
                std::sync::Arc::<std::ffi::OsStr>::from(level);
            ...
            assert_eq!(
                cloned.as_ref(),
                std::ffi::OsStr::new(level.as_str()),
                "Arc<OsStr> clone must read canonical label at {level:?}",
            );
            assert!(
                std::sync::Arc::ptr_eq(&shared, &cloned),
                "Arc<OsStr> clone must share the same underlying allocation at {level:?}",
            );
            assert!(
    ...
    /// At every [`BumpLevel`] variant enumerated by
    /// [`BumpLevel::ALL`],
    /// `std::rc::Rc::<std::ffi::OsStr>::from(level)` (the
    /// [`From<BumpLevel> for Rc<std::ffi::OsStr>`] impl body) yields
    /// the same OS-string view as `std::ffi::OsStr::new(level.as_str())`
    /// — the composition through the canonical-label oracle and the
    /// OS-string constructor. Pins the agreement identity that the
    /// by-value thread-local shared-owned OS-string emit surface reads
    /// the same canonical label the borrowed-view
    /// [`AsRef<std::ffi::OsStr>`], the by-value owned
    /// [`From<BumpLevel> for std::ffi::OsString`], the by-value
    /// borrowed [`From<BumpLevel> for &'static std::ffi::OsStr`], the
    /// by-value borrowed/owned-frontier
    /// [`From<BumpLevel> for Cow<'static, std::ffi::OsStr>`], the by-
    /// value shrunk-owned [`From<BumpLevel> for Box<std::ffi::OsStr>`],
    /// and the by-value atomic-shared-owned
    /// [`From<BumpLevel> for Arc<std::ffi::OsStr>`] surfaces already
    /// read at the OS-string frontier. Structural mirror of
    /// [`test_bump_level_into_rc_str_agrees_with_as_str`] at the UTF-8
    /// sibling frontier and
    /// [`test_bump_level_into_rc_bytes_agrees_with_as_str_as_bytes`]
    /// at the byte-slice sibling frontier, and
    /// [`test_bump_level_into_arc_os_str_agrees_with_os_str_new_as_str`]
    /// at the atomic-shared-owned OS-string sibling one ownership shape
    /// above — projected onto the thread-local shared-owned OS-string
    /// frontier this time. Structural mirror of
    ...
    /// and
    /// `test_admission_tier_into_rc_os_str_agrees_with_os_str_new_as_str`
    /// (commit 2df36e3) at the mid-trio admission-tier ladder — closing
    /// the thread-local shared-owned OS-string emit agreement pin at the
    /// third and last ordered typed sum.
    #[test]
    fn test_bump_level_into_rc_os_str_agrees_with_os_str_new_as_str() {
        for level in BumpLevel::ALL {
            let shared: std::rc::Rc<std::ffi::OsStr> = std::rc::Rc::<std::ffi::OsStr>::from(level);
            assert_eq!(
                shared.as_ref(),
                std::ffi::OsStr::new(level.as_str()),
                "From<BumpLevel> for Rc<OsStr> must agree with OsStr::new(as_str()) at {level:?}",
            );
        }
    }

    /// The [`From<BumpLevel> for Rc<std::ffi::OsStr>`] identity carries
    /// through a generic `impl Into<Rc<std::ffi::OsStr>>` consumer at
    /// every [`BumpLevel::ALL`] variant. A tiny generic function
    ...
    /// than a per-lookup allocation) reads the canonical lowercase
    /// label directly from a [`BumpLevel`] value as a thread-local
    /// shared-owned immutable heap-owned [`Rc<std::ffi::OsStr>`]. The
    /// structural witness that a [`BumpLevel`] is genuinely usable at
    ...
    /// `&BumpLevel` and losing the by-value semantics, or dropping to a
    /// [`Box<std::ffi::OsStr>`]-then-[`Rc::from`] composition that
    /// would allocate twice) fails here at compile time or at the
    /// assertion instead of at every downstream generic call site.
    /// Structural mirror of
    ...
    /// (commit d484f80) at the trio-opener per-attempt-region ladder
    /// and
    /// `test_admission_tier_into_rc_os_str_carries_through_generic_consumer`
    /// (commit 2df36e3) at the mid-trio admission-tier ladder — closing
    /// the thread-local shared-owned OS-string emit generic-consumer
    /// pin at the third and last ordered typed sum.
    #[test]
    fn test_bump_level_into_rc_os_str_carries_through_generic_consumer() {
        ...
        for level in BumpLevel::ALL {
            assert_eq!(
                read(level).as_ref(),
                std::ffi::OsStr::new(level.as_str()),
                "generic Into<Rc<OsStr>> consumer must read canonical label at {level:?}",
            );
        }
    }

    /// The [`From<BumpLevel> for Rc<std::ffi::OsStr>`] thread-local
    /// shared-owned semantics hold across [`std::rc::Rc::clone`] at
    /// every [`BumpLevel::ALL`] variant: the clone reads exactly the
    /// same canonical lowercase label the original reads, points at
    ...
    /// owned receiver contract at the OS-string emit surface — a
    ...
    /// the UTF-8 frontier, the [`Rc<[u8]>`] clone-identity pin
    /// [`test_bump_level_into_rc_bytes_shares_label_across_clones`] at
    /// the byte-slice frontier, and the [`Arc<std::ffi::OsStr>`]
    /// clone-identity pin
    /// [`test_bump_level_into_arc_os_str_shares_label_across_clones`]
    /// one ownership shape above — the four clone-identity pins
    /// together close the structural witness that the receiver actually
    /// holds a thread-local shared-owned [`Rc<std::ffi::OsStr>`] slot
    /// rather than an [`Rc<std::ffi::OsStr>`]-typed wrapper around a
    /// per-clone-allocated [`Box<std::ffi::OsStr>`], across UTF-8,
    /// byte-slice, OS-string, and atomic-shared-owned OS-string
    /// frontiers. Structural mirror of
    /// `test_per_attempt_region_into_rc_os_str_shares_label_across_clones`
    /// (commit d484f80) at the trio-opener per-attempt-region ladder
    /// and
    /// `test_admission_tier_into_rc_os_str_shares_label_across_clones`
    /// (commit 2df36e3) at the mid-trio admission-tier ladder — closing
    /// the thread-local shared-owned OS-string clone-identity pin at
    /// the third and last ordered typed sum.
    #[test]
    fn test_bump_level_into_rc_os_str_shares_label_across_clones() {
        for level in BumpLevel::ALL {
            let shared: std::rc::Rc<std::ffi::OsStr> = std::rc::Rc::<std::ffi::OsStr>::from(level);
            ...
            assert_eq!(
                cloned.as_ref(),
                std::ffi::OsStr::new(level.as_str()),
                "Rc<OsStr> clone must read canonical label at {level:?}",
            );
            assert!(
                std::rc::Rc::ptr_eq(&shared, &cloned),
                "Rc<OsStr> clone must share the same underlying allocation at {level:?}",
            );
            assert!(
                std::rc::Rc::strong_count(&shared) >= 2,
                "Rc<OsStr> strong count must be at least 2 after clone at {level:?}",
            );
        }
    }
    ...
    /// At every [`BumpLevel`] variant enumerated by
    /// [`BumpLevel::ALL`],
    /// `std::rc::Rc::<std::path::Path>::from(level)` (the
    /// [`From<BumpLevel> for Rc<std::path::Path>`] impl body) yields
    /// the same filesystem-path view as
    /// `std::path::Path::new(level.as_str())` — the composition
    ...
    /// [`From<BumpLevel> for std::path::PathBuf`], the by-value
    /// borrowed [`From<BumpLevel> for &'static std::path::Path`], the
    /// by-value borrowed/owned-frontier
    /// [`From<BumpLevel> for Cow<'static, std::path::Path>`], the by-
    /// value shrunk-owned
    /// [`From<BumpLevel> for Box<std::path::Path>`], and the by-value
    /// atomic-shared-owned
    /// [`From<BumpLevel> for Arc<std::path::Path>`] surfaces already
    /// read at the filesystem-path frontier. Structural mirror of
    /// [`test_bump_level_into_rc_str_agrees_with_as_str`] at the
    /// UTF-8 sibling frontier,
    /// [`test_bump_level_into_rc_bytes_agrees_with_as_str_as_bytes`]
    /// at the byte-slice sibling frontier,
    /// [`test_bump_level_into_rc_os_str_agrees_with_os_str_new_as_str`]
    /// at the OS-string sibling frontier directly above, and
    /// [`test_bump_level_into_arc_path_agrees_with_path_new_as_str`]
    /// at the atomic-shared-owned filesystem-path sibling one
    /// ownership shape above — projected onto the thread-local
    /// shared-owned filesystem-path frontier this time. Structural
    /// mirror of
    /// `test_per_attempt_region_into_rc_path_agrees_with_path_new_as_str`
    /// (commit af6b09a) at the trio-opener per-attempt-region ladder
    /// and
    /// `test_admission_tier_into_rc_path_agrees_with_path_new_as_str`
    /// (commit 949d2e3) at the mid-trio admission-tier ladder —
    /// closing the thread-local shared-owned filesystem-path emit
    /// agreement pin at the third and last ordered typed sum.
    #[test]
    fn test_bump_level_into_rc_path_agrees_with_path_new_as_str() {
        for level in BumpLevel::ALL {
            let shared: std::rc::Rc<std::path::Path> = std::rc::Rc::<std::path::Path>::from(level);
            assert_eq!(
                shared.as_ref(),
                std::path::Path::new(level.as_str()),
                "From<BumpLevel> for Rc<Path> must agree with Path::new(as_str()) at {level:?}",
            );
        }
    }

    /// The [`From<BumpLevel> for Rc<std::path::Path>`] identity
    /// carries through a generic `impl Into<Rc<std::path::Path>>`
    /// consumer at every [`BumpLevel::ALL`] variant. A tiny generic
    /// function `fn read<T: Into<Rc<std::path::Path>>>(t: T) ->
    /// Rc<std::path::Path> { t.into() }` — the shape of an actual
    /// downstream consumer (a same-thread cached-filesystem-path slot
    /// typed as [`Rc<std::path::Path>`] to share a canonical
    ...
    /// [`Rc<std::path::Path>`] to hand cheap [`std::rc::Rc::clone`]s
    /// to sibling structures on the same thread, a serde container
    /// that opts into `#[serde(from = "Rc<std::path::Path>")]` at the
    /// thread-local shared-owned filesystem-path frontier, a single-
    /// threaded arena-keyed table whose filesystem-path-value slot
    /// readers want an [`Rc`] clone rather than a per-lookup
    /// allocation, a graph-walk visitor that clones filesystem-path
    /// labels across nodes without needing [`Send`] / [`Sync`]) reads
    /// the canonical lowercase label directly from a [`BumpLevel`]
    /// value as a thread-local shared-owned immutable heap-owned
    /// [`Rc<std::path::Path>`]. The structural witness that a
    /// [`BumpLevel`] is genuinely usable at
    /// `impl Into<Rc<std::path::Path>>` call sites — a regression
    /// that drifted the [`From`] impl signature (returning
    /// [`Box<std::path::Path>`] or [`Arc<std::path::Path>`] instead
    /// of [`Rc<std::path::Path>`], returning a [`std::path::PathBuf`]
    /// instead of the thread-local shared-owned handle, requiring
    /// `&BumpLevel` and losing the by-value semantics, or dropping to
    /// a [`Box<std::path::Path>`]-then-[`Rc::from`] composition that
    /// would allocate twice) fails here at compile time or at the
    /// assertion instead of at every downstream generic call site.
    /// Structural mirror of
    /// `test_per_attempt_region_into_rc_path_carries_through_generic_consumer`
    /// (commit af6b09a) at the trio-opener per-attempt-region ladder
    /// and
    /// `test_admission_tier_into_rc_path_carries_through_generic_consumer`
    /// (commit 949d2e3) at the mid-trio admission-tier ladder —
    /// closing the thread-local shared-owned filesystem-path generic-
    /// consumer pin at the third and last ordered typed sum.
    #[test]
    fn test_bump_level_into_rc_path_carries_through_generic_consumer() {
        ...
        for level in BumpLevel::ALL {
            assert_eq!(
                read(level).as_ref(),
                std::path::Path::new(level.as_str()),
                "generic Into<Rc<Path>> consumer must read canonical label at {level:?}",
            );
        }
    }

    /// The [`From<BumpLevel> for Rc<std::path::Path>`] thread-local
    /// shared-owned semantics hold across [`std::rc::Rc::clone`] at
    /// every [`BumpLevel::ALL`] variant: the clone reads exactly the
    /// same canonical lowercase label the original reads, points at
    /// the same allocation (identity of the underlying filesystem-
    /// path pointer via [`std::rc::Rc::ptr_eq`]), and the non-atomic
    /// refcount lifts to at least two after the clone (via
    /// [`std::rc::Rc::strong_count`]). Pins the thread-local shared-
    /// owned receiver contract at the filesystem-path emit surface —
    /// a regression that drifted the impl body to a non-[`Rc`]
    /// composition ([`Box::<std::path::Path>::from`]-then-ad-hoc-
    /// rewrap, a [`std::path::PathBuf`] intermediate, or a spurious
    /// atomic-tier [`Arc::from`] step) would break the pointer-
    /// identity assertion (each clone would land at a distinct
    /// allocation) or the strong-count symbol even if the canonical-
    /// label bytes still agreed. Structural mirror of the
    /// [`Rc<str>`] clone-identity pin
    /// [`test_bump_level_into_rc_str_shares_label_across_clones`] at
    /// the UTF-8 frontier, the [`Rc<[u8]>`] clone-identity pin
    /// [`test_bump_level_into_rc_bytes_shares_label_across_clones`]
    /// at the byte-slice frontier, the [`Rc<std::ffi::OsStr>`] clone-
    /// identity pin
    /// [`test_bump_level_into_rc_os_str_shares_label_across_clones`]
    /// at the OS-string frontier directly above, and the
    /// [`Arc<std::path::Path>`] clone-identity pin
    /// [`test_bump_level_into_arc_path_shares_label_across_clones`]
    /// at the atomic-shared-owned filesystem-path sibling one
    /// ownership shape above — the five clone-identity pins together
    /// close the structural witness that the receiver actually holds
    /// a thread-local shared-owned [`Rc<std::path::Path>`] slot
    /// rather than an [`Rc<std::path::Path>`]-typed wrapper around a
    /// per-clone-allocated [`Box<std::path::Path>`], across UTF-8,
    /// byte-slice, OS-string, and filesystem-path frontiers.
    /// Structural mirror of
    /// `test_per_attempt_region_into_rc_path_shares_label_across_clones`
    /// (commit af6b09a) at the trio-opener per-attempt-region ladder
    /// and
    /// `test_admission_tier_into_rc_path_shares_label_across_clones`
    /// (commit 949d2e3) at the mid-trio admission-tier ladder —
    /// closing the thread-local shared-owned filesystem-path clone-
    /// identity pin at the third and last ordered typed sum.
    #[test]
    fn test_bump_level_into_rc_path_shares_label_across_clones() {
        for level in BumpLevel::ALL {
            let shared: std::rc::Rc<std::path::Path> = std::rc::Rc::<std::path::Path>::from(level);
            let cloned = std::rc::Rc::clone(&shared);
            assert_eq!(
                cloned.as_ref(),
                std::path::Path::new(level.as_str()),
                "Rc<Path> clone must read canonical label at {level:?}",
            );
            assert!(
                std::rc::Rc::ptr_eq(&shared, &cloned),
                "Rc<Path> clone must share the same underlying allocation at {level:?}",
            );
            assert!(
                std::rc::Rc::strong_count(&shared) >= 2,
    ...
    /// [`TryFrom<std::ffi::CString> for BumpLevel`] agrees with
    /// [`FromStr`] at every [`BumpLevel::ALL`] variant. Owning the
    /// [`std::ffi::CString`] input round-trips through
    /// [`std::ffi::CString::new(level.as_str())`] and the by-value
    /// owned-buffer NUL-terminated C-string try-conversion recovers the
    /// canonical variant — the by-value owned-buffer NUL-terminated
    /// C-string parse peer of the by-reference
    /// [`TryFrom<&std::ffi::CStr>`] surface reads the same one-oracle
    /// grammar the by-value owned-buffer UTF-8 parse peer
    /// [`TryFrom<String>`], the by-value owned-buffer byte-slice parse
    /// peer [`TryFrom<Vec<u8>>`], and the by-value owned-buffer OS-string
    /// parse peer [`TryFrom<std::ffi::OsString>`] read — one round-trip
    /// pin per variant, refuses a future variant insertion that drops
    /// the `TryFrom<CString>`/`CString::new(as_str())` agreement.
    /// Structural mirror of
    /// `test_per_attempt_region_try_from_cstring_agrees_with_from_str`
    /// (commit e73daa7) at the per-attempt-region ladder and
    /// `test_admission_tier_try_from_cstring_agrees_with_from_str`
    /// (commit 77c0cdd) at the admission-tier ladder.
    #[test]
    fn test_bump_level_try_from_cstring_agrees_with_from_str() {
        for level in BumpLevel::ALL {
            let owned = std::ffi::CString::new(level.as_str())
                .expect("canonical label must contain no interior NUL");
            let parsed = <BumpLevel as std::convert::TryFrom<std::ffi::CString>>::try_from(owned)
                .expect("canonical label CString must parse through TryFrom<CString>");
            assert_eq!(
                parsed, level,
                "TryFrom<CString> must round-trip through CString::new(as_str()) at {level:?}",
            );
        }
    }

    /// The [`TryFrom<std::ffi::CString> for BumpLevel`] identity carries
    /// through a generic `impl TryFrom<std::ffi::CString>` consumer at
    /// every [`BumpLevel::ALL`] variant. A tiny generic function
    /// `fn parse<T>(c: CString) -> T where T: TryFrom<CString>,
    /// T::Error: std::fmt::Debug` — the shape of an actual downstream
    /// consumer (a validated-input newtype builder that consumes an
    /// owned [`std::ffi::CString`], a
    /// [`std::ffi::CString::from_vec_with_nul`] receiver that owns the
    /// returned [`std::ffi::CString`] over a FFI-produced buffer, a
    /// [`std::ffi::CStr::from_ptr`] + [`std::ffi::CStr::to_owned`]
    /// compose that owns a freshly-allocated NUL-terminated buffer over
    /// a `*const c_char` FFI return, a generic try-conversion helper) —
    /// recovers the canonical variant from the canonical lowercase-label
    /// owned NUL-terminated C-string at every variant. The structural
    /// witness that a [`BumpLevel`] is genuinely usable at
    /// `impl TryFrom<std::ffi::CString>` call sites — a regression that
    /// drifted the [`TryFrom`] impl signature (requiring a borrowed
    /// [`&std::ffi::CStr`] input, dropping the
    /// [`std::ffi::CString::as_c_str`] borrow step and misparsing
    /// non-UTF-8 input, returning a different variant than [`FromStr`]
    /// would) fails here at compile time or at the assertion instead of
    /// at every downstream generic call site.
    #[test]
    fn test_bump_level_try_from_cstring_carries_through_generic_consumer() {
        fn parse<T>(c: std::ffi::CString) -> T
        where
            T: std::convert::TryFrom<std::ffi::CString>,
            <T as std::convert::TryFrom<std::ffi::CString>>::Error: std::fmt::Debug,
        {
            <T as std::convert::TryFrom<std::ffi::CString>>::try_from(c)
                .expect("canonical label CString must parse through generic TryFrom<CString>")
        }
        ...
        for level in BumpLevel::ALL {
            ...
            assert_eq!(
                ...
                level,
                "generic TryFrom<CString> consumer must recover canonical variant at {level:?}",
            );
        }
    }

    /// [`TryFrom<std::ffi::CString> for BumpLevel`] rejects non-UTF-8
    /// owned NUL-terminated C-string sequences at the
    /// [`std::ffi::CStr::to_str`] decode frontier inherited through the
    /// by-reference [`TryFrom<&std::ffi::CStr>`] delegation. A
    /// [`std::ffi::CString`] may hold any byte sequence excluding
    /// interior NUL — an owned `getenv(3)` payload under a non-UTF-8
    /// locale, an owned C-emitted log frame under a foreign character
    /// set, an owned [`std::ffi::CStr::to_owned`] of a raw
    /// `*const c_char` from a legacy C library. Pins the
    /// encoding-strictness contract at the owned-buffer NUL-terminated
    /// C-string frontier's first strictness gate so a downstream
    /// consumer bound by [`TryFrom<std::ffi::CString>`] inherits the
    /// same UTF-8-only encoding discipline the by-reference peer offers,
    /// at ONE typed-primitive site rather than a per-consumer
    /// [`std::ffi::CString::into_string`] + [`str::parse`] restatement.
    /// Sibling of the by-reference NUL-terminated C-string pin
    /// [`test_bump_level_try_from_cstr_rejects_non_utf8_input`] at the
    /// by-reference peer, the byte-slice-owned-buffer pin
    /// [`test_bump_level_try_from_vec_bytes_rejects_non_utf8_input`] at
    /// the byte-slice frontier, and the OS-string-owned-buffer pin
    ...
    #[test]
    fn test_bump_level_try_from_cstring_rejects_non_utf8_input() {
        for bad in [
            vec![0xffu8],
            vec![0xffu8, 0xfe],
            vec![0x80],
            vec![b'p', b'a', 0xff, b't', b'c', b'h'],
            vec![b'm', b'i', b'n', b'o', b'r', 0xff],
        ] {
            let bad_c = std::ffi::CString::new(bad.clone())
                .expect("test fixture must not contain interior NUL");
            assert!(
                <BumpLevel as std::convert::TryFrom<std::ffi::CString>>::try_from(bad_c).is_err(),
                "TryFrom<CString> must reject non-UTF-8 input {bad:?}",
            );
        }
    }

    /// [`TryFrom<std::ffi::CString> for BumpLevel`] rejects valid-UTF-8
    /// non-canonical owned NUL-terminated C-string sequences at the
    /// underlying [`FromStr`] strictness gate inherited through the
    /// by-reference [`TryFrom<&std::ffi::CStr>`] delegation — empty
    /// owned NUL-terminated C-string, UpperCamel rendering, uppercase,
    /// whitespace padding, and lowercase labels with a dropped character
    /// all reject. Pins the canonical-label strictness contract at the
    /// owned-buffer NUL-terminated C-string frontier's second strictness
    /// gate so a downstream consumer bound by
    /// [`TryFrom<std::ffi::CString>`] inherits the same canonical-only
    /// grammar the direct `.parse::<BumpLevel>()` call sites and the
    /// sibling [`TryFrom<&str>`], [`TryFrom<String>`], [`TryFrom<&[u8]>`],
    /// [`TryFrom<Vec<u8>>`], [`TryFrom<&std::ffi::OsStr>`],
    /// [`TryFrom<std::ffi::OsString>`], and [`TryFrom<&std::ffi::CStr>`]
    /// impls already read, and a future permissive-parse regression at
    /// the underlying [`FromStr`] impl lights up here rather than
    /// drifting silently through the owned-buffer NUL-terminated
    /// C-string try-conversion surface. Sibling of the by-reference
    /// NUL-terminated C-string pin
    /// [`test_bump_level_try_from_cstr_rejects_non_canonical_input`] at
    /// the by-reference peer, the UTF-8-owned-buffer pin
    /// [`test_bump_level_try_from_string_rejects_non_canonical_input`]
    /// at the UTF-8 frontier, the byte-slice-owned-buffer pin
    /// [`test_bump_level_try_from_vec_bytes_rejects_non_canonical_input`]
    /// at the byte-slice frontier, and the OS-string-owned-buffer pin
    /// [`test_bump_level_try_from_os_string_rejects_non_canonical_input`]
    /// at the OS-string frontier — the five pins together close the
    /// canonical-only strictness contract across the UTF-8, byte-slice,
    /// OS-string, and NUL-terminated C-string frontiers' owned-buffer
    /// parse surfaces.
    #[test]
    fn test_bump_level_try_from_cstring_rejects_non_canonical_input() {
        for bad in [
    ...
    /// [`TryFrom<Arc<std::ffi::CStr>> for BumpLevel`] agrees with
    /// [`FromStr`] at every [`BumpLevel::ALL`] variant. Owning the
    /// [`std::sync::Arc<std::ffi::CStr>`] input round-trips through
    /// [`std::sync::Arc::<std::ffi::CStr>::from`] of the shrunk-owned
    /// [`Box<std::ffi::CStr>`] view of
    /// [`std::ffi::CString::new(level.as_str()).into_boxed_c_str()`] and
    /// the by-value shared-owned NUL-terminated C-string try-conversion
    /// recovers the canonical variant — the shared-owned NUL-terminated
    /// C-string parse peer of the by-reference
    /// [`TryFrom<&std::ffi::CStr>`] surface reads the same one-oracle
    /// grammar the shared-owned UTF-8-string parse peer
    /// [`TryFrom<Arc<str>>`], the shared-owned byte-slice parse peer
    /// [`TryFrom<Arc<[u8]>>`], the shared-owned OS-string parse peer
    /// [`TryFrom<Arc<std::ffi::OsStr>>`], and the shared-owned
    /// filesystem-path parse peer [`TryFrom<Arc<std::path::Path>>`]
    /// read — one round-trip pin per variant, refuses a future variant
    /// insertion that drops the
    /// `TryFrom<Arc<CStr>>`/`Arc::<CStr>::from(CString::new(as_str()).into_boxed_c_str())`
    /// agreement. Structural mirror of
    /// `test_per_attempt_region_try_from_arc_cstr_agrees_with_from_str`
    /// (commit d0e9b09) at the per-attempt-region ladder and
    /// `test_admission_tier_try_from_arc_cstr_agrees_with_from_str`
    /// (commit 57e7b7b) at the admission-tier ladder.
    #[test]
    fn test_bump_level_try_from_arc_cstr_agrees_with_from_str() {
        for level in BumpLevel::ALL {
            let boxed: Box<std::ffi::CStr> = std::ffi::CString::new(level.as_str())
                .expect("canonical label must contain no interior NUL")
                .into_boxed_c_str();
            let shared: std::sync::Arc<std::ffi::CStr> =
                std::sync::Arc::<std::ffi::CStr>::from(boxed);
            let parsed =
                <BumpLevel as std::convert::TryFrom<std::sync::Arc<std::ffi::CStr>>>::try_from(
                    shared,
                )
                .expect("canonical label Arc<CStr> must parse through TryFrom<Arc<CStr>>");
            assert_eq!(
                parsed, level,
                "TryFrom<Arc<CStr>> must round-trip through Arc::<CStr>::from(CString::new(as_str()).into_boxed_c_str()) at {level:?}",
            );
        }
    }

    /// The [`TryFrom<Arc<std::ffi::CStr>> for BumpLevel`] identity
    /// carries through a generic
    /// `impl TryFrom<std::sync::Arc<std::ffi::CStr>>` consumer at every
    /// [`BumpLevel::ALL`] variant. A tiny generic function
    /// `fn parse<T>(c: Arc<CStr>) -> T where T:
    /// TryFrom<Arc<CStr>>, T::Error: std::fmt::Debug` — the shape of
    /// an actual downstream consumer (a
    /// [`std::sync::Arc::<std::ffi::CStr>::from`]`(Box<CStr>)` receiver
    /// that shares the shrunk NUL-terminated buffer across threads
    /// through an atomic-refcount header, a
    /// [`std::ffi::CStr::from_ptr`] + [`std::ffi::CStr::to_owned`] +
    /// [`std::ffi::CString::into_boxed_c_str`] +
    /// [`std::sync::Arc::<std::ffi::CStr>::from`] compose that shares a
    /// freshly-allocated NUL-terminated buffer over a `*const c_char`
    /// FFI return, a serde `#[serde(try_from = "Arc<CStr>")]` container,
    /// a generic try-conversion helper) — recovers the canonical
    /// variant from the canonical lowercase-label shared-owned
    /// NUL-terminated C-string at every variant. The structural witness
    /// that a [`BumpLevel`] is genuinely usable at
    /// `impl TryFrom<std::sync::Arc<std::ffi::CStr>>` call sites — a
    ...
    /// [`Box<std::ffi::CStr>`] input, dropping the
    /// [`std::sync::Arc::<std::ffi::CStr>::as_ref`] borrow step and
    /// misparsing non-UTF-8 input, returning a different variant than
    /// [`FromStr`] would) fails here at compile time or at the
    /// assertion instead of at every downstream generic call site.
    #[test]
    fn test_bump_level_try_from_arc_cstr_carries_through_generic_consumer() {
        fn parse<T>(c: std::sync::Arc<std::ffi::CStr>) -> T
        where
            T: std::convert::TryFrom<std::sync::Arc<std::ffi::CStr>>,
            <T as std::convert::TryFrom<std::sync::Arc<std::ffi::CStr>>>::Error: std::fmt::Debug,
        {
            <T as std::convert::TryFrom<std::sync::Arc<std::ffi::CStr>>>::try_from(c)
                .expect("canonical label Arc<CStr> must parse through generic TryFrom<Arc<CStr>>")
        }
        ...
        for level in BumpLevel::ALL {
            ...
            assert_eq!(
                ...
                level,
                "generic TryFrom<Arc<CStr>> consumer must recover canonical variant at {level:?}",
            );
        }
    }

    /// [`TryFrom<Arc<std::ffi::CStr>> for BumpLevel`] rejects non-UTF-8
    /// shared-owned NUL-terminated byte sequences at the
    /// [`std::ffi::CStr::to_str`] decode frontier inherited through
    /// the [`std::sync::Arc::<std::ffi::CStr>::as_ref`] +
    /// [`TryFrom<&std::ffi::CStr>`] delegation. An
    /// [`std::sync::Arc<std::ffi::CStr>`] may hold any byte sequence
    /// excluding interior NUL — a shared-owned counterpart of the
    /// borrowed [`&std::ffi::CStr`] non-UTF-8 fixtures returned by
    /// legacy C libraries under foreign locales, and of the shrunk-
    /// owned [`Box<std::ffi::CStr>`] non-UTF-8 fixtures the sibling
    /// [`test_bump_level_try_from_box_cstr_rejects_non_utf8_input`]
    /// pin exercises. Pins the encoding-strictness contract at the
    /// shared-owned NUL-terminated C-string frontier's first
    /// strictness gate, matching the by-reference peer
    /// [`test_bump_level_try_from_cstr_rejects_non_utf8_input`]
    /// discipline at the shared-owned axis — the shared-owned buffer
    /// pays the same UTF-8-strictness at the same decode frontier a
    /// direct [`std::ffi::CStr::to_str`] + [`str::parse`] composition
    /// would offer, at ONE typed-primitive site rather than a per-
    /// consumer two-step restatement.
    #[test]
    fn test_bump_level_try_from_arc_cstr_rejects_non_utf8_input() {
        for bad in [
            &[0xffu8, 0u8][..],
            &[0xffu8, 0xfe, 0u8][..],
            &[0x80u8, 0u8][..],
            ...
            assert!(
                ...
                .is_err(),
                "TryFrom<Arc<CStr>> must reject non-UTF-8 input {bad:?}",
            );
        }
    }

    /// [`TryFrom<Arc<std::ffi::CStr>> for BumpLevel`] rejects
    /// valid-UTF-8 non-canonical shared-owned NUL-terminated C-string
    /// sequences at the underlying [`FromStr`] strictness gate
    /// inherited through the
    /// [`std::sync::Arc::<std::ffi::CStr>::as_ref`] +
    /// [`TryFrom<&std::ffi::CStr>`] + [`TryFrom<&str>`] delegation —
    ...
    /// uppercase, whitespace padding, and lowercase labels with a
    /// dropped character all reject. Pins the canonical-label
    /// strictness contract at the shared-owned NUL-terminated
    /// C-string frontier's second strictness gate so a downstream
    /// consumer bound by [`TryFrom<std::sync::Arc<std::ffi::CStr>>`]
    /// inherits the same canonical-only grammar the direct
    /// `.parse::<BumpLevel>()` call sites and the sibling
    /// [`TryFrom<Arc<str>>`], [`TryFrom<Arc<[u8]>>`],
    /// [`TryFrom<Arc<std::ffi::OsStr>>`],
    /// [`TryFrom<Arc<std::path::Path>>`],
    /// [`TryFrom<&std::ffi::CStr>`],
    /// [`TryFrom<std::ffi::CString>`], and
    /// [`TryFrom<Box<std::ffi::CStr>>`] impls already read, and a
    /// future permissive-parse regression at the underlying
    /// [`FromStr`] impl lights up here rather than drifting silently
    /// through the shared-owned NUL-terminated C-string try-conversion
    /// surface. Sibling of the by-reference peer
    /// [`test_bump_level_try_from_cstr_rejects_non_canonical_input`],
    /// the owned-buffer peer
    /// [`test_bump_level_try_from_cstring_rejects_non_canonical_input`],
    /// and the shrunk-owned peer
    /// [`test_bump_level_try_from_box_cstr_rejects_non_canonical_input`]
    /// at the NUL-terminated C-string frontier — the four pins together
    /// close the canonical-only strictness contract across the
    /// borrowed-view, owned-buffer, shrunk-owned, and shared-owned axes
    /// of the NUL-terminated C-string frontier.
    #[test]
    fn test_bump_level_try_from_arc_cstr_rejects_non_canonical_input() {
        for bad in [
            "", "Patch", "Minor", "Major", "PATCH", " patch", "patch ", "pat",
        ] {
            let bad_c =
    ...
    /// [`TryFrom<std::rc::Rc<std::path::Path>> for BumpLevel`] recovers
    /// the original variant at every [`BumpLevel::ALL`] variant when
    /// the canonical label emitted by [`BumpLevel::as_str`] is
    /// materialized as a [`std::path::PathBuf`], shrunk to a
    /// [`Box<std::path::Path>`] via
    /// [`std::path::PathBuf::into_boxed_path`], and lifted to a
    ...
    /// canonical-label oracle at the trio-closing slot of the thread-
    /// local shared-owned filesystem-path parse trio, structural mirror
    /// of
    /// [`crate::retry::tests::test_per_attempt_region_try_from_rc_path_agrees_with_from_str`]
    /// carried at the first ordered typed sum (opening slot at
    /// 4497115) and
    /// [`crate::probe_outcome::tests::test_admission_tier_try_from_rc_path_agrees_with_from_str`]
    /// carried at the second ordered typed sum (mid-trio slot at
    /// c973b49).
    #[test]
    fn test_bump_level_try_from_rc_path_agrees_with_from_str() {
        for level in BumpLevel::ALL {
            let boxed: Box<std::path::Path> =
                std::path::PathBuf::from(level.as_str()).into_boxed_path();
            let rc: std::rc::Rc<std::path::Path> = std::rc::Rc::from(boxed);
            let parsed =
                <BumpLevel as std::convert::TryFrom<std::rc::Rc<std::path::Path>>>::try_from(rc)
                    .expect("canonical Rc<Path> must parse through TryFrom<Rc<Path>>");
            assert_eq!(
                parsed, level,
                "TryFrom<Rc<Path>> must round-trip at {level:?}",
            );
        }
    }

    /// The [`TryFrom<std::rc::Rc<std::path::Path>> for BumpLevel`]
    /// identity carries through a generic
    ...
    /// contract) — recovers the canonical variant at every variant.
    /// Structural witness that a [`BumpLevel`] is genuinely usable at
    /// `impl TryFrom<Rc<std::path::Path>>` call sites — a regression
    /// that drifted the [`TryFrom`] impl signature fails here at
    /// compile time or at the assertion instead of at every downstream
    /// generic call site.
    #[test]
    fn test_bump_level_try_from_rc_path_carries_through_generic_consumer() {
        ...
        for level in BumpLevel::ALL {
            ...
            assert_eq!(
                parse::<BumpLevel>(rc),
                level,
                "generic TryFrom<Rc<Path>> consumer must recover canonical variant at {level:?}",
            );
        }
    }

    /// [`TryFrom<std::rc::Rc<std::path::Path>> for BumpLevel`] rejects
    /// non-Unicode filesystem-path sequences at the
    /// [`std::ffi::OsStr::to_str`] Unicode-decode frontier reached
    /// through [`std::path::Path::as_os_str`] via
    ...
    /// the only stable public API for constructing a non-Unicode
    /// [`std::ffi::OsString`] view is via
    /// [`std::os::unix::ffi::OsStringExt::from_vec`]. Structural mirror
    /// of
    /// [`crate::probe_outcome::tests::test_admission_tier_try_from_rc_path_rejects_non_unicode_input`]
    /// at the mid-trio slot and
    /// [`crate::retry::tests::test_per_attempt_region_try_from_rc_path_rejects_non_unicode_input`]
    /// at the opening slot.
    #[cfg(unix)]
    #[test]
    fn test_bump_level_try_from_rc_path_rejects_non_unicode_input() {
        use std::os::unix::ffi::OsStringExt;
    ...
    /// valid-Unicode non-canonical filesystem-path sequences with the
    /// same strictness [`std::str::FromStr`] enforces — empty string,
    /// UpperCamel rendering (`"Patch"`, `"Minor"`, `"Major"`),
    /// uppercase (`"PATCH"`), whitespace padding, and truncated stem
    /// (`"pat"`) all reject after the Unicode-decode stage passes.
    /// Pins the FromStr-gate strict-rejection contract at the thread-
    /// local shared-owned filesystem-path try-conversion surface so a
    /// downstream consumer bound by
    /// [`TryFrom<std::rc::Rc<std::path::Path>>`] inherits the same
    /// canonical-only grammar the direct `.parse::<BumpLevel>()` call
    /// sites and the sibling [`TryFrom<&std::path::Path>`],
    /// [`TryFrom<std::path::PathBuf>`],
    /// [`TryFrom<Cow<'_, std::path::Path>>`],
    /// [`TryFrom<Box<std::path::Path>>`], and
    /// [`TryFrom<std::sync::Arc<std::path::Path>>`] impls already read.
    #[test]
    fn test_bump_level_try_from_rc_path_rejects_non_canonical_input() {
        for bad in [
            "", "Patch", "Minor", "Major", "PATCH", " patch", "patch ", "pat",
        ] {
            let boxed: Box<std::path::Path> = std::path::PathBuf::from(bad).into_boxed_path();
