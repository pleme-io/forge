...
/// By-value borrowed-or-owned-UTF-8 emit peer for [`StorePath`] via
/// [`std::borrow::Cow<'static, str>`] — the sixth step of the by-value
...
/// [`From<StorePath> for Vec<u8>`] (line 3841), and
/// [`From<StorePath> for Box<str>`] (line 3970), chaining through the same
/// [`StorePath::into_string`] one-oracle discipline every owned-emit peer
/// in the family routes through.
///
/// Delegates through `std::borrow::Cow::Owned(String::from(sp))`:
/// `String::from(sp)` moves [`StorePath::full`] out at zero-copy via
/// [`StorePath::into_string`]; the moved [`String`] is then wrapped in the
/// [`Cow::Owned`] arm at the `'static` lifetime (the arm requires no
/// borrow, so it satisfies any output lifetime the trait signature demands,
/// including `'static`). Zero re-encoding: the owned arm carries the same
/// [`String`] buffer that [`From<StorePath> for String`] emits, so a
/// downstream site that consumes the [`Cow`] by
/// [`std::borrow::Cow::into_owned`] recovers the exact backing buffer the
/// direct-[`String`] peer would hand over.
///
/// # Why the trait peer earns its keep
///
/// [`AsRef<str> for StorePath`] (line 1684) covers the borrowed-view UTF-8
/// frontier at zero allocation. [`From<StorePath> for String`] (line 3461)
/// covers the by-value owned-UTF-8 frontier when the sink pins its
/// contract on the [`String`] shape. [`From<StorePath> for Box<str>`]
/// (line 3970) covers the shrunk-owned UTF-8 frontier when the sink pins
/// its contract on the payload-sized [`Box<str>`] shape.
/// [`From<StorePath> for Cow<'static, str>`] covers the disjoint
/// borrowed-or-owned UTF-8 frontier the [`String`] and [`Box<str>`] emits
/// can never reach without a per-site [`Cow::Owned`] re-statement:
///
/// - `serde_json::Value::String(Cow<'static, str>)`-shaped tagged unions
///   and `serde`-derived records with `Cow<'static, str>` fields (the
///   crate idiom for a field that accepts either a compile-time
///   `&'static str` literal or a runtime-owned buffer through the same
///   type slot). Pre-peer the site had to write
///   `Cow::Owned(sp.into_string())` (the [`Cow::Owned`] wrap re-stated at
///   every site); post-peer `sp.into()` reads through this impl.
/// - `fn f(input: impl Into<Cow<'static, str>>)` sink signatures — a
///   stdlib idiom for accepting `&'static str` literals AND runtime
///   [`String`] values through the same argument slot at zero allocation
///   on the literal path and one move on the owned path (no per-arm
///   restatement at the call site). Pre-peer the call site had to write
///   `f(Cow::Owned(sp.into_string()))`; post-peer `f(sp)` reads through
///   this impl.
/// - `Result<Cow<'static, str>>`-returning wrappers preserving a
///   pre-existing public shape — a canonical-label extractor that types
///   its return as `Result<Cow<'static, str>>` (a shape that lets a
///   compile-time-known static-label branch coexist with a runtime-emit
///   dynamic-label branch through the same return type). Pre-peer
///   `Ok(Cow::Owned(result.store_path.into_string()))` restated the wrap
///   at every site; post-peer `Ok(result.store_path.into())` reads
///   through this impl.
/// - `Cow<'static, str>`-keyed intern tables and `Vec<Cow<'static, str>>`
///   pools that accept both compile-time-known static-label entries and
///   runtime-owned entries through the same collection slot. This impl
///   reaches every `impl Into<Cow<'static, str>>` consumer at zero
...
/// [`Box<str>`] emit peers already route through, so a future refinement
/// to the canonical form (a tighter trim, a `SmartString` handle, a
/// canonicalising projection at the emit surface) lands at one site and
/// every owned-emit peer inherits it automatically. A per-impl direct
/// `Cow::Owned(sp.into_string())` here would work today but would ossify
/// the "String backing" assumption at every emit surface; the
/// `Cow::Owned(String::from(sp))` chain reads through the reference-
/// frontier opening peer so the family stays composable by construction.
///
/// # Symmetric closure with the parse frontier
///
/// The symmetric emit-side sibling of [`TryFrom<Cow<'_, str>> for StorePath`]
/// (line 568): the two together close the by-value borrowed-or-owned-UTF-8
/// input+output symmetry on the store-path grammar — [`TryFrom<Cow<'_, str>>`]
/// parses a canonical `/nix/store/<hash>-<name>` from either arm of a
/// borrowed-or-owned [`Cow`] through the [`StorePath::parse`] oracle
/// (borrowed arm reads directly, owned arm dereferences the [`String`]
/// buffer, both routing through [`FromStr::from_str`] at line 402), this
/// [`From<StorePath>`] emits the canonical `/nix/store/<hash>-<name>` as
/// the owned arm of a [`Cow<'static, str>`] through the moved backing
/// buffer. A consumer that receives a borrowed-or-owned UTF-8 buffer from
/// a `serde` field or a helper-function argument, parses it into a
/// [`StorePath`], validates it, and hands it back out as a
/// [`Cow<'static, str>`] at its own downstream boundary reads through the
/// same one-oracle discipline the parse peer already carries with zero
/// per-consumer bridge cost — a full `Cow<'_, str> → StorePath →
/// Cow<'static, str>` round-trip pinned by
/// [`tests::test_from_store_path_cow_str_parse_round_trip`].
///
/// # Identity invariants pinned
///
/// - `Cow::<str>::from(sp.clone()) == Cow::Owned(sp.as_str().to_owned())`
///   (equality with the borrowed-view UTF-8 projection routed through the
///   same [`Cow::Owned`] arm) — pinned by
///   [`tests::test_from_store_path_cow_str_matches_borrowed_projections`].
/// - Trim-discipline preservation: a `\n`-terminated raw input emits a
///   [`Cow<'static, str>`] whose bytes are the trimmed canonical bytes —
///   pinned by [`tests::test_from_store_path_cow_str_carries_trim_discipline`].
/// - Trait-generic composition: an `impl Into<Cow<'static, str>>`
///   consumer recovers the same validated bytes a direct
///   `Cow::Owned(sp.into_string())` would — pinned by
///   [`tests::test_from_store_path_cow_str_carries_through_generic_consumer`].
...
///   fabricating a `'static` reference from a runtime buffer) — pinned by
///   [`tests::test_from_store_path_cow_str_matches_borrowed_projections`].
/// - Round-trip fidelity: the emitted [`Cow<'static, str>`] parses back
///   through [`TryFrom<Cow<'_, str>>`] to the same [`StorePath`] — pinned
///   by [`tests::test_from_store_path_cow_str_parse_round_trip`].
///
/// THEORY.md §III typed primitives: the by-value borrowed-or-owned-UTF-8
/// emit surface is a typed-primitive peer on [`StorePath`], not a
/// per-consumer `Cow::Owned(sp.into_string())` or
/// `Cow::Owned(<StorePath as AsRef<str>>::as_ref(&sp).to_owned())`
...
/// projected through the [`StorePath::into_string`] owned-projection
/// accessor, exposed as an owned [`String`] via
/// [`From<StorePath> for String`]), and this borrowed-or-owned-[`Cow`]
/// emit surface reads through the same canonical-projection chain — a
/// future divergence between the emit peer and the borrowed
/// [`AsRef<str>`] projection would fail the pinning invariance test.
impl From<StorePath> for std::borrow::Cow<'static, str> {
    fn from(sp: StorePath) -> std::borrow::Cow<'static, str> {
        std::borrow::Cow::Owned(String::from(sp))
    }
}
...
/// By-value cross-thread shared-owned-byte-slice emit peer for
/// [`StorePath`] via [`std::sync::Arc<[u8]>`] — the tenth step of the
...
/// [`From<StorePath> for std::rc::Rc<str>`] (line 4427), and
/// [`From<StorePath> for Box<[u8]>`] (line 4596), chaining through the
/// same [`StorePath::into_string`] one-oracle discipline every owned-emit
/// peer in the family routes through.
///
/// Delegates through `std::sync::Arc::<[u8]>::from(Vec::<u8>::from(sp))`:
...
/// [`std::sync::Arc`]`::<[u8]>::from(Vec<u8>)` impl then constructs a
/// single fresh allocation carrying the atomic refcount header alongside
/// the moved byte payload. The moved [`Vec<u8>`] backing is dropped after
/// the payload copy, but the copy is unavoidable: [`std::sync::Arc<[u8]>`]
/// and [`Vec<u8>`] have different memory layouts (an [`Arc<[u8]>`]
/// allocation prefixes the payload with a
/// [`std::sync::atomic::AtomicUsize`] strong-count header and an
/// [`std::sync::atomic::AtomicUsize`] weak-count header, while a
/// [`Vec<u8>`] carries a `(ptr, len, cap)` triple pointing at a
/// header-less heap-allocated buffer). No re-encoding: the moved bytes
/// are the same bytes [`From<StorePath> for Vec<u8>`] emits, projected
/// through [`std::sync::Arc<[u8]>`]'s allocation shape one time and shared
/// cheaply across every subsequent [`std::sync::Arc::clone`] recipient at
/// zero buffer-clone cost per downstream consumer.
///
/// # Why the trait peer earns its keep
///
/// [`AsRef<[u8]> for StorePath`] (line 2001) covers the borrowed-view
...
/// [`From<StorePath> for Arc<[u8]>`] covers the disjoint cross-thread
/// shared-owned byte-slice frontier the [`Vec<u8>`] and [`Box<[u8]>`]
/// emits can never reach without a per-site `Arc::<[u8]>::from(...)`
/// re-statement — and which pays a real cost when substituted with the
/// non-shared [`Vec<u8>`] or [`Box<[u8]>`] shapes on a multi-thread
/// fan-out, since every recipient thread would otherwise own its own
/// buffer clone instead of sharing one allocation through atomic
/// refcount bumps:
///
/// - `struct { payload: Arc<[u8]> }` field initializers on a
///   message-passing record — a worker-pool byte payload whose validated
///   store-path label is handed across worker threads through
///   [`std::sync::Arc::clone`] at zero buffer-clone cost per recipient,
///   a Tokio-task pool that ships a canonical byte-slice payload into
///   every spawned worker without allocating per task, a
///   channel-carried wire descriptor that fans a single validated byte
///   allocation out to N receivers. Pre-peer the init site had to write
///   `std::sync::Arc::<[u8]>::from(sp.into_string().into_bytes())` (the
///   two-step [`Arc<[u8]>`] projection restated at every field-init
///   site); post-peer the [`StorePath`] value is moved directly into the
///   field.
/// - `#[serde(into = "Arc<[u8]>")]` — the [`serde_bytes`]-adjacent serde
///   container attribute for the cross-thread shared-owned byte-slice
///   emit case (a serializer that writes an atomically refcounted
///   raw-byte label buffer into the wire form so downstream
///   deserializers can hold cheap-clone shared byte ownership rather
///   than each allocating a copy) keys off
///   [`From<StorePath> for Arc<[u8]>`], not
///   [`From<StorePath> for Vec<u8>`],
///   [`From<StorePath> for Box<[u8]>`], or
///   [`From<StorePath> for Arc<str>`]. A future serde field wrapping a
///   [`StorePath`] and opting into the cross-thread shared-owned raw-byte
///   `into` grammar reaches this emit peer without a shim.
/// - `Result<Arc<[u8]>>`-returning wrappers preserving a pre-existing
///   public shape — a canonical-byte-slice extractor that types its
///   return as `Result<Arc<[u8]>>` so every fan-out branch downstream
///   shares the same heap allocation through refcount bumps rather than
///   buffer clones. Pre-peer
///   `Ok(std::sync::Arc::<[u8]>::from(result.store_path.into_string().into_bytes()))`
///   restated the two-step wrap at every site; post-peer
///   `Ok(result.store_path.into())` reads through this impl.
/// - `HashMap<Arc<[u8]>, _>::insert` intern tables — a
///   byte-addressed closure-scan or attic-push staging pool that keys
///   entries off canonically-projected store-path byte labels shared
///   cheaply across worker threads through [`std::sync::Arc::clone`]
///   instead of buffer-cloned. Sibling peer to the parse-side
///   [`TryFrom<Arc<[u8]>> for StorePath`] at line 1294 — an intern table
///   populated by parsing raw byte labels through the parse peer and
///   re-emitting canonical byte labels through this emit peer routes
///   through both peers at zero per-consumer bridge cost.
/// - Generic sinks bounded by `impl Into<Arc<[u8]>>` — any
///   content-addressed byte-payload sink, any tracing-crate span-attribute
///   setter accepting a shared byte buffer, any Tokio-task spawn-argument
///   slot that accepts `impl Into<Arc<[u8]>>` when it wants a
///   cross-thread shared-owned raw-byte buffer. This impl reaches every
///   `impl Into<Arc<[u8]>>` consumer at zero per-site bridge cost.
///
/// The chain-through-[`From<StorePath> for Vec<u8>`] design is the
/// one-oracle discipline THEORY §VI.1 demands: the moved-out canonical
/// bytes travel through the same [`StorePath::into_string`] projection
/// every owned-emit peer routes through — [`Vec<u8>`] moves the backing
/// buffer out of the [`String`] wrapper, this [`Arc<[u8]>`] peer then
/// hands that owned buffer to [`std::sync::Arc<[u8]>`]'s single-copy
/// shared-allocation constructor — so a future refinement to the
/// canonical form (a tighter trim, a `SmartString` handle, a
/// canonicalising projection at the emit surface) lands at one site and
/// every owned-emit peer inherits it automatically. A per-impl direct
/// call to
/// `std::sync::Arc::<[u8]>::from(sp.into_string().into_bytes())` here
/// would work today but would ossify the "String backing" assumption at
/// every emit surface; the
/// `std::sync::Arc::<[u8]>::from(Vec::<u8>::from(sp))` chain reads
/// through the owned-byte-vector emit peer, which itself reads through
/// the reference-frontier opening peer, so the family stays composable
/// by construction.
///
/// # Symmetric closure with the parse frontier
///
/// The symmetric emit-side sibling of
/// [`TryFrom<Arc<[u8]>> for StorePath`] (line 1294): the two together
/// close the by-value cross-thread shared-owned-byte-slice input+output
/// symmetry on the store-path grammar — [`TryFrom<Arc<[u8]>>`] parses a
/// canonical `/nix/store/<hash>-<name>` from an atomically refcounted
/// [`Arc<[u8]>`] through the [`StorePath::parse`] oracle (via
/// byte-slice delegation), this [`From<StorePath>`] emits the canonical
/// `/nix/store/<hash>-<name>` as an atomically refcounted [`Arc<[u8]>`]
/// through one fresh allocation whose contents are the moved-out
/// canonical bytes. A consumer that receives a cross-thread shared-owned
/// raw-byte buffer from a Tokio-task boundary or a channel receiver,
/// parses it into a [`StorePath`], validates it, and hands it back out
/// as an [`Arc<[u8]>`] at its own downstream fan-out reads through the
/// same one-oracle discipline the parse peer already carries with zero
/// per-consumer bridge cost — a full
/// `Arc<[u8]> → StorePath → Arc<[u8]>` round-trip pinned by
/// [`tests::test_from_store_path_arc_bytes_parse_round_trip`].
///
/// Also the byte-slice sibling of [`From<StorePath> for Arc<str>`]
/// (line 4261): the two together close the cross-thread shared-owned
/// emit frontier on both the UTF-8-string axis and the raw-byte-slice
/// axis of the reference-grammar family, so a downstream site that types
/// both its UTF-8-payload and its raw-byte-payload boundary as
/// cross-thread shared-owned (a serde container that opts into
/// `#[serde(into = "Arc<str>")]` on one field AND
/// `#[serde(into = "Arc<[u8]>")]` on another, a validated-output emitter
/// whose canonical text-facing AND byte-facing sinks are both stated as
/// cross-thread shared-owned) reaches both through the same primitive
/// without a per-consumer restatement of either shared-allocation
/// discipline.
///
/// # Identity invariants pinned
///
/// - `<std::sync::Arc<[u8]>>::from(sp.clone()) == sp.as_str().as_bytes().into()`
///   (equality with the borrowed-view byte-slice projection routed through
///   the same [`Arc<[u8]>`] shape) — pinned by
///   [`tests::test_from_store_path_arc_bytes_matches_borrowed_projections`].
/// - Trim-discipline preservation: a `\n`-terminated raw input emits an
///   [`Arc<[u8]>`] whose bytes are the trimmed canonical bytes — pinned
///   by [`tests::test_from_store_path_arc_bytes_carries_trim_discipline`].
/// - Trait-generic composition: an `impl Into<Arc<[u8]>>` consumer
///   recovers the same validated bytes a direct
///   `std::sync::Arc::<[u8]>::from(sp.into_string().into_bytes())` would
///   — pinned by
///   [`tests::test_from_store_path_arc_bytes_carries_through_generic_consumer`].
/// - Round-trip fidelity: the emitted [`Arc<[u8]>`] parses back through
///   [`TryFrom<Arc<[u8]>>`] to the same [`StorePath`] — pinned by
///   [`tests::test_from_store_path_arc_bytes_parse_round_trip`].
///
/// THEORY.md §III typed primitives: the by-value cross-thread
/// shared-owned-byte-slice emit surface is a typed-primitive peer on
/// [`StorePath`], not a per-consumer
/// `std::sync::Arc::<[u8]>::from(sp.into_string().into_bytes())` or
/// `std::sync::Arc::<[u8]>::from(<StorePath as AsRef<[u8]>>::as_ref(&sp))`
/// restatement at every downstream site that accepts
/// `impl Into<Arc<[u8]>>`.
/// THEORY.md §VI.1 one-oracle: the validated full-path string is named
/// at one site ([`StorePath::parse`]-guarded [`StorePath::full`]
...
/// owned-projection accessor, exposed as an owned [`String`] via
/// [`From<StorePath> for String`], moved into an owned [`Vec<u8>`] via
/// [`From<StorePath> for Vec<u8>`]), and this cross-thread
/// shared-owned-[`Arc<[u8]>`] emit surface reads through the same
/// canonical-projection chain — a future divergence between the emit
/// peer and the borrowed [`AsRef<[u8]>`] projection would fail the
/// pinning invariance test.
impl From<StorePath> for std::sync::Arc<[u8]> {
    fn from(sp: StorePath) -> std::sync::Arc<[u8]> {
        std::sync::Arc::<[u8]>::from(Vec::<u8>::from(sp))
    }
}
...
/// By-reference owned-UTF-8 emit peer for [`StorePath`] via [`String`] —
/// opens the by-reference emit frontier the by-value
/// [`From<StorePath> for String`] (line 3461) closure named as next in
/// line after the by-value byte-side frontier
/// ([`From<StorePath> for std::borrow::Cow<'static, [u8]>`]) sealed.
///
/// Delegates through `<StorePath as AsRef<str>>::as_ref(sp).to_owned()`:
/// the borrowed-view [`AsRef<str>`] read peer (line 1684) returns the
/// canonical `/nix/store/<hash>-<name>` slice off the same
/// [`StorePath::full`] backing the by-value emit peer moves out through
/// [`StorePath::into_string`]; [`str::to_owned`] copies the trimmed bytes
/// into a fresh [`String`] without touching the source [`StorePath`],
/// leaving it available for further reads. Zero re-encoding — the same
/// validated bytes the [`StorePath::parse`] oracle proved
/// `/nix/store/<32-char-base32-hash>-<name>`-shaped reach the owned-UTF-8
/// frontier without materialising the intermediate [`String`] the by-value
/// peer would move out (which would require consuming the value).
///
/// # Why the trait peer earns its keep
///
/// [`From<StorePath> for String`] (line 3461) covers the by-value owned-
/// UTF-8 emit frontier — a consumer that owns a [`StorePath`] and hands
/// it into an `impl Into<String>` sink at zero-copy off the moved
/// [`StorePath::full`] backing. But a large family of downstream sites
/// hold a [`&StorePath`], not an owned [`StorePath`], and must not consume
/// the source:
///
/// - Iteration over a `[StorePath]` / `Vec<StorePath>` / `&[StorePath]`
///   collection (a closure listing, a derivation manifest, a
///   [`crate::store_path::parse_closure_paths`] result) where each
///   element must project into an owned [`String`] for downstream
///   [`serde_json::Value`] serialisation, a [`HashMap<String, _>`] key,
///   or a `Vec<String>` accumulation — the by-value peer's move
///   consumes the element, which iteration cannot afford. Pre-peer the
///   site had to write `sp.as_str().to_owned()` or `sp.to_string()`
///   restating the two-step conversion at every consumer; post-peer
///   `String::from(sp)` or `sp.into()` reads through this impl.
/// - `&StorePath`-typed method receivers or function arguments (a
///   pipeline-step primitive that inspects a store path without taking
///   ownership, an attestation record builder that borrows its inputs)
///   projecting into an `impl Into<String>` sink at their downstream
///   boundary — the by-value peer requires a clone at the boundary if
///   the caller still needs the value; the by-reference peer avoids the
///   `sp.clone().into()` restatement.
/// - Trait objects and generic bounds that admit `&T` conversions
///   through the standard `impl<T> From<&T> for T::Owned`
...
///   contexts.
///
/// The [`as_str().to_owned()`] chain routes through the same
/// [`AsRef<str>`] read peer every borrowed-view consumer already reads
/// through, so a future canonical-form refinement to the backing string
/// (a tighter trim, a `SmartString` handle, a canonicalising projection
/// at the read surface) lands at one site (the [`AsRef<str>`] impl) and
/// every by-reference owned-emit peer in the family — [`String`], and
/// every future by-reference peer chained through it — inherits it
/// automatically. THEORY §VI.1 one-oracle discipline.
///
/// # Identity invariants pinned
///
/// - `String::from(&sp) == String::from(sp.clone())` (matches the by-
///   value peer's projection byte-for-byte) — pinned by
///   [`tests::test_from_store_path_ref_string_matches_by_value_peer`].
/// - Trim-discipline preservation: a `\n`-terminated raw input emits a
///   [`String`] whose bytes equal the trimmed canonical form — pinned
///   by [`tests::test_from_store_path_ref_string_carries_trim_discipline`].
/// - Trait-generic composition: an `impl Into<String>` consumer holding
///   a [`&StorePath`] projects the same canonical bytes — pinned by
///   [`tests::test_from_store_path_ref_string_carries_through_generic_consumer`].
/// - Source-preservation: the source [`StorePath`] remains usable after
///   the emit (the by-reference peer does not consume) — pinned by
///   [`tests::test_from_store_path_ref_string_preserves_source`].
impl From<&StorePath> for String {
    fn from(sp: &StorePath) -> String {
        <StorePath as AsRef<str>>::as_ref(sp).to_owned()
    }
}

/// By-reference owned-byte-vector emit peer for [`StorePath`] via
/// [`Vec<u8>`] — the second step of the by-reference owned-emit frontier
/// opened by [`From<&StorePath> for String`] (line 5265), chaining through
/// the same by-reference [`AsRef<str>`] one-oracle discipline every
/// by-reference emit peer in the family routes through, mirroring how the
/// by-value [`From<StorePath> for Vec<u8>`] (line 3841) chained through
/// the [`StorePath::into_string`] one-oracle every by-value emit peer
/// routes through.
///
/// Delegates through `<StorePath as AsRef<[u8]>>::as_ref(sp).to_vec()`:
/// the borrowed-view [`AsRef<[u8]>`] read peer (line 2001) returns the
/// canonical `/nix/store/<hash>-<name>` byte slice off the same
/// [`StorePath::full`] backing every borrowed-view consumer already reads
/// through; [`<[u8]>::to_vec`] copies the trimmed bytes into a fresh
/// [`Vec<u8>`] without touching the source [`StorePath`], leaving it
/// available for further reads. Zero re-encoding — the same validated
/// bytes the [`StorePath::parse`] oracle proved
/// `/nix/store/<32-char-base32-hash>-<name>`-shaped reach the owned-byte-
/// vector frontier without materialising the intermediate [`String`] the
/// by-value peer would move out (which would require consuming the value).
///
/// # Frontier-blanket calculus — why byte-vec, not filesystem-path
///
/// The [`From<&StorePath> for String`] closure notes named the family
/// `{PathBuf, OsString, Vec<u8>, Box<str>, Cow<'static, str>, Arc<str>,
/// Rc<str>, Box<[u8]>, Arc<[u8]>, Rc<[u8]>, Cow<'static, [u8]>}` as the
/// by-reference peers it opens the frontier for. Two of those — [`PathBuf`]
/// and [`std::ffi::OsString`] — are **already provided by stdlib blanket
/// impls** keyed on this crate's [`AsRef<std::ffi::OsStr> for StorePath`]
/// (line 1920): `impl<T: AsRef<OsStr> + ?Sized> From<&T> for
/// std::path::PathBuf` (stdlib) and `impl<T: AsRef<OsStr> + ?Sized>
/// From<&T> for std::ffi::OsString` (stdlib) both fire for `T = StorePath`
/// with zero declared code. A custom `impl From<&StorePath> for PathBuf`
/// or `impl From<&StorePath> for OsString` is not merely redundant — it is
/// a coherence-rule conflict (E0119) and refuses to compile.
///
/// [`Vec<u8>`] is the next by-reference peer that isn't already blanketed
/// through the OS-string [`AsRef`] chain: stdlib has no
/// `impl<T> From<&T> for Vec<u8>` in scope for a type that isn't
/// `&[u8]`-shaped, and this crate's [`AsRef<[u8]>`] returns a
/// borrowed-view slice off a different backing than [`AsRef<OsStr>`] does
/// (the UTF-8 byte view vs the OS-string byte view — identical on Unix
/// for an ASCII payload, distinct on Windows in general). So the
/// by-reference [`Vec<u8>`] peer opens the byte-vec frontier the stdlib
/// blanket doesn't reach and does so at the same one-oracle discipline
/// the by-reference [`String`] frontier-opener established.
///
/// # Why the trait peer earns its keep
///
/// The by-value [`From<StorePath> for Vec<u8>`] (line 3841) covers the
/// by-value owned-byte-vector emit frontier. But a large family of
/// downstream sites hold a [`&StorePath`], not an owned [`StorePath`],
/// and must not consume the source:
///
/// - Iteration over a `[StorePath]` / `Vec<StorePath>` / `&[StorePath]`
...
///   element must project into an owned [`Vec<u8>`] for downstream
///   [`serde_bytes`]-tagged field serialisation, a
///   [`HashMap<Vec<u8>, _>`] key, or a `Vec<Vec<u8>>` accumulation for
///   digest-over-bytes hashing (BLAKE3 / SHA-256 / Merkle-tree leaf
///   assembly at the attestation frontier) — the by-value peer's move
///   consumes the element, which iteration cannot afford. Pre-peer the
///   site had to write `sp.as_str().as_bytes().to_vec()` or
///   `<StorePath as AsRef<[u8]>>::as_ref(&sp).to_vec()` at every consumer;
///   post-peer `Vec::<u8>::from(sp)` or `sp.into()` reads through this
///   impl.
/// - `&StorePath`-typed method receivers or function arguments (a
///   pipeline-step primitive inspecting a store path without ownership,
///   an attestation record builder borrowing its inputs, a digest-input
///   accessor accepting a borrow) projecting into an
///   `impl Into<Vec<u8>>` sink at their downstream boundary — the by-value
///   peer requires a `sp.clone().into()` at the boundary if the caller
///   still needs the value; the by-reference peer avoids the clone.
/// - Trait objects and generic bounds that admit `&T` conversions
///   through the standard `impl<T> From<&T> for T::Owned`
///   [`ToOwned`](std::borrow::ToOwned) family — the by-reference peer
///   completes the [`From<&StorePath>`] symmetry the by-value
///   [`From<StorePath>`] emit peer requires to compose in trait-generic
///   contexts (a Merkle-tree leaf builder receiving `&StorePath` and
///   handing it into an `impl Into<Vec<u8>>` digest-input slot, an
///   HMAC-input capture accessor typed on `impl Into<Vec<u8>>`).
///
/// The chain-through-`AsRef<[u8]>` design is the one-oracle discipline
/// THEORY §VI.1 demands: the emitted canonical bytes travel through the
/// same borrowed-view byte-slice read peer every borrowed-view byte-slice
/// consumer already reads through, so a future canonical-form refinement
/// to the backing string (a tighter trim, a `SmartString` handle, a
/// canonicalising projection at the read surface) lands at one site (the
/// [`AsRef<str>`] impl, which the [`AsRef<[u8]>`] impl reads through) and
/// every by-reference owned-emit peer in the family inherits it
/// automatically. A per-impl direct
/// `String::from(sp).into_bytes()` here would work today but would ossify
/// the "String backing" assumption at the by-reference emit surface AND
/// materialise a fresh [`String`] the [`Vec<u8>::from(sp.into_bytes())`]
/// consumer never asked for; the `.as_ref().to_vec()` chain reads through
/// the borrowed-view byte-slice read peer directly, at one allocation.
///
/// # Identity invariants pinned
///
/// - `Vec::<u8>::from(&sp) == Vec::<u8>::from(sp.clone())` (matches the
///   by-value peer's projection byte-for-byte) — pinned by
///   [`tests::test_from_store_path_ref_bytes_matches_by_value_peer`].
/// - Trim-discipline preservation: a `\n`-terminated raw input emits a
///   [`Vec<u8>`] whose bytes equal the trimmed canonical form — pinned
///   by [`tests::test_from_store_path_ref_bytes_carries_trim_discipline`].
/// - Trait-generic composition: an `impl Into<Vec<u8>>` consumer holding
///   a [`&StorePath`] projects the same canonical bytes — pinned by
///   [`tests::test_from_store_path_ref_bytes_carries_through_generic_consumer`].
/// - Source-preservation: the source [`StorePath`] remains usable after
///   the emit (the by-reference peer does not consume) — pinned by
///   [`tests::test_from_store_path_ref_bytes_preserves_source`].
impl From<&StorePath> for Vec<u8> {
    fn from(sp: &StorePath) -> Vec<u8> {
        <StorePath as AsRef<[u8]>>::as_ref(sp).to_vec()
    }
}
...
/// By-reference borrowed-or-owned-UTF-8 emit peer for [`StorePath`] via
/// [`std::borrow::Cow<'static, str>`] — the fourth step of the by-reference
/// owned-emit frontier opened by [`From<&StorePath> for String`] (line 5265)
/// and extended by [`From<&StorePath> for Vec<u8>`] (line 5384) and
/// [`From<&StorePath> for Box<str>`] (line 5497), chaining through the same
/// by-reference [`String::from`] one-oracle discipline every by-reference
/// string-side emit peer in the family routes through, mirroring how the
/// by-value [`From<StorePath> for Cow<'static, str>`] (line 4106) chained
/// through the by-value [`String::from`] one-oracle every by-value
...
/// source; the moved [`String`] is then wrapped in the [`Cow::Owned`] arm at
/// the `'static` lifetime (the arm requires no borrow, so it satisfies any
/// output lifetime the trait signature demands, including `'static`). Zero
/// re-encoding: the owned arm carries the same [`String`] buffer that
/// [`From<&StorePath> for String`] emits, so a downstream site that consumes
/// the [`Cow`] by [`std::borrow::Cow::into_owned`] recovers the exact
/// backing buffer the direct-by-reference-[`String`] peer would hand over —
/// one allocation per emit, no per-consumer clone of the validated bytes,
/// source [`StorePath`] left available for further reads.
///
/// # Frontier-blanket calculus — why Cow<'static, str> is not blanketed
///
/// The [`From<&StorePath> for Vec<u8>`] impl doc records that stdlib
/// blanket `impl<T: AsRef<OsStr> + ?Sized> From<&T> for {PathBuf, OsString}`
/// fires for `T = StorePath` through this crate's [`AsRef<OsStr>`] peer
/// (line 1920) and forbids a custom by-reference peer at those two shapes
/// (E0119). [`Cow<'static, str>`] carries no such stdlib blanket: stdlib
/// provides `impl<'a> From<&'a str> for Cow<'a, str>` (which the by-value
/// peer's downstream sites reach through `.as_str().into()` in pre-peer
/// call sites at the borrowed-arm lifetime), but the `&'a str` receiver
/// is a concrete type at a specific lifetime, not a generic bound, so it
/// does not fire for `T = &StorePath` (which is not `&str`-shaped and
/// cannot be deref-coerced through [`From`]). The by-reference
/// [`Cow<'static, str>`] peer therefore opens a frontier the stdlib
/// blanket doesn't reach and does so at the same one-oracle discipline
/// the by-reference [`String`], [`Vec<u8>`], and [`Box<str>`]
/// frontier openers established.
///
/// # Why the trait peer earns its keep
///
/// The by-value [`From<StorePath> for Cow<'static, str>`] (line 4106) covers
/// the by-value borrowed-or-owned UTF-8 emit frontier. But a large family of
/// downstream sites hold a [`&StorePath`], not an owned [`StorePath`], and
/// must not consume the source:
///
/// - Iteration over a `[StorePath]` / `Vec<StorePath>` / `&[StorePath]`
...
///   [`crate::store_path::parse_closure_paths`], a derivation-input
///   manifest, an attestation-adjacent per-input path list) where each
///   element must project into a [`Cow<'static, str>`] for downstream
///   `serde`-derived record insertion, a `Vec<Cow<'static, str>>`
///   intern-pool accumulation, or a `HashMap<Cow<'static, str>, _>` key
///   slot — the by-value peer's move consumes the element, which iteration
///   cannot afford. Pre-peer the site had to write
///   `Cow::Owned(sp.as_str().to_owned())` or
///   `Cow::Owned(String::from(&sp))` at every consumer; post-peer
///   `Cow::<str>::from(sp)` or `sp.into()` reads through this impl.
/// - `&StorePath`-typed method receivers or function arguments (a
///   pipeline-step primitive inspecting a store path without ownership, an
///   attestation record builder borrowing its inputs, a canonical-label
///   extractor accepting a borrow) projecting into an
///   `impl Into<Cow<'static, str>>` sink at their downstream boundary — the
///   by-value peer requires a `sp.clone().into()` at the boundary if the
///   caller still needs the value; the by-reference peer avoids the clone.
/// - Trait objects and generic bounds that admit `&T` conversions through
///   the standard `impl<T> From<&T> for T::Owned`
...
///   completes the [`From<&StorePath>`] symmetry the by-value
///   [`From<StorePath>`] emit peer requires to compose in trait-generic
///   contexts (an intern-table populator receiving `&StorePath` and handing
///   it into an `impl Into<Cow<'static, str>>` key slot, a
///   `serde`-derive-adjacent field emitter typed on
///   `impl Into<Cow<'static, str>>`, a `serde_json::Value::String`-shaped
///   tagged-union builder that accepts `impl Into<Cow<'static, str>>` from
///   a borrowed [`StorePath`] iteration).
///
/// The chain-through-`From<&StorePath> for String` design is the one-oracle
/// discipline THEORY §VI.1 demands: the emitted canonical bytes travel
/// through the same by-reference [`String::from`] projection every
...
/// [`String::from`] peer reads through) and every by-reference owned-emit
/// peer in the family inherits it automatically. A per-impl direct call to
/// `Cow::Owned(sp.as_str().to_owned())` here would work today but would
/// ossify the "AsRef<str> to_owned" two-step at the by-reference emit
/// surface; the `Cow::Owned(String::from(sp))` chain reads through the
/// by-reference [`String`] frontier-opener so the family stays composable
/// by construction.
///
/// # Identity invariants pinned
///
/// - `Cow::<str>::from(&sp) == Cow::<str>::from(sp.clone())` (matches the
///   by-value peer's projection byte-for-byte) — pinned by
///   [`tests::test_from_store_path_ref_cow_str_matches_by_value_peer`].
/// - Trim-discipline preservation: a `\n`-terminated raw input emits a
///   [`Cow<'static, str>`] whose bytes equal the trimmed canonical form —
///   pinned by
///   [`tests::test_from_store_path_ref_cow_str_carries_trim_discipline`].
/// - Trait-generic composition: an `impl Into<Cow<'static, str>>` consumer
///   holding a [`&StorePath`] projects the same canonical bytes — pinned by
///   [`tests::test_from_store_path_ref_cow_str_carries_through_generic_consumer`].
/// - Owned-arm discipline: the emitted [`Cow`] is
///   [`std::borrow::Cow::Owned`] (a by-reference emit that allocated a
///   fresh [`String`] cannot land in the borrowed arm without fabricating
///   a `'static` reference from a runtime buffer) — pinned by
///   [`tests::test_from_store_path_ref_cow_str_matches_by_value_peer`].
/// - Source-preservation: the source [`StorePath`] remains usable after
///   the emit (the by-reference peer does not consume) — pinned by
///   [`tests::test_from_store_path_ref_cow_str_preserves_source`].
impl From<&StorePath> for std::borrow::Cow<'static, str> {
    fn from(sp: &StorePath) -> std::borrow::Cow<'static, str> {
        std::borrow::Cow::Owned(String::from(sp))
    }
...
/// By-reference thread-local shared-owned-UTF-8 emit peer for [`StorePath`]
/// via [`std::rc::Rc<str>`] — the sixth step of the by-reference owned-emit
/// frontier opened by [`From<&StorePath> for String`] (line 5265) and extended
/// by [`From<&StorePath> for Vec<u8>`] (line 5384),
/// [`From<&StorePath> for Box<str>`] (line 5497),
/// [`From<&StorePath> for Cow<'static, str>`] (line 5620), and
/// [`From<&StorePath> for Arc<str>`] (line 5746), chaining through the same
/// by-reference [`String::from`] one-oracle discipline every by-reference
/// string-side emit peer in the family routes through, mirroring how the
/// by-value [`From<StorePath> for Rc<str>`] (line 4427) chained through the
/// by-value [`String::from`] one-oracle every by-value string-side emit peer
/// routes through.
///
/// Delegates through `std::rc::Rc::<str>::from(String::from(sp))`:
/// [`From<&StorePath> for String`] copies the borrowed [`StorePath::as_str`]
/// view into a fresh [`String`] at one allocation without consuming the
/// source; the stdlib [`std::rc::Rc`]`::<str>::from` impl then constructs a
/// single fresh allocation carrying the non-atomic refcount header alongside
/// the moved UTF-8 payload bytes. The moved [`String`] backing is dropped
/// after the payload copy, but the copy is unavoidable: [`std::rc::Rc<str>`]
/// and [`String`] have different memory layouts (an [`Rc<str>`] allocation
/// prefixes the payload with a [`std::cell::Cell<usize>`] strong-count header
/// and a [`std::cell::Cell<usize>`] weak-count header, while a [`String`]
/// carries a `(ptr, len, cap)` triple pointing at a header-less heap-
/// allocated buffer). No re-encoding: the emitted bytes are the same bytes
/// [`From<&StorePath> for String`] copies out, projected through
/// [`std::rc::Rc<str>`]'s allocation shape one time and shared cheaply across
/// every subsequent [`std::rc::Rc::clone`] recipient at zero buffer-clone
/// cost per downstream consumer — source [`StorePath`] left available for
/// further reads.
///
/// # Frontier-blanket calculus — why Rc<str> is not blanketed
///
...
/// [`Rc<str>`] carries no such stdlib blanket: stdlib provides
/// `impl<'a> From<&'a str> for Rc<str>` (line 4427's by-value peer routes
/// downstream sites through it via `.as_str().into()` in pre-peer call
/// sites), but the `&'a str` receiver is a concrete type, not a generic
/// bound, so it does not fire for `T = &StorePath` (which is not
/// `&str`-shaped and cannot be deref-coerced through [`From`]). The
/// by-reference [`Rc<str>`] peer therefore opens a frontier the stdlib
/// blanket doesn't reach and does so at the same one-oracle discipline the
/// by-reference [`String`], [`Vec<u8>`], [`Box<str>`], [`Cow<'static, str>`],
/// and [`Arc<str>`] frontier openers established.
///
/// # Why the trait peer earns its keep — Rc distinct from Arc
///
/// The by-value [`From<StorePath> for Rc<str>`] (line 4427) covers the
/// by-value thread-local shared-owned UTF-8 emit frontier. But a large family
/// of downstream sites hold a [`&StorePath`], not an owned [`StorePath`], and
/// must not consume the source:
///
/// - Iteration over a `[StorePath]` / `Vec<StorePath>` / `&[StorePath]`
///   collection (a closure listing from
///   [`crate::store_path::parse_closure_paths`], a derivation-input manifest,
///   an attestation-adjacent per-input path list) where each element must
///   project into a thread-local shared-owned [`Rc<str>`] for downstream
///   [`std::collections::HashMap<Rc<str>, _>`] insertion, a `Vec<Rc<str>>`
///   intern-pool accumulation, or a single-thread task-graph node builder
///   that fans a canonical label out to N sibling nodes on the same thread
///   through [`std::rc::Rc::clone`] — the by-value peer's move consumes the
///   element, which iteration cannot afford. Pre-peer the site had to write
///   `std::rc::Rc::<str>::from(sp.as_str())` or
///   `std::rc::Rc::<str>::from(String::from(&sp))` at every consumer;
///   post-peer `std::rc::Rc::<str>::from(sp)` or `sp.into()` reads through
///   this impl.
/// - `&StorePath`-typed method receivers or function arguments (a
///   pipeline-step primitive inspecting a store path without ownership, an
///   attestation record builder borrowing its inputs, a canonical-label
///   extractor accepting a borrow) projecting into an `impl Into<Rc<str>>`
///   sink at their downstream boundary — the by-value peer requires a
///   `sp.clone().into()` at the boundary if the caller still needs the value;
///   the by-reference peer avoids the clone.
/// - Trait objects and generic bounds that admit `&T` conversions through
///   the standard `impl<T> From<&T> for T::Owned`
///   [`ToOwned`](std::borrow::ToOwned) family — the by-reference peer
///   completes the [`From<&StorePath>`] symmetry the by-value
///   [`From<StorePath>`] emit peer requires to compose in trait-generic
///   contexts (an intern-table populator receiving `&StorePath` and handing
///   it into an `impl Into<Rc<str>>` key slot, a single-thread task-graph
///   node attribute setter typed on `impl Into<Rc<str>>` from a borrowed
///   iteration).
///
/// [`Rc<str>`] is disjoint from [`Arc<str>`] (line 5746) at the type level
/// even though the two carry the same bytes: [`Rc<str>`] uses a
/// [`std::cell::Cell<usize>`] non-atomic refcount header (cheaper on single-
/// thread hot paths — no `LOCK` prefix on the refcount increment/decrement)
/// while [`Arc<str>`] uses an [`std::sync::atomic::AtomicUsize`] atomic
/// refcount header ([`Send`] + [`Sync`] safe cross-thread sharing). A
/// downstream site that pins its handle type on [`Rc<str>`] specifically
/// (a single-thread task-graph node, a `!Send` `RefCell`-adjacent builder,
/// a per-thread canonical-label intern pool) cannot substitute [`Arc<str>`]
/// — the [`Send`] + [`Sync`] bounds [`Arc<str>`] carries are load-bearing
/// nowhere in the caller's contract and the atomic-refcount overhead is
/// pure cost. This peer covers exactly that shape at the by-reference
/// frontier.
///
/// The chain-through-`From<&StorePath> for String` design is the one-oracle
/// discipline THEORY §VI.1 demands: the emitted canonical bytes travel
...
/// the same borrowed-view [`AsRef<str>`] peer the by-value emit peers'
/// [`StorePath::into_string`] backing surfaces), so a future canonical-form
/// refinement to the backing string (a tighter trim, a `SmartString` handle,
/// a canonicalising projection at the read surface) lands at one site (the
/// [`AsRef<str>`] impl at line 1684, which the by-reference [`String::from`]
/// peer reads through) and every by-reference owned-emit peer in the family
/// inherits it automatically. A per-impl direct call to
/// `std::rc::Rc::<str>::from(sp.as_str())` here would work today but would
/// ossify the "AsRef<str> to_rc" two-step at the by-reference emit surface;
/// the `std::rc::Rc::<str>::from(String::from(sp))` chain reads through the
/// by-reference [`String`] frontier-opener so the family stays composable by
/// construction.
///
/// # Identity invariants pinned
///
/// - `<Rc<str>>::from(&sp) == <Rc<str>>::from(sp.clone())` (matches the
///   by-value peer's projection byte-for-byte) — pinned by
///   [`tests::test_from_store_path_ref_rc_str_matches_by_value_peer`].
/// - Trim-discipline preservation: a `\n`-terminated raw input emits an
///   [`Rc<str>`] whose bytes equal the trimmed canonical form — pinned by
///   [`tests::test_from_store_path_ref_rc_str_carries_trim_discipline`].
/// - Trait-generic composition: an `impl Into<Rc<str>>` consumer holding a
///   [`&StorePath`] projects the same canonical bytes — pinned by
///   [`tests::test_from_store_path_ref_rc_str_carries_through_generic_consumer`].
/// - Source-preservation: the source [`StorePath`] remains usable after the
///   emit (the by-reference peer does not consume) — pinned by
///   [`tests::test_from_store_path_ref_rc_str_preserves_source`].
impl From<&StorePath> for std::rc::Rc<str> {
    fn from(sp: &StorePath) -> std::rc::Rc<str> {
        std::rc::Rc::<str>::from(String::from(sp))
    }
...
/// By-reference cross-thread shared-owned-byte-slice emit peer for
/// [`StorePath`] via [`std::sync::Arc<[u8]>`] — the eighth step of the
/// by-reference owned-emit frontier opened by
/// [`From<&StorePath> for String`] (line 5265) and extended by
/// [`From<&StorePath> for Vec<u8>`] (line 5384),
/// [`From<&StorePath> for Box<str>`] (line 5497),
/// [`From<&StorePath> for Cow<'static, str>`] (line 5620),
/// [`From<&StorePath> for Arc<str>`] (line 5746),
/// [`From<&StorePath> for Rc<str>`] (line 5883), and
/// [`From<&StorePath> for Box<[u8]>`] (line 6025), and the second step of
/// the by-reference byte-side sub-frontier chaining through the
/// by-reference [`Vec<u8>`] one-oracle. Mirrors how the by-value
/// [`From<StorePath> for Arc<[u8]>`] (line 4789) chained through the
/// by-value [`From<StorePath> for Vec<u8>`] (line 3841) — the byte-side
/// sub-family's analog of how [`From<&StorePath> for Arc<str>`] chains
/// through [`From<&StorePath> for String`] on the UTF-8-string sub-family.
///
/// Delegates through `std::sync::Arc::<[u8]>::from(Vec::<u8>::from(sp))`:
/// [`From<&StorePath> for Vec<u8>`] copies the borrowed [`AsRef<[u8]>`]
/// byte-view into a fresh [`Vec<u8>`] at one allocation without consuming
/// the source; the stdlib
/// [`std::sync::Arc`]`::<[u8]>::from(Vec<u8>)` impl then constructs a
/// single fresh allocation carrying the atomic refcount header alongside
/// the moved byte payload. The moved [`Vec<u8>`] backing is dropped after
/// the payload copy, but the copy is unavoidable: [`std::sync::Arc<[u8]>`]
/// and [`Vec<u8>`] have different memory layouts (an [`Arc<[u8]>`]
/// allocation prefixes the payload with atomic strong/weak count headers
/// so clones may cross threads, while a [`Vec<u8>`] carries a
/// `(ptr, len, cap)` triple pointing at a header-less heap-allocated
/// buffer). No re-encoding — the emitted bytes are the same bytes
/// [`From<&StorePath> for Vec<u8>`] copies out, projected through
/// [`std::sync::Arc<[u8]>`]'s allocation shape one time and shared cheaply
/// across every subsequent [`std::sync::Arc::clone`] recipient at zero
/// buffer-clone cost per downstream consumer, and the source [`StorePath`]
/// is left available for further reads.
///
/// # Frontier-blanket calculus — why Arc<[u8]> is not blanketed
///
/// The [`From<&StorePath> for Vec<u8>`] impl doc records that stdlib
/// blanket `impl<T: AsRef<OsStr> + ?Sized> From<&T> for {PathBuf,
/// OsString}` fires for `T = StorePath` through this crate's
/// [`AsRef<OsStr>`] peer (line 1920) and forbids a custom by-reference
/// peer at those two shapes (E0119). [`std::sync::Arc<[u8]>`] carries no
/// such stdlib blanket: stdlib provides
/// `impl From<&[u8]> for Arc<[u8]>` (via
/// `impl<T: Clone> From<&[T]> for Arc<[T]>`, which line 4789's by-value
/// peer composes on the byte view produced by an upstream [`Vec::from`]),
/// but the `&[u8]` receiver is a concrete slice type, not a generic
/// `T: AsRef<[u8]>` bound, so it does not fire for `T = &StorePath`
/// (which is not `&[u8]`-shaped and cannot be deref-coerced through
/// [`From`]). The by-reference [`Arc<[u8]>`] peer therefore opens a
/// frontier the stdlib blanket doesn't reach and does so at the same
/// one-oracle discipline the by-reference [`String`], [`Vec<u8>`],
/// [`Box<str>`], [`Cow<'static, str>`], [`Arc<str>`], [`Rc<str>`], and
/// [`Box<[u8]>`] frontier openers established.
///
/// # Why the trait peer earns its keep — Arc<[u8]> distinct from Box<[u8]>
///
/// The by-value [`From<StorePath> for Arc<[u8]>`] (line 4789) covers the
/// by-value cross-thread shared-owned byte-slice emit frontier. But a
/// large family of downstream sites hold a [`&StorePath`], not an owned
/// [`StorePath`], and must not consume the source:
///
/// - Iteration over a `[StorePath]` / `Vec<StorePath>` / `&[StorePath]`
///   collection (a closure listing from
///   [`crate::store_path::parse_closure_paths`], a derivation-input
///   manifest, an attestation-adjacent per-input path list) where each
///   element must project into a cross-thread shared-owned
///   [`std::sync::Arc<[u8]>`] for downstream fan-out to `Send + Sync`
///   consumers — a `Vec<Arc<[u8]>>` payload list handed into a
///   [`tokio::spawn`]-per-element attestation-signer pool, a
///   `Vec<Arc<[u8]>>` accumulation shared across a
///   `rayon::par_iter().map(...)` verifier fan-out, a
///   `serde_bytes`-tagged serialize-side field of shape `Vec<Arc<[u8]>>`
///   whose entries downstream reach `Send + Sync` recipients through
///   `Arc::clone` — the by-value peer's move consumes the element, which
///   iteration cannot afford. Pre-peer the site had to write
///   `std::sync::Arc::<[u8]>::from(sp.as_str().as_bytes().to_vec())` or
///   `std::sync::Arc::<[u8]>::from(<StorePath as AsRef<[u8]>>::as_ref(&sp).to_vec())`
///   at every consumer; post-peer `Arc::<[u8]>::from(sp)` or
///   `sp.into()` reads through this impl.
/// - `&StorePath`-typed method receivers or function arguments (a
///   pipeline-step primitive inspecting a store path without ownership, an
///   attestation record builder borrowing its inputs, a canonical-byte
///   digest-input accessor accepting a borrow) projecting into an
///   `impl Into<Arc<[u8]>>` sink at their downstream boundary — the
///   by-value peer requires a `sp.clone().into()` at the boundary if the
///   caller still needs the value; the by-reference peer avoids the clone.
/// - Trait objects and generic bounds that admit `&T` conversions through
///   the standard `impl<T> From<&T> for T::Owned`
...
///   [`From<StorePath>`] emit peer requires to compose in trait-generic
///   contexts (an intern-table populator receiving `&StorePath` and
///   handing it into an `impl Into<Arc<[u8]>>` key slot, a
///   [`std::sync::Arc<[u8]>`]-keyed cross-thread cache populator typed on
///   `impl Into<Arc<[u8]>>` from a borrowed iteration).
///
/// [`std::sync::Arc<[u8]>`] is disjoint from [`Box<[u8]>`] (line 6025) at
/// the type level even though the two carry the same bytes: [`Arc<[u8]>`]
/// prefixes its payload with atomic strong/weak count headers so clones
/// share the same allocation across threads, while [`Box<[u8]>`] is a
/// payload-sized `(ptr, len)` fat pointer that owns its buffer with no
/// refcount slot at all. A downstream site that pins its handle type on
/// [`Arc<[u8]>`] specifically (a byte-payload fan-out that must clone
/// cheaply across `Send + Sync` recipients, a
/// `#[serde(into = "Arc<[u8]>")]` container attribute that opts into the
/// cross-thread shared-owned serialize grammar) cannot substitute
/// [`Box<[u8]>`] — the payload-sized shape cannot be cloned without a full
/// buffer copy, which is exactly the cost the cross-thread shared-owned
/// contract is asked to avoid. This peer covers exactly that shape at the
/// by-reference frontier.
///
/// The chain-through-`From<&StorePath> for Vec<u8>` design is the
/// one-oracle discipline THEORY §VI.1 demands: the emitted canonical bytes
/// travel through the same by-reference [`Vec::<u8>::from`] projection the
/// by-reference byte-side sub-family routes through (and, transitively,
/// the same borrowed-view [`AsRef<[u8]>`] peer the by-reference
/// [`Vec<u8>`] peer reads through), so a future canonical-form refinement
/// to the backing bytes (a tighter trim, a canonicalising projection at
/// the read surface) lands at one site (the [`AsRef<[u8]>`] impl at
/// line 2001, which the by-reference [`Vec::<u8>::from`] peer reads
/// through) and every by-reference byte-side emit peer in the family
/// inherits it automatically. A per-impl direct call to
/// `std::sync::Arc::<[u8]>::from(sp.as_str().as_bytes())` here would work
/// today but would ossify the "AsRef<[u8]> to_arc" two-step at the
/// by-reference emit surface AND materialise a fresh [`&[u8]`] slice the
/// intermediate [`Vec<u8>`] peer already surfaces; the
/// `std::sync::Arc::<[u8]>::from(Vec::<u8>::from(sp))` chain reads through
/// the by-reference [`Vec<u8>`] frontier-opener so the family stays
/// composable by construction.
///
/// # Identity invariants pinned
///
/// - `<Arc<[u8]>>::from(&sp) == <Arc<[u8]>>::from(sp.clone())` (matches
///   the by-value peer's projection byte-for-byte) — pinned by
///   [`tests::test_from_store_path_ref_arc_bytes_matches_by_value_peer`].
/// - Trim-discipline preservation: a `\n`-terminated raw input emits an
///   [`Arc<[u8]>`] whose bytes equal the trimmed canonical form — pinned
///   by [`tests::test_from_store_path_ref_arc_bytes_carries_trim_discipline`].
/// - Trait-generic composition: an `impl Into<Arc<[u8]>>` consumer holding
///   a [`&StorePath`] projects the same canonical bytes — pinned by
///   [`tests::test_from_store_path_ref_arc_bytes_carries_through_generic_consumer`].
/// - Source-preservation: the source [`StorePath`] remains usable after
///   the emit (the by-reference peer does not consume) — pinned by
///   [`tests::test_from_store_path_ref_arc_bytes_preserves_source`].
impl From<&StorePath> for std::sync::Arc<[u8]> {
    fn from(sp: &StorePath) -> std::sync::Arc<[u8]> {
        std::sync::Arc::<[u8]>::from(Vec::<u8>::from(sp))
    }
...
/// By-reference thread-local shared-owned-byte-slice emit peer for
/// [`StorePath`] via [`std::rc::Rc<[u8]>`] — the ninth step of the
/// by-reference owned-emit frontier opened by
/// [`From<&StorePath> for String`] (line 5265) and extended by
/// [`From<&StorePath> for Vec<u8>`] (line 5384),
/// [`From<&StorePath> for Box<str>`] (line 5497),
/// [`From<&StorePath> for Cow<'static, str>`] (line 5620),
/// [`From<&StorePath> for Arc<str>`] (line 5746),
/// [`From<&StorePath> for Rc<str>`] (line 5883),
/// [`From<&StorePath> for Box<[u8]>`] (line 6025), and
/// [`From<&StorePath> for Arc<[u8]>`] (line 6177), and the third step of
/// the by-reference byte-side sub-frontier chaining through the
/// by-reference [`Vec<u8>`] one-oracle. Mirrors how the by-value
/// [`From<StorePath> for Rc<[u8]>`] (line 4991) chained through the by-value
/// [`From<StorePath> for Vec<u8>`] (line 3841) — the byte-side sub-family's
/// analog of how [`From<&StorePath> for Rc<str>`] chains through
/// [`From<&StorePath> for String`] on the UTF-8-string sub-family.
///
/// Delegates through `std::rc::Rc::<[u8]>::from(Vec::<u8>::from(sp))`:
/// [`From<&StorePath> for Vec<u8>`] copies the borrowed [`AsRef<[u8]>`]
/// byte-view into a fresh [`Vec<u8>`] at one allocation without consuming
/// the source; the stdlib [`std::rc::Rc`]`::<[u8]>::from(Vec<u8>)` impl
/// then constructs a single fresh allocation carrying the non-atomic
/// refcount header alongside the moved byte payload. The moved [`Vec<u8>`]
/// backing is dropped after the payload copy, but the copy is unavoidable:
/// [`std::rc::Rc<[u8]>`] and [`Vec<u8>`] have different memory layouts
/// (an [`Rc<[u8]>`] allocation prefixes the payload with non-atomic
/// strong/weak count headers so clones share the allocation on the owning
/// thread, while a [`Vec<u8>`] carries a `(ptr, len, cap)` triple pointing
/// at a header-less heap-allocated buffer). No re-encoding — the emitted
/// bytes are the same bytes [`From<&StorePath> for Vec<u8>`] copies out,
/// projected through [`std::rc::Rc<[u8]>`]'s allocation shape one time and
/// shared cheaply across every subsequent [`std::rc::Rc::clone`] recipient
/// on the same thread at zero buffer-clone cost per downstream consumer,
/// and the source [`StorePath`] is left available for further reads.
///
/// # Frontier-blanket calculus — why Rc<[u8]> is not blanketed
///
/// The [`From<&StorePath> for Vec<u8>`] impl doc records that stdlib
/// blanket `impl<T: AsRef<OsStr> + ?Sized> From<&T> for {PathBuf,
/// OsString}` fires for `T = StorePath` through this crate's
/// [`AsRef<OsStr>`] peer (line 1920) and forbids a custom by-reference
/// peer at those two shapes (E0119). [`std::rc::Rc<[u8]>`] carries no
/// such stdlib blanket: stdlib provides
/// `impl From<&[u8]> for Rc<[u8]>` (via
/// `impl<T: Clone> From<&[T]> for Rc<[T]>`, which line 4991's by-value
/// peer composes on the byte view produced by an upstream [`Vec::from`]),
/// but the `&[u8]` receiver is a concrete slice type, not a generic
/// `T: AsRef<[u8]>` bound, so it does not fire for `T = &StorePath`
/// (which is not `&[u8]`-shaped and cannot be deref-coerced through
/// [`From`]). The by-reference [`Rc<[u8]>`] peer therefore opens a
/// frontier the stdlib blanket doesn't reach and does so at the same
/// one-oracle discipline the by-reference [`String`], [`Vec<u8>`],
/// [`Box<str>`], [`Cow<'static, str>`], [`Arc<str>`], [`Rc<str>`],
/// [`Box<[u8]>`], and [`Arc<[u8]>`] frontier openers established.
///
/// # Why the trait peer earns its keep — Rc<[u8]> distinct from Arc<[u8]>
///
/// The by-value [`From<StorePath> for Rc<[u8]>`] (line 4991) covers the
/// by-value thread-local shared-owned byte-slice emit frontier. But a
/// large family of downstream sites hold a [`&StorePath`], not an owned
/// [`StorePath`], and must not consume the source:
///
/// - Iteration over a `[StorePath]` / `Vec<StorePath>` / `&[StorePath]`
...
///   [`crate::store_path::parse_closure_paths`], a derivation-input
///   manifest, an attestation-adjacent per-input path list) where each
///   element must project into a thread-local shared-owned
///   [`std::rc::Rc<[u8]>`] for downstream fan-out to on-thread consumers
///   — a `Vec<Rc<[u8]>>` payload list handed into a single-thread
///   derivation-graph walker where nodes share incoming-edge byte payloads
///   through cheap [`Rc::clone`] rather than deep-copy per neighbour, a
///   `Vec<Rc<[u8]>>` accumulation shared across a single-thread iterator
///   pipeline whose stages each keep their own [`Rc::clone`] handle, a
///   `serde_bytes`-tagged serialize-side field of shape `Vec<Rc<[u8]>>`
///   whose entries downstream reach on-thread recipients through
///   [`Rc::clone`] — the by-value peer's move consumes the element, which
///   iteration cannot afford. Pre-peer the site had to write
///   `std::rc::Rc::<[u8]>::from(sp.as_str().as_bytes().to_vec())` or
///   `std::rc::Rc::<[u8]>::from(<StorePath as AsRef<[u8]>>::as_ref(&sp).to_vec())`
///   at every consumer; post-peer `Rc::<[u8]>::from(sp)` or
///   `sp.into()` reads through this impl.
/// - `&StorePath`-typed method receivers or function arguments (a
///   pipeline-step primitive inspecting a store path without ownership, an
///   attestation record builder borrowing its inputs, a canonical-byte
///   digest-input accessor accepting a borrow) projecting into an
///   `impl Into<Rc<[u8]>>` sink at their downstream boundary — the
///   by-value peer requires a `sp.clone().into()` at the boundary if the
///   caller still needs the value; the by-reference peer avoids the clone.
/// - Trait objects and generic bounds that admit `&T` conversions through
///   the standard `impl<T> From<&T> for T::Owned`
...
///   [`From<StorePath>`] emit peer requires to compose in trait-generic
///   contexts (an intern-table populator receiving `&StorePath` and
///   handing it into an `impl Into<Rc<[u8]>>` key slot, an
///   [`std::rc::Rc<[u8]>`]-keyed thread-local cache populator typed on
///   `impl Into<Rc<[u8]>>` from a borrowed iteration).
///
/// [`std::rc::Rc<[u8]>`] is disjoint from [`std::sync::Arc<[u8]>`]
/// (line 6177) at the type level even though the two carry the same
/// bytes: [`Rc<[u8]>`] prefixes its payload with non-atomic strong/weak
/// count headers so clones share the same allocation on the owning
/// thread, while [`Arc<[u8]>`] prefixes its payload with atomic
/// strong/weak count headers so clones may cross `Send + Sync`
/// boundaries. [`Rc<[u8]>`] is neither [`Send`] nor [`Sync`] — a
/// downstream site pinned on the on-thread cheap-clone shape cannot pay
/// for the atomic refcount ordering [`Arc<[u8]>`] imposes at every
/// [`Arc::clone`] / [`Arc::drop`], and a site pinned on the cross-thread
/// fan-out shape cannot substitute [`Rc<[u8]>`] without losing
/// [`Send + Sync`]. This peer covers exactly the on-thread cheap-share
/// shape at the by-reference frontier.
///
/// The chain-through-`From<&StorePath> for Vec<u8>` design is the
/// one-oracle discipline THEORY §VI.1 demands: the emitted canonical bytes
/// travel through the same by-reference [`Vec::<u8>::from`] projection the
...
/// through) and every by-reference byte-side emit peer in the family
/// inherits it automatically. A per-impl direct call to
/// `std::rc::Rc::<[u8]>::from(sp.as_str().as_bytes())` here would work
/// today but would ossify the "AsRef<[u8]> to_rc" two-step at the
/// by-reference emit surface AND materialise a fresh [`&[u8]`] slice the
/// intermediate [`Vec<u8>`] peer already surfaces; the
/// `std::rc::Rc::<[u8]>::from(Vec::<u8>::from(sp))` chain reads through
/// the by-reference [`Vec<u8>`] frontier-opener so the family stays
/// composable by construction.
///
/// # Identity invariants pinned
///
/// - `<Rc<[u8]>>::from(&sp) == <Rc<[u8]>>::from(sp.clone())` (matches
///   the by-value peer's projection byte-for-byte) — pinned by
///   [`tests::test_from_store_path_ref_rc_bytes_matches_by_value_peer`].
/// - Trim-discipline preservation: a `\n`-terminated raw input emits an
///   [`Rc<[u8]>`] whose bytes equal the trimmed canonical form — pinned
///   by [`tests::test_from_store_path_ref_rc_bytes_carries_trim_discipline`].
/// - Trait-generic composition: an `impl Into<Rc<[u8]>>` consumer holding
///   a [`&StorePath`] projects the same canonical bytes — pinned by
///   [`tests::test_from_store_path_ref_rc_bytes_carries_through_generic_consumer`].
/// - Source-preservation: the source [`StorePath`] remains usable after
///   the emit (the by-reference peer does not consume) — pinned by
///   [`tests::test_from_store_path_ref_rc_bytes_preserves_source`].
impl From<&StorePath> for std::rc::Rc<[u8]> {
    fn from(sp: &StorePath) -> std::rc::Rc<[u8]> {
        std::rc::Rc::<[u8]>::from(Vec::<u8>::from(sp))
    }
...
/// By-reference borrowed-or-owned-byte-slice emit peer for [`StorePath`] via
/// [`std::borrow::Cow<'static, [u8]>`] — the tenth and closing step of the
/// by-reference owned-emit frontier opened by
/// [`From<&StorePath> for String`] (line 5265) and extended by
/// [`From<&StorePath> for Vec<u8>`] (line 5384),
/// [`From<&StorePath> for Box<str>`] (line 5497),
...
/// [`From<&StorePath> for Arc<str>`] (line 5746),
/// [`From<&StorePath> for Rc<str>`] (line 5883),
/// [`From<&StorePath> for Box<[u8]>`] (line 6025),
/// [`From<&StorePath> for Arc<[u8]>`] (line 6177), and
/// [`From<&StorePath> for Rc<[u8]>`] (line 6329), chaining through the same
/// by-reference [`Vec<u8>`] one-oracle discipline every by-reference
/// byte-side emit peer in the family routes through, mirroring how the
/// by-value [`From<StorePath> for Cow<'static, [u8]>`] (line 5185) chained
/// through the by-value [`Vec<u8>`] one-oracle every by-value byte-side emit
/// peer routes through.
///
/// Delegates through `std::borrow::Cow::Owned(Vec::<u8>::from(sp))`:
/// [`From<&StorePath> for Vec<u8>`] copies the borrowed
/// [`StorePath::as_str`] view's byte projection into a fresh [`Vec<u8>`] at
/// one allocation without consuming the source; the stdlib
/// [`std::borrow::Cow::Owned`] arm then wraps that fresh [`Vec<u8>`] as-is —
/// no fresh allocation, no buffer copy, no refcount header. A by-reference
/// emit cannot land in the [`std::borrow::Cow::Borrowed`] arm without
/// fabricating a `'static` reference from a runtime buffer, so the owned
/// arm is the only valid landing site: pinned by the
/// [`std::borrow::Cow::is_owned`] discipline in
/// [`tests::test_from_store_path_ref_cow_bytes_matches_by_value_peer`].
/// Source [`StorePath`] left available for further reads.
///
/// # Frontier-blanket calculus — closing the by-reference frontier
///
/// The [`From<&StorePath> for Vec<u8>`] impl doc records that stdlib blanket
/// `impl<T: AsRef<OsStr> + ?Sized> From<&T> for {PathBuf, OsString}` fires
/// for `T = StorePath` through this crate's [`AsRef<OsStr>`] peer (line
/// 1920) and forbids a custom by-reference peer at those two shapes
/// (E0119). [`Cow<'static, [u8]>`] carries no such stdlib blanket: stdlib
/// provides `impl<'a> From<&'a [u8]> for Cow<'a, [u8]>` (which the by-value
/// peer's downstream sites reach through `.as_ref().into()` in pre-peer
/// call sites at the borrowed-arm lifetime), but the `&'a [u8]` receiver
/// is a concrete type at a specific lifetime, not a generic bound, so it
/// does not fire for `T = &StorePath` (which is not `&[u8]`-shaped and
/// cannot be deref-coerced through [`From`]). The by-reference
/// [`Cow<'static, [u8]>`] peer therefore opens the frontier the stdlib
/// blanket doesn't reach and does so at the same one-oracle discipline the
/// preceding nine by-reference frontier peers established.
///
/// This is the closing peer of the by-reference owned-emit frontier: with
/// this landing, every non-blanketed owned shape on both the UTF-8-string
/// axis and the raw-byte-slice axis — `{String, Vec<u8>, Box<str>,
/// Cow<'static, str>, Arc<str>, Rc<str>, Box<[u8]>, Arc<[u8]>, Rc<[u8]>,
/// Cow<'static, [u8]>}` — has a by-reference emit peer symmetric to its
/// by-value sibling, and the two frontier axes close in symmetric shape
/// (five UTF-8-string peers, five raw-byte-slice peers). The
/// remaining by-value-only shapes ([`PathBuf`], [`std::ffi::OsString`]) are
/// stdlib-blanketed at the by-reference frontier and do not admit a
/// custom impl.
///
/// # Why the trait peer earns its keep
///
/// The by-value [`From<StorePath> for Cow<'static, [u8]>`] (line 5185)
/// covers the by-value borrowed-or-owned byte-slice emit frontier. But a
/// large family of downstream sites hold a [`&StorePath`], not an owned
/// [`StorePath`], and must not consume the source:
///
/// - Iteration over a `[StorePath]` / `Vec<StorePath>` / `&[StorePath]`
...
///   [`crate::store_path::parse_closure_paths`], a derivation-input
///   manifest, an attestation-adjacent per-input path list) where each
///   element must project into a [`Cow<'static, [u8]>`] for downstream
///   `serde_bytes`-derived record insertion, a `Vec<Cow<'static, [u8]>>`
///   intern-pool accumulation, or a
///   `HashMap<Cow<'static, [u8]>, _>` key slot — the by-value peer's move
///   consumes the element, which iteration cannot afford. Pre-peer the
///   site had to write
///   `Cow::Owned(<StorePath as AsRef<[u8]>>::as_ref(&sp).to_vec())` or
///   `Cow::Owned(String::from(&sp).into_bytes())` at every consumer;
///   post-peer `Cow::<[u8]>::from(sp)` or `sp.into()` reads through this
///   impl.
/// - `&StorePath`-typed method receivers or function arguments (a
///   pipeline-step primitive inspecting a store path without ownership, an
///   attestation record builder borrowing its inputs, a canonical-byte-label
///   extractor accepting a borrow) projecting into an
///   `impl Into<Cow<'static, [u8]>>` sink at their downstream boundary —
///   the by-value peer requires a `sp.clone().into()` at the boundary if
///   the caller still needs the value; the by-reference peer avoids the
///   clone.
/// - Trait objects and generic bounds that admit `&T` conversions through
///   the standard `impl<T> From<&T> for T::Owned`
...
///   completes the [`From<&StorePath>`] symmetry the by-value
///   [`From<StorePath>`] emit peer requires to compose in trait-generic
///   contexts (a wire-frame builder receiving `&StorePath` and handing it
///   into an `impl Into<Cow<'static, [u8]>>` payload slot, a
///   `#[serde(into = "Cow<'static, [u8]>")]`-decorated field emitter, a
///   content-addressed byte-payload sink typed on
///   `impl Into<Cow<'static, [u8]>>` from a borrowed iteration).
///
/// The chain-through-`From<&StorePath> for Vec<u8>` design is the
/// one-oracle discipline THEORY §VI.1 demands: the emitted canonical bytes
/// travel through the same by-reference [`Vec<u8>`] projection every
/// by-reference byte-side emit peer routes through (and, transitively, the
/// same borrowed-view [`AsRef<[u8]>`] peer the by-value emit peers'
/// [`StorePath::into_string`] backing surfaces), so a future canonical-form
/// refinement to the backing string (a tighter trim, a `SmartString`
/// handle, a canonicalising projection at the read surface) lands at one
/// site (the [`AsRef<str>`] impl at line 1684, which the [`AsRef<[u8]>`]
/// impl reads through) and every by-reference owned-emit peer in the
/// family inherits it automatically. A per-impl direct call to
/// `Cow::Owned(sp.as_str().as_bytes().to_vec())` here would work today but
/// would ossify the "as_str -> as_bytes -> to_vec" three-step at the
/// by-reference emit surface; the `Cow::Owned(Vec::<u8>::from(sp))` chain
/// reads through the by-reference [`Vec<u8>`] frontier-opener so the
/// family stays composable by construction.
///
/// # Symmetric closure with the parse frontier
///
/// The symmetric by-reference emit-side sibling of the by-value
/// [`TryFrom<Cow<'_, [u8]>> for StorePath`] (line 1189): a consumer that
/// receives a borrowed-or-owned byte-slice buffer from a
/// [`serde_bytes`]-adjacent field or a helper argument, parses it into a
/// [`StorePath`], validates it, and hands the *validated* value back out
/// as a [`Cow<'static, [u8]>`] at its own downstream boundary without
/// consuming the source (a common pattern where the validated value is
/// also cached for follow-up projections) reads through this peer without
/// a `sp.clone().into()` bridge.
///
/// # Identity invariants pinned
///
/// - `Cow::<[u8]>::from(&sp) == Cow::<[u8]>::from(sp.clone())` (matches
///   the by-value peer's projection byte-for-byte) — pinned by
///   [`tests::test_from_store_path_ref_cow_bytes_matches_by_value_peer`].
/// - Trim-discipline preservation: a `\n`-terminated raw input emits a
///   [`Cow<'static, [u8]>`] whose bytes equal the trimmed canonical form —
///   pinned by
///   [`tests::test_from_store_path_ref_cow_bytes_carries_trim_discipline`].
/// - Trait-generic composition: an `impl Into<Cow<'static, [u8]>>`
///   consumer holding a [`&StorePath`] projects the same canonical bytes —
///   pinned by
///   [`tests::test_from_store_path_ref_cow_bytes_carries_through_generic_consumer`].
/// - Owned-arm discipline: the emitted [`Cow`] is
///   [`std::borrow::Cow::Owned`] (a by-reference emit that allocated a
///   fresh [`Vec<u8>`] cannot land in the borrowed arm without fabricating
///   a `'static` reference from a runtime buffer) — pinned by
///   [`tests::test_from_store_path_ref_cow_bytes_matches_by_value_peer`].
/// - Source-preservation: the source [`StorePath`] remains usable after
///   the emit (the by-reference peer does not consume) — pinned by
///   [`tests::test_from_store_path_ref_cow_bytes_preserves_source`].
impl From<&StorePath> for std::borrow::Cow<'static, [u8]> {
    fn from(sp: &StorePath) -> std::borrow::Cow<'static, [u8]> {
        std::borrow::Cow::Owned(Vec::<u8>::from(sp))
    }
...
mod tests {
    ...
    /// [`From<StorePath> for Cow<'static, str>`] agrees with every
    /// borrowed-view UTF-8 projection [`StorePath`] already carries — the
    /// borrowed-view [`StorePath::as_str`] wrapped in [`Cow::Owned`], the
    /// [`AsRef<str>`] projection wrapped in [`Cow::Owned`], the
    /// [`std::fmt::Display`] format wrapped in [`Cow::Owned`], the
    /// inherent [`StorePath::into_string`] wrapped in [`Cow::Owned`], and
    /// the [`String::from`] emit wrapped in [`Cow::Owned`]. Pins the
    /// "borrowed-or-owned emit peer routes through the same one-oracle
    /// backing string the read peers project and the inherent
    /// owned-projection moves" invariant across every canonical
    /// store-path shape — a future divergence between the emitted
    /// [`Cow<'static, str>`] and the borrowed `&str` view fails this
    /// test. Also pins the owned-arm discipline: a moved-backing emit
    /// cannot land in the borrowed arm without fabricating a `'static`
    /// reference from a runtime buffer, so [`Cow::is_owned`] must hold at
    /// every shape.
    #[test]
    fn test_from_store_path_cow_str_matches_borrowed_projections() {
        for raw in [
            format!("/nix/store/{H}-x"),
            format!("/nix/store/{H}-hello-2.10"),
            format!("/nix/store/{H}-mysvc.drv"),
            ...
            let sp = StorePath::parse(&raw).expect("valid store path");
            let borrowed_as_str: std::borrow::Cow<'static, str> =
                std::borrow::Cow::Owned(sp.as_str().to_owned());
            let borrowed_as_ref: std::borrow::Cow<'static, str> =
                std::borrow::Cow::Owned(<StorePath as AsRef<str>>::as_ref(&sp).to_owned());
            let via_display: std::borrow::Cow<'static, str> =
                std::borrow::Cow::Owned(format!("{sp}"));
            let via_into_string: std::borrow::Cow<'static, str> =
                std::borrow::Cow::Owned(sp.clone().into_string());
            let via_string_from: std::borrow::Cow<'static, str> =
                std::borrow::Cow::Owned(String::from(sp.clone()));
            let emitted: std::borrow::Cow<'static, str> =
                <std::borrow::Cow<'static, str> as From<StorePath>>::from(sp.clone());
            assert_eq!(
                emitted, borrowed_as_str,
                "From<StorePath> for Cow<'static, str> must match Cow::Owned(sp.as_str().to_owned())"
            );
            assert_eq!(
                emitted, borrowed_as_ref,
                "From<StorePath> for Cow<'static, str> must match Cow::Owned(<StorePath as AsRef<str>>::as_ref)"
            );
            assert_eq!(
                emitted, via_display,
                "From<StorePath> for Cow<'static, str> must match Cow::Owned(format!(\"{{sp}}\"))"
            );
            assert_eq!(
                emitted, via_into_string,
                "From<StorePath> for Cow<'static, str> must match Cow::Owned(sp.into_string())"
            );
            assert_eq!(
                emitted, via_string_from,
                "From<StorePath> for Cow<'static, str> must match Cow::Owned(String::from(sp))"
            );
            assert_eq!(
                emitted.as_ref(),
                raw.as_str(),
                "From<StorePath> for Cow<'static, str> on a whitespace-free input must equal the raw input"
            );
            assert!(
                matches!(&emitted, std::borrow::Cow::Owned(_)),
                "From<StorePath> for Cow<'static, str> must land in the Owned arm (a moved backing cannot forge a 'static borrow)"
            );
            // Payload-preserving discipline: the emitted Cow's byte length
            // equals the payload byte count, so a `<Cow<'_, str> as
            // AsRef<str>>` sink sees the canonical form byte-for-byte.
            assert_eq!(emitted.as_ref().len(), raw.len());
        }
    }

    /// [`From<StorePath> for Cow<'static, str>`] carries
    /// [`StorePath::parse`]'s trimming discipline through the by-value
    /// borrowed-or-owned-UTF-8 emit surface — a value parsed from a
    /// newline-terminated nix-frontier stdout buffer emits a
    /// [`Cow<'static, str>`] whose bytes are the trimmed canonical bytes,
    /// not the raw newline-terminated buffer. Pins the same trim
    /// discipline the [`String`], [`std::path::PathBuf`],
    /// [`std::ffi::OsString`], [`Vec<u8>`], and [`Box<str>`] emit peers
    /// already carry, now on the [`Cow<'static, str>`] emit peer so a
    /// downstream `fn f<T: Into<Cow<'static, str>>>(sp: T)` sees the
    /// canonical form with no per-site
    /// `Cow::Owned(sp.as_str().trim().to_owned())` restatement.
    #[test]
    fn test_from_store_path_cow_str_carries_trim_discipline() {
        let sp = StorePath::parse(&format!("/nix/store/{H}-x\n")).expect("valid");
        let emitted: std::borrow::Cow<'static, str> = sp.into();
        let expected = format!("/nix/store/{H}-x");
        assert_eq!(emitted.as_ref(), expected.as_str());
        assert!(
            !emitted.as_ref().ends_with('\n'),
            "trailing newline must be trimmed"
        );
        assert!(
            !emitted.as_ref().starts_with(' '),
            "leading whitespace must be trimmed"
        );
    }

    /// The [`From<StorePath> for Cow<'static, str>`] impl composes with a
    /// generic borrowed-or-owned-string helper bounded by
    /// `impl Into<Cow<'static, str>>` — the compositional motivation for
    /// landing the trait separately from the borrowed-view [`AsRef<str>`]
    /// read peer, the [`From<StorePath> for String`] owned emit peer, and
    /// the [`From<StorePath> for Box<str>`] shrunk-owned emit peer. Pins
    /// the trait-generic consumer surface: a downstream site that types
    /// its input contract as `impl Into<Cow<'static, str>>` (a
    /// [`std::collections::HashMap`]`<Cow<'static, str>, _>::insert` key
    /// that accepts either a compile-time `&'static str` literal or a
    /// runtime-owned buffer through the same slot, a
    /// `Result<Cow<'static, str>>`-returning outer wrapper preserving a
    /// pre-existing public shape, a config-schema setter that accepts a
    ...
    /// `Cow::Owned(sp.into_string())` call would.
    #[test]
    fn test_from_store_path_cow_str_carries_through_generic_consumer() {
        fn first_char_of<T: Into<std::borrow::Cow<'static, str>>>(t: T) -> char {
            let s: std::borrow::Cow<'static, str> = t.into();
            s.chars().next().unwrap()
        }
        ...
        fn owned_eq<T: Into<std::borrow::Cow<'static, str>>>(t: T, expected: &str) -> bool {
            let s: std::borrow::Cow<'static, str> = t.into();
            s.as_ref() == expected
        }
        let raw = format!("/nix/store/{H}-hello-2.10");
    ...
    /// Round-trip fidelity across the by-value borrowed-or-owned-UTF-8
    /// input+output symmetry: [`From<StorePath> for Cow<'static, str>`]
    /// emits the canonical bytes (in the owned arm), and
    /// [`TryFrom<Cow<'_, str>> for StorePath`] (line 568) parses them
    ...
    /// [`test_from_store_path_vec_bytes_parse_round_trip`], and
    /// [`test_from_store_path_box_str_parse_round_trip`] pin on their
    /// respective emit-parse symmetries, now on the
    /// [`Cow<'static, str>`] emit-parse symmetry — a consumer that
    /// receives a borrowed-or-owned UTF-8 buffer from a `serde` field or
    /// a helper argument, parses it into a [`StorePath`], and hands it
    /// back out as a [`Cow<'static, str>`] at its own downstream
    /// boundary reads through both peers at zero per-consumer bridge
    /// cost.
    #[test]
    fn test_from_store_path_cow_str_parse_round_trip() {
        for raw in [
            format!("/nix/store/{H}-x"),
            format!("/nix/store/{H}-hello-2.10"),
            format!("/nix/store/{H}-mysvc.drv"),
    ...
    /// [`From<StorePath> for Arc<[u8]>`] projects the canonical store-path
    /// bytes through the by-value cross-thread shared-owned-byte-slice
    /// emit peer symmetric to the borrowed-view [`AsRef<[u8]>`], the
    /// by-value owned [`Vec<u8>`], the shrunk-owned [`Box<[u8]>`], and
    /// the UTF-8-axis [`Arc<str>`] read/emit peers. Reads through the
    /// same one-oracle backing string [`StorePath::full`] the read-peer
    /// siblings project ([`AsRef<[u8]>`]) and the inherent
    /// [`StorePath::into_string`] owned projection moves out, chained via
    /// [`Vec::<u8>::from`] into [`std::sync::Arc<[u8]>::from`]. Pins the
    /// "cross-thread shared-owned byte-slice emit peer routes through the
    /// same one-oracle backing string the read peers project and the
    /// inherent owned-projection moves" invariant across every canonical
    /// store-path shape — a future divergence between the moved-then-arc
    /// [`Arc<[u8]>`] and the borrowed `&[u8]` view fails this test.
    #[test]
    fn test_from_store_path_arc_bytes_matches_borrowed_projections() {
        for raw in [
            format!("/nix/store/{H}-x"),
            format!("/nix/store/{H}-hello-2.10"),
            format!("/nix/store/{H}-mysvc.drv"),
            ...
            let sp = StorePath::parse(&raw).expect("valid store path");
            let borrowed_as_bytes: std::sync::Arc<[u8]> =
                <StorePath as AsRef<[u8]>>::as_ref(&sp).into();
            let via_str_as_bytes: std::sync::Arc<[u8]> = sp.as_str().as_bytes().into();
            let via_vec_from: std::sync::Arc<[u8]> =
                std::sync::Arc::<[u8]>::from(Vec::<u8>::from(sp.clone()));
            let via_string_from: std::sync::Arc<[u8]> =
                std::sync::Arc::<[u8]>::from(String::from(sp.clone()).into_bytes());
            let via_box_bytes: std::sync::Arc<[u8]> =
                std::sync::Arc::<[u8]>::from(Box::<[u8]>::from(sp.clone()));
            let emitted: std::sync::Arc<[u8]> = std::sync::Arc::<[u8]>::from(sp.clone());
            assert_eq!(
                &*emitted, &*borrowed_as_bytes,
                "From<StorePath> for Arc<[u8]> must match <StorePath as AsRef<[u8]>>::as_ref"
            );
            assert_eq!(
                &*emitted, &*via_str_as_bytes,
                "From<StorePath> for Arc<[u8]> must match sp.as_str().as_bytes().into()"
            );
            assert_eq!(
                &*emitted, &*via_vec_from,
                "From<StorePath> for Arc<[u8]> must match Arc::<[u8]>::from(Vec::<u8>::from(sp))"
            );
            assert_eq!(
                &*emitted, &*via_string_from,
                "From<StorePath> for Arc<[u8]> must match Arc::<[u8]>::from(String::from(sp).into_bytes())"
            );
            assert_eq!(
                &*emitted, &*via_box_bytes,
                "From<StorePath> for Arc<[u8]> must match Arc::<[u8]>::from(Box::<[u8]>::from(sp))"
            );
            assert_eq!(
                &*emitted,
                raw.as_bytes(),
                "From<StorePath> for Arc<[u8]> on a whitespace-free input must equal raw bytes"
            );
            // Cross-thread shared-owned discipline: the emitted Arc<[u8]>'s
            // deref length equals the payload byte count, so an
            // `<Arc<[u8]> as AsRef<[u8]>>` sink sees the canonical form
            // byte-for-byte with no capacity slack visible through the
            // shared allocation.
            assert_eq!(emitted.len(), raw.len());
            // Cheap-clone discipline: an Arc::clone bumps the atomic
            ...
            // `Arc::ptr_eq` witnesses that invariant on the emitted value
            // itself.
            let shared = emitted.clone();
            assert!(
                std::sync::Arc::ptr_eq(&emitted, &shared),
    ...
    /// [`From<StorePath> for Arc<[u8]>`] carries [`StorePath::parse`]'s
    /// trimming discipline through the by-value cross-thread
    /// shared-owned-byte-slice emit surface — a value parsed from a
    /// newline-terminated nix-frontier stdout buffer emits an
    /// [`Arc<[u8]>`] whose bytes are the trimmed canonical bytes, not the
    /// raw newline-terminated buffer. Pins the same trim discipline the
    /// [`String`], [`std::path::PathBuf`], [`std::ffi::OsString`],
    /// [`Vec<u8>`], [`Box<str>`], [`Cow<'static, str>`], [`Arc<str>`],
    /// [`Rc<str>`], and [`Box<[u8]>`] emit peers already carry, now on
    /// the [`Arc<[u8]>`] emit peer so a downstream
    /// `fn f<T: Into<Arc<[u8]>>>(sp: T)` sees the canonical byte form
    /// with no per-site
    /// `Arc::<[u8]>::from(sp.as_str().trim().as_bytes().to_vec())`
    /// restatement.
    #[test]
    fn test_from_store_path_arc_bytes_carries_trim_discipline() {
        let sp = StorePath::parse(&format!("/nix/store/{H}-x\n")).expect("valid");
    ...
    /// The [`From<StorePath> for Arc<[u8]>`] impl composes with a generic
    /// cross-thread shared-owned-byte-slice helper bounded by
    /// `impl Into<Arc<[u8]>>` — the compositional motivation for landing
    /// the trait separately from the borrowed-view [`AsRef<[u8]>`] read
    /// peer, the [`From<StorePath> for Vec<u8>`] owned emit peer, and the
    /// [`From<StorePath> for Box<[u8]>`] shrunk-owned emit peer. Pins the
    /// trait-generic consumer surface: a downstream site that types its
    /// input contract as `impl Into<Arc<[u8]>>` (a
    /// [`std::collections::HashMap`]`<Arc<[u8]>, _>::insert` key that
    /// hands the same canonical-byte-label allocation across worker
    /// threads through [`std::sync::Arc::clone`], a
    /// `Result<Arc<[u8]>>`-returning outer wrapper preserving a
    /// pre-existing public shape, a task-spawn argument slot on a
    /// Tokio-worker pool that ships a canonical byte-slice label into
    /// every spawned worker without allocating per task) recovers the
    /// same validated bytes a direct
    /// `std::sync::Arc::<[u8]>::from(sp.into_string().into_bytes())` call
    /// would.
    #[test]
    fn test_from_store_path_arc_bytes_carries_through_generic_consumer() {
        fn len_of<T: Into<std::sync::Arc<[u8]>>>(t: T) -> usize {
            let s: std::sync::Arc<[u8]> = t.into();
            s.len()
        }
        fn to_utf8<T: Into<std::sync::Arc<[u8]>>>(t: T) -> String {
            let s: std::sync::Arc<[u8]> = t.into();
            std::str::from_utf8(&s).expect("valid utf-8").to_owned()
        }
        fn owned_eq<T: Into<std::sync::Arc<[u8]>>>(t: T, expected: &[u8]) -> bool {
            let s: std::sync::Arc<[u8]> = t.into();
            &*s == expected
        }
        let raw = format!("/nix/store/{H}-hello-2.10");
    ...
    /// [`From<StorePath> for Arc<[u8]>`] emits the canonical bytes, and
    /// [`TryFrom<Arc<[u8]>> for StorePath`] (line 1294) parses them back
    /// through the same [`StorePath::parse`] one-oracle. Pins the same
    /// round-trip fidelity
    /// [`test_from_store_path_vec_bytes_parse_round_trip`],
    /// [`test_from_store_path_box_bytes_parse_round_trip`], and
    /// [`test_from_store_path_arc_str_parse_round_trip`] pin on their
    /// respective emit-parse symmetries, now on the [`Arc<[u8]>`]
    /// emit-parse symmetry — a consumer that receives a cross-thread
    /// shared-owned raw-byte buffer from a Tokio-task boundary or a
    /// channel receiver, parses it into a [`StorePath`], and hands it
    /// back out as an [`Arc<[u8]>`] at its own downstream fan-out reads
    /// through both peers at zero per-consumer bridge cost.
    #[test]
    fn test_from_store_path_arc_bytes_parse_round_trip() {
        for raw in [
            format!("/nix/store/{H}-x"),
            format!("/nix/store/{H}-hello-2.10"),
            format!("/nix/store/{H}-mysvc.drv"),
    ...
    /// [`From<&StorePath> for String`] projects the same canonical
    /// bytes the by-value [`From<StorePath> for String`] peer emits and
    /// the borrowed-view [`AsRef<str>`] read peer surfaces — pins the
    ...
    #[test]
    fn test_from_store_path_ref_string_matches_by_value_peer() {
        for raw in [
            format!("/nix/store/{H}-x"),
            format!("/nix/store/{H}-hello-2.10"),
            format!("/nix/store/{H}-mysvc.drv"),
            ...
            let sp = StorePath::parse(&raw).expect("valid store path");
            let by_ref_emit: String = String::from(&sp);
            let by_ref_into: String = (&sp).into();
            let by_val_emit: String = String::from(sp.clone());
            let via_as_str = sp.as_str().to_owned();
            let via_as_ref: String = <StorePath as AsRef<str>>::as_ref(&sp).to_owned();
            let via_display = format!("{sp}");
            assert_eq!(
                by_ref_emit, by_val_emit,
                "From<&StorePath> must match From<StorePath> byte-for-byte"
            );
            assert_eq!(by_ref_emit, by_ref_into, "From vs Into consistency");
            assert_eq!(by_ref_emit, via_as_str, "must match StorePath::as_str");
            assert_eq!(by_ref_emit, via_as_ref, "must match AsRef<str>");
            assert_eq!(by_ref_emit, via_display, "must match Display");
            assert_eq!(
                by_ref_emit, raw,
                "on whitespace-free input must equal raw input"
            );
        }
    }

    /// [`From<&StorePath> for String`] carries [`StorePath::parse`]'s
    /// trim discipline through the by-reference owned-UTF-8 emit
    /// surface — a value parsed from a newline-terminated nix-frontier
    /// stdout buffer emits the trimmed canonical bytes through the
    /// borrowed accessor with no per-site `sp.as_str().trim().to_owned()`
    /// restatement.
    #[test]
    fn test_from_store_path_ref_string_carries_trim_discipline() {
        let sp = StorePath::parse(&format!("/nix/store/{H}-x\n")).expect("valid");
        let emitted: String = String::from(&sp);
        let expected = format!("/nix/store/{H}-x");
        assert_eq!(emitted, expected);
    ...
    /// The [`From<&StorePath> for String`] impl composes with a generic
    /// owned-string helper bounded by `impl Into<String>` receiving a
    /// [`&StorePath`] — the compositional motivation for landing the
    /// by-reference peer separately from the by-value
    /// [`From<StorePath> for String`] emit peer. Pins the trait-generic
    /// consumer surface: a downstream site holding a [`&StorePath`]
    /// (an iteration over a `[StorePath]` slice, an attestation record
    /// builder borrowing its inputs, a `&self`-receiver method) that
    /// types its input contract as `impl Into<String>` recovers the
    /// same validated bytes a direct `sp.as_str().to_owned()` call would.
    #[test]
    fn test_from_store_path_ref_string_carries_through_generic_consumer() {
        fn first_char_of<T: Into<String>>(t: T) -> char {
            let s: String = t.into();
            s.chars().next().unwrap()
        }
    ...
    /// [`From<&StorePath> for String`] does NOT consume the source
    /// [`StorePath`] — the borrowed emit surface must leave the value
    /// available for further reads. Pins the load-bearing distinction
    /// from the by-value [`From<StorePath> for String`] peer: an
    /// iteration over a `[StorePath]` slice or a `&StorePath`-receiver
    /// method that emits an owned [`String`] must retain the source for
    /// downstream consumers on the same expression, which the by-value
    /// peer's move discipline forbids.
    #[test]
    fn test_from_store_path_ref_string_preserves_source() {
        let raw = format!("/nix/store/{H}-hello-2.10");
        let sp = StorePath::parse(&raw).expect("valid");
        let first: String = String::from(&sp);
        let second: String = String::from(&sp);
        let third: String = (&sp).into();
        assert_eq!(first, raw);
    ...
    /// [`From<&StorePath> for Vec<u8>`] projects the same canonical
    /// bytes the by-value [`From<StorePath> for Vec<u8>`] peer emits and
    /// the borrowed-view [`AsRef<[u8]>`] read peer surfaces — pins the
    /// "by-reference emit peer routes through the same one-oracle
    /// backing every projection reads from" invariant across every
    /// canonical store-path shape.
    #[test]
    fn test_from_store_path_ref_bytes_matches_by_value_peer() {
        for raw in [
            format!("/nix/store/{H}-x"),
            format!("/nix/store/{H}-hello-2.10"),
            format!("/nix/store/{H}-mysvc.drv"),
            ...
            let sp = StorePath::parse(&raw).expect("valid store path");
            let by_ref_emit: Vec<u8> = Vec::<u8>::from(&sp);
            let by_ref_into: Vec<u8> = (&sp).into();
            let by_val_emit: Vec<u8> = Vec::<u8>::from(sp.clone());
            let via_as_ref: Vec<u8> = <StorePath as AsRef<[u8]>>::as_ref(&sp).to_vec();
            let via_as_str_bytes: Vec<u8> = sp.as_str().as_bytes().to_vec();
            let via_ref_string_bytes: Vec<u8> = String::from(&sp).into_bytes();
            assert_eq!(
                by_ref_emit, by_val_emit,
                "From<&StorePath> for Vec<u8> must match From<StorePath> for Vec<u8> byte-for-byte"
            );
            assert_eq!(by_ref_emit, by_ref_into, "From vs Into consistency");
            assert_eq!(
                by_ref_emit, via_as_ref,
                "must match <StorePath as AsRef<[u8]>>::as_ref(&sp).to_vec()"
            );
            assert_eq!(
                by_ref_emit, via_as_str_bytes,
                "must match sp.as_str().as_bytes().to_vec()"
            );
            assert_eq!(
                by_ref_emit, via_ref_string_bytes,
                "must match String::from(&sp).into_bytes()"
            );
            assert_eq!(
                by_ref_emit,
                raw.as_bytes().to_vec(),
                "on whitespace-free input must equal raw.as_bytes().to_vec()"
            );
        }
    }

    /// [`From<&StorePath> for Vec<u8>`] carries [`StorePath::parse`]'s
    /// trim discipline through the by-reference owned-byte-vector emit
    /// surface — a value parsed from a newline-terminated nix-frontier
    /// stdout buffer emits the trimmed canonical bytes as a [`Vec<u8>`]
    /// with no per-site `sp.as_str().trim().as_bytes().to_vec()`
    /// restatement.
    #[test]
    fn test_from_store_path_ref_bytes_carries_trim_discipline() {
        let sp = StorePath::parse(&format!("/nix/store/{H}-x\n")).expect("valid");
        let emitted: Vec<u8> = Vec::<u8>::from(&sp);
        let expected = format!("/nix/store/{H}-x").into_bytes();
        assert_eq!(emitted, expected);
        assert!(!emitted.ends_with(b"\n"));
        assert!(!emitted.starts_with(b" "));
        // Payload-preserving discipline: emitted len equals payload len,
        // so a byte-slice sink sees the canonical form with no capacity
        // slack visible through the borrow.
        assert_eq!(emitted.len(), sp.as_str().len());
    }

    /// The [`From<&StorePath> for Vec<u8>`] impl composes with a generic
    /// owned-byte-vector helper bounded by `impl Into<Vec<u8>>`
    /// receiving a [`&StorePath`] — the compositional motivation for
    /// landing the by-reference peer separately from the by-value
    /// [`From<StorePath> for Vec<u8>`] emit peer. Pins the trait-generic
    /// consumer surface: a downstream site holding a [`&StorePath`] (an
    /// iteration over a `[StorePath]` slice, an attestation record
    /// builder borrowing its inputs, a digest-input `&self`-receiver
    /// method) that types its input contract as `impl Into<Vec<u8>>`
    /// recovers the same validated bytes a direct
    /// `<StorePath as AsRef<[u8]>>::as_ref(&sp).to_vec()` call would.
    #[test]
    fn test_from_store_path_ref_bytes_carries_through_generic_consumer() {
        fn first_byte_of<T: Into<Vec<u8>>>(t: T) -> u8 {
            let v: Vec<u8> = t.into();
            v[0]
        }
        fn length_of<T: Into<Vec<u8>>>(t: T) -> usize {
            let v: Vec<u8> = t.into();
            v.len()
        }
        fn bytes_eq<T: Into<Vec<u8>>>(t: T, expected: &[u8]) -> bool {
            let v: Vec<u8> = t.into();
            v == expected
        }
        let raw = format!("/nix/store/{H}-hello-2.10");
    ...
    /// [`From<&StorePath> for Vec<u8>`] does NOT consume the source
    /// [`StorePath`] — the borrowed emit surface must leave the value
    /// available for further reads. Pins the load-bearing distinction
    /// from the by-value [`From<StorePath> for Vec<u8>`] peer: an
    /// iteration over a `[StorePath]` slice or a `&StorePath`-receiver
    /// method that emits an owned [`Vec<u8>`] must retain the source for
    /// downstream consumers on the same expression, which the by-value
    /// peer's move discipline forbids.
    #[test]
    fn test_from_store_path_ref_bytes_preserves_source() {
        let raw = format!("/nix/store/{H}-hello-2.10");
        let sp = StorePath::parse(&raw).expect("valid");
        let first: Vec<u8> = Vec::<u8>::from(&sp);
        let second: Vec<u8> = Vec::<u8>::from(&sp);
        let third: Vec<u8> = (&sp).into();
        let expected = raw.as_bytes().to_vec();
    ...
    /// [`From<&StorePath> for Cow<'static, str>`] projects the same
    /// canonical bytes the by-value [`From<StorePath> for Cow<'static, str>`]
    /// peer emits and every borrowed-view UTF-8 projection surfaces — pins
    /// the "by-reference emit peer routes through the same one-oracle
    /// backing every projection reads from" invariant across every
    /// canonical store-path shape, and pins the owned-arm discipline
    /// (a by-reference emit that allocates a fresh [`String`] can never
    /// land in the borrowed arm without fabricating a `'static` reference
    /// from a runtime buffer).
    #[test]
    fn test_from_store_path_ref_cow_str_matches_by_value_peer() {
        for raw in [
            format!("/nix/store/{H}-x"),
            format!("/nix/store/{H}-hello-2.10"),
            format!("/nix/store/{H}-mysvc.drv"),
            ...
            let sp = StorePath::parse(&raw).expect("valid store path");
            let by_ref_emit: std::borrow::Cow<'static, str> =
                std::borrow::Cow::<'static, str>::from(&sp);
            let by_ref_into: std::borrow::Cow<'static, str> = (&sp).into();
            let by_val_emit: std::borrow::Cow<'static, str> =
                std::borrow::Cow::<'static, str>::from(sp.clone());
            let via_ref_string: std::borrow::Cow<'static, str> =
                std::borrow::Cow::Owned(String::from(&sp));
            let via_as_str_owned: std::borrow::Cow<'static, str> =
                std::borrow::Cow::Owned(sp.as_str().to_owned());
            assert_eq!(
                by_ref_emit, by_val_emit,
                "From<&StorePath> for Cow<'static, str> must match From<StorePath> for Cow<'static, str> byte-for-byte"
            );
            assert_eq!(by_ref_emit, by_ref_into, "From vs Into consistency");
            assert_eq!(
                by_ref_emit, via_ref_string,
                "must match Cow::Owned(String::from(&sp))"
            );
            assert_eq!(
                by_ref_emit, via_as_str_owned,
                "must match Cow::Owned(sp.as_str().to_owned())"
            );
            assert_eq!(
                by_ref_emit.as_ref(),
                raw.as_str(),
            ...
            // Owned-arm discipline: a by-reference allocating emit cannot
            // land in the borrowed arm without fabricating a `'static`
            // reference from a runtime buffer.
            assert!(
                matches!(by_ref_emit, std::borrow::Cow::Owned(_)),
                "by-reference Cow emit must be Cow::Owned"
            );
        }
    }

    /// [`From<&StorePath> for Cow<'static, str>`] carries
    /// [`StorePath::parse`]'s trim discipline through the by-reference
    /// borrowed-or-owned UTF-8 emit surface — a value parsed from a
    /// newline-terminated nix-frontier stdout buffer emits a
    /// [`Cow<'static, str>`] whose bytes are the trimmed canonical form
    /// with no per-site `Cow::Owned(sp.as_str().trim().to_owned())`
    /// restatement.
    #[test]
    fn test_from_store_path_ref_cow_str_carries_trim_discipline() {
        let sp = StorePath::parse(&format!("/nix/store/{H}-x\n")).expect("valid");
        let emitted: std::borrow::Cow<'static, str> = std::borrow::Cow::<'static, str>::from(&sp);
        let expected = format!("/nix/store/{H}-x");
        assert_eq!(emitted.as_ref(), expected.as_str());
        assert!(!emitted.ends_with('\n'));
        assert!(!emitted.starts_with(' '));
        assert!(
            matches!(emitted, std::borrow::Cow::Owned(_)),
            "trim-carrying by-reference Cow emit must be Cow::Owned"
        );
    }

    /// The [`From<&StorePath> for Cow<'static, str>`] impl composes with a
    /// generic borrowed-or-owned-UTF-8 helper bounded by
    /// `impl Into<Cow<'static, str>>` receiving a [`&StorePath`] — the
    /// compositional motivation for landing the by-reference peer separately
    /// from the by-value [`From<StorePath> for Cow<'static, str>`] emit
    /// peer. Pins the trait-generic consumer surface: a downstream site
    /// holding a [`&StorePath`] (an iteration over a `[StorePath]` slice,
    /// an attestation record builder borrowing its inputs, an intern-table
    /// populator `&self`-receiver method) that types its input contract as
    /// `impl Into<Cow<'static, str>>` recovers the same validated bytes a
    /// direct `Cow::Owned(sp.as_str().to_owned())` call would.
    #[test]
    fn test_from_store_path_ref_cow_str_carries_through_generic_consumer() {
        fn first_char_of<T: Into<std::borrow::Cow<'static, str>>>(t: T) -> char {
        ...
        fn bytes_eq<T: Into<std::borrow::Cow<'static, str>>>(t: T, expected: &str) -> bool {
            let c: std::borrow::Cow<'static, str> = t.into();
            c.as_ref() == expected
        }
        fn is_owned<T: Into<std::borrow::Cow<'static, str>>>(t: T) -> bool {
            let c: std::borrow::Cow<'static, str> = t.into();
            matches!(c, std::borrow::Cow::Owned(_))
        }
        let raw = format!("/nix/store/{H}-hello-2.10");
        let sp = StorePath::parse(&raw).expect("valid");
        assert_eq!(first_char_of(&sp), '/');
        ...
        assert!(
            is_owned(&sp),
            "generic-consumer by-reference Cow emit must be Cow::Owned"
        );
    }

    /// [`From<&StorePath> for Cow<'static, str>`] does NOT consume the
    /// source [`StorePath`] — the borrowed emit surface must leave the
    /// value available for further reads. Pins the load-bearing distinction
    /// from the by-value [`From<StorePath> for Cow<'static, str>`] peer:
    /// an iteration over a `[StorePath]` slice or a `&StorePath`-receiver
    /// method that emits an owned [`Cow<'static, str>`] must retain the
    /// source for downstream consumers on the same expression, which the
    /// by-value peer's move discipline forbids.
    #[test]
    fn test_from_store_path_ref_cow_str_preserves_source() {
        let raw = format!("/nix/store/{H}-hello-2.10");
        let sp = StorePath::parse(&raw).expect("valid");
        let first: std::borrow::Cow<'static, str> = std::borrow::Cow::<'static, str>::from(&sp);
        let second: std::borrow::Cow<'static, str> = std::borrow::Cow::<'static, str>::from(&sp);
        let third: std::borrow::Cow<'static, str> = (&sp).into();
        assert_eq!(first.as_ref(), raw.as_str());
        assert_eq!(second.as_ref(), raw.as_str());
        assert_eq!(third.as_ref(), raw.as_str());
        assert_eq!(sp.as_str(), raw);
    ...
    /// [`From<&StorePath> for Rc<str>`] projects the same canonical bytes
    /// the by-value [`From<StorePath> for Rc<str>`] peer emits and every
    /// borrowed-view UTF-8 projection surfaces — pins the "by-reference
    /// emit peer routes through the same one-oracle backing every
    /// projection reads from" invariant across every canonical store-path
    /// shape. The cheap-clone assertion at the tail witnesses the
    /// [`std::rc::Rc<str>`] shape's non-atomic refcount discipline: an
    /// [`Rc::clone`](std::rc::Rc::clone) on the emitted value must share
    /// the underlying allocation (not copy the payload) so a downstream
    /// single-thread fan-out through the by-reference peer carries the
    /// same allocation-sharing discipline the by-value peer carries.
    #[test]
    fn test_from_store_path_ref_rc_str_matches_by_value_peer() {
        for raw in [
            format!("/nix/store/{H}-x"),
            format!("/nix/store/{H}-hello-2.10"),
            format!("/nix/store/{H}-mysvc.drv"),
            ...
            let sp = StorePath::parse(&raw).expect("valid store path");
            let by_ref_emit: std::rc::Rc<str> = std::rc::Rc::<str>::from(&sp);
            let by_ref_into: std::rc::Rc<str> = (&sp).into();
            let by_val_emit: std::rc::Rc<str> = std::rc::Rc::<str>::from(sp.clone());
            let via_as_str: std::rc::Rc<str> = sp.as_str().into();
            let via_as_ref: std::rc::Rc<str> = <StorePath as AsRef<str>>::as_ref(&sp).into();
            let via_ref_string: std::rc::Rc<str> = std::rc::Rc::<str>::from(String::from(&sp));
            assert_eq!(
                &*by_ref_emit, &*by_val_emit,
                "From<&StorePath> for Rc<str> must match From<StorePath> for Rc<str> byte-for-byte"
            );
            assert_eq!(&*by_ref_emit, &*by_ref_into, "From vs Into consistency");
            assert_eq!(&*by_ref_emit, &*via_as_str, "must match sp.as_str().into()");
            assert_eq!(
            ...
            assert_eq!(
                &*by_ref_emit, &*via_ref_string,
                "must match Rc::<str>::from(String::from(&sp))"
            );
            assert_eq!(
                &*by_ref_emit,
                raw.as_str(),
            ...
            let shared = by_ref_emit.clone();
            assert!(
                std::rc::Rc::ptr_eq(&by_ref_emit, &shared),
                "Rc::clone must share the by-reference emit peer's allocation, not copy the payload"
            );
        }
    }

    /// [`From<&StorePath> for Rc<str>`] carries [`StorePath::parse`]'s
    /// trim discipline through the by-reference thread-local shared-owned
    /// UTF-8 emit surface — a value parsed from a newline-terminated
    /// nix-frontier stdout buffer emits an [`Rc<str>`] whose bytes are
    /// the trimmed canonical form with no per-site
    /// `Rc::<str>::from(sp.as_str().trim())` restatement.
    #[test]
    fn test_from_store_path_ref_rc_str_carries_trim_discipline() {
        let sp = StorePath::parse(&format!("/nix/store/{H}-x\n")).expect("valid");
        let emitted: std::rc::Rc<str> = std::rc::Rc::<str>::from(&sp);
        let expected = format!("/nix/store/{H}-x");
        assert_eq!(&*emitted, expected.as_str());
        assert!(!emitted.ends_with('\n'));
        assert!(!emitted.starts_with(' '));
    ...
    /// The [`From<&StorePath> for Rc<str>`] impl composes with a generic
    /// thread-local shared-owned-UTF-8 helper bounded by
    /// `impl Into<Rc<str>>` receiving a [`&StorePath`] — the
    /// compositional motivation for landing the by-reference peer
    /// separately from the by-value [`From<StorePath> for Rc<str>`] emit
    /// peer. Pins the trait-generic consumer surface: a downstream site
    /// holding a [`&StorePath`] (an iteration over a `[StorePath]` slice,
    /// an attestation record builder borrowing its inputs, an
    /// intern-table populator `&self`-receiver method) that types its
    /// input contract as `impl Into<Rc<str>>` recovers the same
    /// validated bytes a direct
    /// `std::rc::Rc::<str>::from(sp.as_str())` call would.
    #[test]
    fn test_from_store_path_ref_rc_str_carries_through_generic_consumer() {
        fn first_char_of<T: Into<std::rc::Rc<str>>>(t: T) -> char {
            let s: std::rc::Rc<str> = t.into();
            s.chars().next().unwrap()
        }
    ...
    /// [`From<&StorePath> for Rc<str>`] does NOT consume the source
    /// [`StorePath`] — the borrowed emit surface must leave the value
    /// available for further reads. Pins the load-bearing distinction
    /// from the by-value [`From<StorePath> for Rc<str>`] peer: an
    /// iteration over a `[StorePath]` slice or a `&StorePath`-receiver
    /// method that emits an owned [`Rc<str>`] must retain the source
    /// for downstream consumers on the same expression, which the
    /// by-value peer's move discipline forbids.
    #[test]
    fn test_from_store_path_ref_rc_str_preserves_source() {
        let raw = format!("/nix/store/{H}-hello-2.10");
        let sp = StorePath::parse(&raw).expect("valid");
        let first: std::rc::Rc<str> = std::rc::Rc::<str>::from(&sp);
        let second: std::rc::Rc<str> = std::rc::Rc::<str>::from(&sp);
        let third: std::rc::Rc<str> = (&sp).into();
        assert_eq!(&*first, raw.as_str());
        assert_eq!(&*second, raw.as_str());
        assert_eq!(&*third, raw.as_str());
        assert_eq!(sp.as_str(), raw);
    ...
    /// [`From<&StorePath> for std::sync::Arc<[u8]>`] agrees byte-for-byte
    /// with the by-value [`From<StorePath> for std::sync::Arc<[u8]>`] peer
    /// AND with every existing byte-surface projection: the
    /// [`AsRef<[u8]>`] borrowed view, `as_str().as_bytes()`, and the
    /// by-reference [`Vec::<u8>::from`] emit peer this by-ref
    /// [`Arc<[u8]>`] impl chains through. Also asserts `From` vs `Into`
    /// consistency — reaching the impl through the trait's dual grammar
    /// projects the same bytes. Exercised across representative payload
    /// shapes (single-char name, hyphen-rich name, `.drv` derivation,
    /// deep-hyphen name) so the invariance holds at every parse-admitted
    /// canonical shape.
    #[test]
    fn test_from_store_path_ref_arc_bytes_matches_by_value_peer() {
        for raw in [
            format!("/nix/store/{H}-x"),
            format!("/nix/store/{H}-hello-2.10"),
            format!("/nix/store/{H}-mysvc.drv"),
            ...
            let sp = StorePath::parse(&raw).expect("valid store path");
            let by_ref_emit: std::sync::Arc<[u8]> = std::sync::Arc::<[u8]>::from(&sp);
            let by_ref_into: std::sync::Arc<[u8]> = (&sp).into();
            let by_val_emit: std::sync::Arc<[u8]> = std::sync::Arc::<[u8]>::from(sp.clone());
            let via_as_ref: std::sync::Arc<[u8]> =
                std::sync::Arc::<[u8]>::from(<StorePath as AsRef<[u8]>>::as_ref(&sp).to_vec());
            let via_as_str_bytes: std::sync::Arc<[u8]> =
                std::sync::Arc::<[u8]>::from(sp.as_str().as_bytes().to_vec());
            let via_ref_vec: std::sync::Arc<[u8]> =
                std::sync::Arc::<[u8]>::from(Vec::<u8>::from(&sp));
            assert_eq!(
                by_ref_emit, by_val_emit,
                "From<&StorePath> for Arc<[u8]> must match From<StorePath> for Arc<[u8]> byte-for-byte"
            );
            assert_eq!(by_ref_emit, by_ref_into, "From vs Into consistency");
            assert_eq!(
                by_ref_emit, via_as_ref,
                "must match Arc::<[u8]>::from(<StorePath as AsRef<[u8]>>::as_ref(&sp).to_vec())"
            );
            assert_eq!(
                by_ref_emit, via_as_str_bytes,
                "must match Arc::<[u8]>::from(sp.as_str().as_bytes().to_vec())"
            );
            assert_eq!(
                by_ref_emit, via_ref_vec,
                "must match Arc::<[u8]>::from(Vec::<u8>::from(&sp))"
            );
            assert_eq!(
                &*by_ref_emit,
                raw.as_bytes(),
            ...
            // Cross-thread shared-owned discipline: Arc<[u8]>::len equals
            // the payload byte count (no capacity slack), and cheap-clone
            // sharing through Arc::clone must observe the same bytes.
            assert_eq!(by_ref_emit.len(), raw.len());
            let cloned = std::sync::Arc::clone(&by_ref_emit);
            assert_eq!(&*cloned, raw.as_bytes());
            assert!(
                std::sync::Arc::ptr_eq(&by_ref_emit, &cloned),
                "Arc::clone must share allocation, not deep-copy the byte payload"
            );
        }
    }

    /// [`From<&StorePath> for std::sync::Arc<[u8]>`] carries
    /// [`StorePath::parse`]'s trim discipline through the by-reference
    /// cross-thread shared-owned-byte-slice emit surface — a value parsed
    /// from a newline-terminated nix-frontier stdout buffer emits an
    /// [`Arc<[u8]>`] whose bytes are the trimmed canonical form with no
    /// per-site
    /// `std::sync::Arc::<[u8]>::from(sp.as_str().trim().as_bytes().to_vec())`
    /// restatement.
    #[test]
    fn test_from_store_path_ref_arc_bytes_carries_trim_discipline() {
        let sp = StorePath::parse(&format!("/nix/store/{H}-x\n")).expect("valid");
        let emitted: std::sync::Arc<[u8]> = std::sync::Arc::<[u8]>::from(&sp);
        let expected = format!("/nix/store/{H}-x").into_bytes();
        assert_eq!(&*emitted, expected.as_slice());
        assert!(!emitted.ends_with(b"\n"));
        assert!(!emitted.starts_with(b" "));
        // Payload matches the trimmed canonical bytes exactly.
        assert_eq!(emitted.len(), sp.as_str().len());
    }

    /// The [`From<&StorePath> for std::sync::Arc<[u8]>`] impl composes with
    /// a generic cross-thread shared-owned-byte-slice helper bounded by
    /// `impl Into<std::sync::Arc<[u8]>>` receiving a [`&StorePath`] — the
    /// compositional motivation for landing the by-reference peer
    /// separately from the by-value
    /// [`From<StorePath> for std::sync::Arc<[u8]>`] emit peer. Pins the
    /// trait-generic consumer surface: a downstream site holding a
    /// [`&StorePath`] (an iteration over a `[StorePath]` slice, an
    /// attestation record builder borrowing its inputs, a
    /// `Send + Sync`-fan-out byte-payload sink `&self`-receiver method)
    /// that types its input contract as
    /// `impl Into<std::sync::Arc<[u8]>>` recovers the same validated bytes
    /// a direct
    /// `std::sync::Arc::<[u8]>::from(<StorePath as AsRef<[u8]>>::as_ref(&sp).to_vec())`
    /// call would.
    #[test]
    fn test_from_store_path_ref_arc_bytes_carries_through_generic_consumer() {
        fn first_byte_of<T: Into<std::sync::Arc<[u8]>>>(t: T) -> u8 {
            let s: std::sync::Arc<[u8]> = t.into();
            s[0]
        }
        fn length_of<T: Into<std::sync::Arc<[u8]>>>(t: T) -> usize {
            let s: std::sync::Arc<[u8]> = t.into();
            s.len()
        }
        fn bytes_eq<T: Into<std::sync::Arc<[u8]>>>(t: T, expected: &[u8]) -> bool {
            let s: std::sync::Arc<[u8]> = t.into();
            &*s == expected
        }
        let raw = format!("/nix/store/{H}-hello-2.10");
    ...
    /// [`From<&StorePath> for std::sync::Arc<[u8]>`] does NOT consume the
    /// source [`StorePath`] — the borrowed emit surface must leave the
    /// value available for further reads. Pins the load-bearing distinction
    /// from the by-value [`From<StorePath> for std::sync::Arc<[u8]>`]
    /// peer: an iteration over a `[StorePath]` slice or a
    /// `&StorePath`-receiver method that emits an owned [`Arc<[u8]>`] must
    /// retain the source for downstream consumers on the same expression,
    /// which the by-value peer's move discipline forbids.
    #[test]
    fn test_from_store_path_ref_arc_bytes_preserves_source() {
        let raw = format!("/nix/store/{H}-hello-2.10");
        let sp = StorePath::parse(&raw).expect("valid");
        let first: std::sync::Arc<[u8]> = std::sync::Arc::<[u8]>::from(&sp);
        let second: std::sync::Arc<[u8]> = std::sync::Arc::<[u8]>::from(&sp);
        let third: std::sync::Arc<[u8]> = (&sp).into();
        let expected = raw.as_bytes();
        assert_eq!(&*first, expected);
        assert_eq!(&*second, expected);
        assert_eq!(&*third, expected);
    ...
    /// [`From<&StorePath> for std::rc::Rc<[u8]>`] emits the same canonical
    /// bytes as the by-value [`From<StorePath> for std::rc::Rc<[u8]>`]
    /// peer (line 4991) — the borrowed-receiver surface reads the same
    /// [`AsRef<[u8]>`] one-oracle the by-value peer's
    /// [`StorePath::into_string`]/[`String::into_bytes`] move chain
    /// terminates on, so both peers project byte-identical [`Rc<[u8]>`]
    /// payloads. Pinned across a representative parse-admitted shape set,
    /// AND with every existing byte-surface projection: the
    /// [`AsRef<[u8]>`] borrowed view, `as_str().as_bytes()`, and the
    /// by-reference [`Vec::<u8>::from`] emit peer this by-ref
    /// [`Rc<[u8]>`] impl chains through. Also asserts `From` vs `Into`
    /// consistency — reaching the impl through the trait's dual grammar
    /// projects the same bytes. Exercised across representative payload
    /// shapes (single-char name, hyphen-rich name, `.drv` derivation,
    /// deep-hyphen name) so the invariance holds at every parse-admitted
    /// canonical shape.
    #[test]
    fn test_from_store_path_ref_rc_bytes_matches_by_value_peer() {
        for raw in [
            format!("/nix/store/{H}-x"),
            format!("/nix/store/{H}-hello-2.10"),
            format!("/nix/store/{H}-mysvc.drv"),
            ...
            let sp = StorePath::parse(&raw).expect("valid store path");
            let by_ref_emit: std::rc::Rc<[u8]> = std::rc::Rc::<[u8]>::from(&sp);
            let by_ref_into: std::rc::Rc<[u8]> = (&sp).into();
            let by_val_emit: std::rc::Rc<[u8]> = std::rc::Rc::<[u8]>::from(sp.clone());
            let via_as_ref: std::rc::Rc<[u8]> =
                std::rc::Rc::<[u8]>::from(<StorePath as AsRef<[u8]>>::as_ref(&sp).to_vec());
            let via_as_str_bytes: std::rc::Rc<[u8]> =
                std::rc::Rc::<[u8]>::from(sp.as_str().as_bytes().to_vec());
            let via_ref_vec: std::rc::Rc<[u8]> = std::rc::Rc::<[u8]>::from(Vec::<u8>::from(&sp));
            assert_eq!(
                by_ref_emit, by_val_emit,
                "From<&StorePath> for Rc<[u8]> must match From<StorePath> for Rc<[u8]> byte-for-byte"
            );
            assert_eq!(by_ref_emit, by_ref_into, "From vs Into consistency");
            assert_eq!(
                by_ref_emit, via_as_ref,
                "must match Rc::<[u8]>::from(<StorePath as AsRef<[u8]>>::as_ref(&sp).to_vec())"
            );
            assert_eq!(
                by_ref_emit, via_as_str_bytes,
                "must match Rc::<[u8]>::from(sp.as_str().as_bytes().to_vec())"
            );
            assert_eq!(
                by_ref_emit, via_ref_vec,
                "must match Rc::<[u8]>::from(Vec::<u8>::from(&sp))"
            );
            assert_eq!(
                &*by_ref_emit,
                raw.as_bytes(),
            ...
            // Thread-local shared-owned discipline: Rc<[u8]>::len equals
            // the payload byte count (no capacity slack), and cheap-clone
            // sharing through Rc::clone must observe the same bytes.
            assert_eq!(by_ref_emit.len(), raw.len());
            let cloned = std::rc::Rc::clone(&by_ref_emit);
            assert_eq!(&*cloned, raw.as_bytes());
            assert!(
                std::rc::Rc::ptr_eq(&by_ref_emit, &cloned),
                "Rc::clone must share allocation, not deep-copy the byte payload"
            );
        }
    }

    /// [`From<&StorePath> for std::rc::Rc<[u8]>`] carries
    /// [`StorePath::parse`]'s trim discipline through the by-reference
    /// thread-local shared-owned-byte-slice emit surface — a value parsed
    /// from a newline-terminated nix-frontier stdout buffer emits an
    /// [`Rc<[u8]>`] whose bytes are the trimmed canonical form with no
    /// per-site
    /// `std::rc::Rc::<[u8]>::from(sp.as_str().trim().as_bytes().to_vec())`
    /// restatement.
    #[test]
    fn test_from_store_path_ref_rc_bytes_carries_trim_discipline() {
        let sp = StorePath::parse(&format!("/nix/store/{H}-x\n")).expect("valid");
        let emitted: std::rc::Rc<[u8]> = std::rc::Rc::<[u8]>::from(&sp);
        let expected = format!("/nix/store/{H}-x").into_bytes();
        assert_eq!(&*emitted, expected.as_slice());
        assert!(!emitted.ends_with(b"\n"));
        assert!(!emitted.starts_with(b" "));
    ...
    /// The [`From<&StorePath> for std::rc::Rc<[u8]>`] impl composes with
    /// a generic thread-local shared-owned-byte-slice helper bounded by
    /// `impl Into<std::rc::Rc<[u8]>>` receiving a [`&StorePath`] — the
    /// compositional motivation for landing the by-reference peer
    /// separately from the by-value
    /// [`From<StorePath> for std::rc::Rc<[u8]>`] emit peer. Pins the
    /// trait-generic consumer surface: a downstream site holding a
    /// [`&StorePath`] (an iteration over a `[StorePath]` slice, an
    /// attestation record builder borrowing its inputs, an on-thread
    /// fan-out byte-payload sink `&self`-receiver method) that types its
    /// input contract as `impl Into<std::rc::Rc<[u8]>>` recovers the
    /// same validated bytes a direct
    /// `std::rc::Rc::<[u8]>::from(<StorePath as AsRef<[u8]>>::as_ref(&sp).to_vec())`
    /// call would.
    #[test]
    fn test_from_store_path_ref_rc_bytes_carries_through_generic_consumer() {
        fn first_byte_of<T: Into<std::rc::Rc<[u8]>>>(t: T) -> u8 {
            let s: std::rc::Rc<[u8]> = t.into();
            s[0]
        }
        fn length_of<T: Into<std::rc::Rc<[u8]>>>(t: T) -> usize {
            let s: std::rc::Rc<[u8]> = t.into();
            s.len()
        }
        fn bytes_eq<T: Into<std::rc::Rc<[u8]>>>(t: T, expected: &[u8]) -> bool {
            let s: std::rc::Rc<[u8]> = t.into();
            &*s == expected
        }
        let raw = format!("/nix/store/{H}-hello-2.10");
    ...
    /// [`From<&StorePath> for std::rc::Rc<[u8]>`] does NOT consume the
    /// source [`StorePath`] — the borrowed emit surface must leave the
    /// value available for further reads. Pins the load-bearing distinction
    /// from the by-value [`From<StorePath> for std::rc::Rc<[u8]>`]
    /// peer: an iteration over a `[StorePath]` slice or a
    /// `&StorePath`-receiver method that emits an owned [`Rc<[u8]>`] must
    /// retain the source for downstream consumers on the same expression,
    /// which the by-value peer's move discipline forbids.
    #[test]
    fn test_from_store_path_ref_rc_bytes_preserves_source() {
        let raw = format!("/nix/store/{H}-hello-2.10");
        let sp = StorePath::parse(&raw).expect("valid");
        let first: std::rc::Rc<[u8]> = std::rc::Rc::<[u8]>::from(&sp);
        let second: std::rc::Rc<[u8]> = std::rc::Rc::<[u8]>::from(&sp);
        let third: std::rc::Rc<[u8]> = (&sp).into();
        let expected = raw.as_bytes();
        assert_eq!(&*first, expected);
        assert_eq!(&*second, expected);
        assert_eq!(&*third, expected);
    ...
    /// [`From<&StorePath> for std::borrow::Cow<'static, [u8]>`] projects
    /// the same canonical bytes the by-value
    /// [`From<StorePath> for std::borrow::Cow<'static, [u8]>`] peer emits
    /// and every by-reference byte-side projection surfaces — pins the
    /// "by-reference emit peer routes through the same one-oracle backing
    /// every projection reads from" invariant across every canonical
    /// store-path shape. Owned-arm discipline: a by-reference emit that
    /// allocated a fresh [`Vec<u8>`] via the by-reference [`Vec<u8>`]
    /// peer cannot land in the [`std::borrow::Cow::Borrowed`] arm without
    /// fabricating a `'static` reference from a runtime buffer, so the
    /// [`std::borrow::Cow::is_owned`] gate must hold at every shape.
    #[test]
    fn test_from_store_path_ref_cow_bytes_matches_by_value_peer() {
        for raw in [
            format!("/nix/store/{H}-x"),
            format!("/nix/store/{H}-hello-2.10"),
            format!("/nix/store/{H}-mysvc.drv"),
            ...
            let sp = StorePath::parse(&raw).expect("valid store path");
            let by_ref_emit: std::borrow::Cow<'static, [u8]> =
                <std::borrow::Cow<'static, [u8]> as From<&StorePath>>::from(&sp);
            let by_ref_into: std::borrow::Cow<'static, [u8]> = (&sp).into();
            let by_val_emit: std::borrow::Cow<'static, [u8]> =
                <std::borrow::Cow<'static, [u8]> as From<StorePath>>::from(sp.clone());
            let via_as_ref: std::borrow::Cow<'static, [u8]> =
                std::borrow::Cow::Owned(<StorePath as AsRef<[u8]>>::as_ref(&sp).to_vec());
            let via_as_str_bytes: std::borrow::Cow<'static, [u8]> =
                std::borrow::Cow::Owned(sp.as_str().as_bytes().to_vec());
            let via_ref_vec: std::borrow::Cow<'static, [u8]> =
                std::borrow::Cow::Owned(Vec::<u8>::from(&sp));
            let via_ref_string_bytes: std::borrow::Cow<'static, [u8]> =
                std::borrow::Cow::Owned(String::from(&sp).into_bytes());
            assert_eq!(
                by_ref_emit, by_val_emit,
                "From<&StorePath> for Cow<'static, [u8]> must match From<StorePath> for Cow<'static, [u8]> byte-for-byte"
            );
            assert_eq!(by_ref_emit, by_ref_into, "From vs Into consistency");
            assert_eq!(
                by_ref_emit, via_as_ref,
                "must match Cow::Owned(<StorePath as AsRef<[u8]>>::as_ref(&sp).to_vec())"
            );
            assert_eq!(
                by_ref_emit, via_as_str_bytes,
                "must match Cow::Owned(sp.as_str().as_bytes().to_vec())"
            );
            assert_eq!(
                by_ref_emit, via_ref_vec,
                "must match Cow::Owned(Vec::<u8>::from(&sp))"
            );
            assert_eq!(
                by_ref_emit, via_ref_string_bytes,
                "must match Cow::Owned(String::from(&sp).into_bytes())"
            );
            assert_eq!(
                by_ref_emit.as_ref(),
                raw.as_bytes(),
            ...
            assert!(
                matches!(&by_ref_emit, std::borrow::Cow::Owned(_)),
                "From<&StorePath> for Cow<'static, [u8]> must land in the Owned arm (a by-reference emit cannot forge a 'static borrow from a runtime buffer)"
            );
            // Payload-preserving discipline: the emitted Cow's deref
            // length equals the payload byte count, so a `<Cow<'_, [u8]>
            // as AsRef<[u8]>>` sink sees the canonical form byte-for-byte
            // with no capacity slack visible through the Cow surface.
            assert_eq!(by_ref_emit.as_ref().len(), raw.len());
        }
    }

    /// [`From<&StorePath> for std::borrow::Cow<'static, [u8]>`] carries
    /// [`StorePath::parse`]'s trim discipline through the by-reference
    /// borrowed-or-owned byte-slice emit surface — a value parsed from a
    /// newline-terminated nix-frontier stdout buffer emits a
    /// [`Cow<'static, [u8]>`] whose bytes are the trimmed canonical form
    /// with no per-site
    /// `Cow::Owned(sp.as_str().trim().as_bytes().to_vec())`
    /// restatement.
    #[test]
    fn test_from_store_path_ref_cow_bytes_carries_trim_discipline() {
        let sp = StorePath::parse(&format!("/nix/store/{H}-x\n")).expect("valid");
        let emitted: std::borrow::Cow<'static, [u8]> =
            <std::borrow::Cow<'static, [u8]> as From<&StorePath>>::from(&sp);
        let expected = format!("/nix/store/{H}-x").into_bytes();
        ...
        // Payload matches the trimmed canonical bytes exactly.
        assert_eq!(emitted.as_ref().len(), sp.as_str().len());
        assert!(
            matches!(&emitted, std::borrow::Cow::Owned(_)),
            "trim-carrying emit still lands in the Owned arm"
        );
    }

    /// The [`From<&StorePath> for std::borrow::Cow<'static, [u8]>`] impl
    /// composes with a generic borrowed-or-owned-byte-slice helper bounded
    /// by `impl Into<std::borrow::Cow<'static, [u8]>>` receiving a
    /// [`&StorePath`] — the compositional motivation for landing the
    /// by-reference peer separately from the by-value
    /// [`From<StorePath> for std::borrow::Cow<'static, [u8]>`] emit peer.
    /// Pins the trait-generic consumer surface: a downstream site holding
    /// a [`&StorePath`] (an iteration over a `[StorePath]` slice, an
    /// attestation record builder borrowing its inputs, a wire-frame
    /// builder `&self`-receiver method that types its input contract as
    /// `impl Into<Cow<'static, [u8]>>`, a `#[serde(into = "Cow<'static,
    /// [u8]>")]`-decorated field emitter) recovers the same validated
    /// bytes a direct
    /// `Cow::Owned(<StorePath as AsRef<[u8]>>::as_ref(&sp).to_vec())`
    /// call would.
    #[test]
    fn test_from_store_path_ref_cow_bytes_carries_through_generic_consumer() {
        fn len_of<T: Into<std::borrow::Cow<'static, [u8]>>>(t: T) -> usize {
            let s: std::borrow::Cow<'static, [u8]> = t.into();
            s.len()
        }
        ...
        let raw = format!("/nix/store/{H}-hello-2.10");
        let sp = StorePath::parse(&raw).expect("valid");
        assert_eq!(len_of(&sp), raw.len());
        assert_eq!(to_utf8(&sp), raw);
        assert!(owned_eq(&sp, raw.as_bytes()));
    }

    /// [`From<&StorePath> for std::borrow::Cow<'static, [u8]>`] does NOT
    /// consume the source [`StorePath`] — the borrowed emit surface must
    /// leave the value available for further reads. Pins the load-bearing
    /// distinction from the by-value
    /// [`From<StorePath> for std::borrow::Cow<'static, [u8]>`] peer: an
    /// iteration over a `[StorePath]` slice or a `&StorePath`-receiver
    /// method that emits an owned [`Cow<'static, [u8]>`] must retain the
    /// source for downstream consumers on the same expression, which the
    /// by-value peer's move discipline forbids.
    #[test]
    fn test_from_store_path_ref_cow_bytes_preserves_source() {
        let raw = format!("/nix/store/{H}-hello-2.10");
        let sp = StorePath::parse(&raw).expect("valid");
        let first: std::borrow::Cow<'static, [u8]> =
            <std::borrow::Cow<'static, [u8]> as From<&StorePath>>::from(&sp);
        let second: std::borrow::Cow<'static, [u8]> =
            <std::borrow::Cow<'static, [u8]> as From<&StorePath>>::from(&sp);
        let third: std::borrow::Cow<'static, [u8]> = (&sp).into();
        let expected = raw.as_bytes();
        assert_eq!(first.as_ref(), expected);
        assert_eq!(second.as_ref(), expected);
        assert_eq!(third.as_ref(), expected);
        assert_eq!(sp.as_str(), raw);
        assert_eq!(sp.hash(), &raw["/nix/store/".len()..][..32]);
    }
}
