...
/// [`TryFrom<Box<str>>`] for [`DigestAlgorithm`] routes through
/// [`String::from`] (which unboxes the `Box<str>` at zero-copy into the
/// same heap allocation as the returned [`String`]'s backing storage,
/// per the standard library's length-only conversion contract) then
/// delegates to [`<DigestAlgorithm as TryFrom<String>>::try_from`] so a
/// downstream consumer holding a shrunk-owned UTF-8 label buffer at a
/// two-word boxed-slice frontier (a serde container that opts into
/// `#[serde(try_from = "Box<str>")]` on a wrapper field to consume a
/// shrunk-owned canonical label without a per-consumer allocation, a
/// `Cow::Owned` arm that shed its growth-header capacity through
/// [`String::into_boxed_str`] before landing on a downstream parse
/// gate, a validated-builder frontier whose canonical input contract is
/// pinned as [`Box<str>`] to shed the growth-header word off the intake
/// before the parse pass, a `Vec<Box<str>>` per-attempt algorithm-label
/// list flowed into a parse worker without cloning per element)
/// recovers a [`DigestAlgorithm`] variant from its canonical lowercase
/// label through the same one-oracle grammar the direct
/// `.parse::<DigestAlgorithm>()` string call sites and the sibling
/// [`TryFrom<String>`] peer already read — WITHOUT a per-consumer
/// `String::from(boxed).try_into()` bridge at every downstream site,
/// WITHOUT the by-reference bridge (`DigestAlgorithm::try_from(boxed.
/// as_ref())`) that would leak the owned [`Box<str>`] allocation past
/// the parse gate for the delegated string oracle to ignore.
///
/// Zero-copy on the happy path: [`String::from(Box<str>)`] takes
/// ownership of the boxed slice's heap allocation and reuses it as the
/// returned [`String`]'s backing storage (the standard library
/// documents this as a length-only conversion with no re-allocation).
/// The resulting owned [`String`] then flows into the by-value string
/// parse peer [`TryFrom<String> for DigestAlgorithm`] which itself
...
/// [`TryFrom<&[u8]> for DigestAlgorithm`] (commit a403501), and
/// [`TryFrom<Vec<u8>> for DigestAlgorithm`] (commit 0689fde) on the
...
/// input frontiers so a downstream site that receives its input at
/// any of those frontiers routes through the same
/// [`DigestAlgorithm::parse`] canonical-label oracle without a
/// per-consumer bridge — the pattern this impl absorbs at one site so
/// no downstream site restates it. Structural mirror of the sibling
/// [`TryFrom<Box<str>> for ContentDigest`] impl above (commit 2d5eb7e)
/// on the composite digest reference-grammar type: the digest
/// reference-grammar family carries the complete shrunk / shared UTF-8
/// parse-peer surface ([`TryFrom<Box<str>>`], [`TryFrom<Arc<str>>`],
/// [`TryFrom<Rc<str>>`]), and the algorithm-axis typed sum this impl
/// widens is the underlying label oracle that reference-grammar
/// surface routes through — the two typed primitives grow in
/// lockstep so a consumer that types both the label axis and the
/// composite digest at their shrunk-owned frontiers reads the same
/// one-oracle grammar at both layers.
///
...
/// message drift across the shrunk-owned receiver frontier. The
/// UTF-8-invalid failure mode the byte-frontier peers
/// ([`TryFrom<&[u8]>`], [`TryFrom<Vec<u8>>`]) enforce is unreachable
/// here by construction: a [`Box<str>`] carries a UTF-8-validated
/// backing slice at the type level, so the [`String::from_utf8`] gate
/// is not run on this peer.
///
/// The identity
/// `DigestAlgorithm::try_from(Box::<str>::from(algo.as_str())).unwrap()
/// == algo` at every [`DigestAlgorithm::ALL`] variant is pinned by
/// [`tests::test_digest_algorithm_try_from_box_str_agrees_with_from_str`];
/// the identity carried through a generic `impl TryFrom<Box<str>>`
/// consumer at every variant is pinned by
/// [`tests::test_digest_algorithm_try_from_box_str_carries_through_generic_consumer`];
/// the strict-rejection contract on non-canonical input is pinned by
/// [`tests::test_digest_algorithm_try_from_box_str_rejects_non_canonical_input`].
///
/// THEORY.md §III.1 typescape: the by-value shrunk-owned UTF-8
/// try-conversion surface is a typed-primitive site on
/// [`DigestAlgorithm`] itself (one [`TryFrom<Box<str>>`] impl routing
/// through [`String::from`] then [`TryFrom<String>`]), not a
/// per-consumer `String::from(boxed).try_into()` bridge at every
/// downstream site that owns a shrunk UTF-8 label buffer. THEORY.md
/// §VI.1 generation over composition: the canonical-label grammar is
/// named at one site ([`DigestAlgorithm::as_str`]), inverted at one
/// site ([`DigestAlgorithm::parse`]), and every parse surface —
/// [`std::str::FromStr`], [`TryFrom<&str>`], [`TryFrom<String>`],
/// [`TryFrom<Cow<'_, str>>`], [`TryFrom<&[u8]>`], [`TryFrom<Vec<u8>>`],
/// this [`TryFrom<Box<str>>`], the [`ContentDigest::parse`] grammar
/// oracle's algorithm arm — reads through it.
impl TryFrom<Box<str>> for DigestAlgorithm {
    type Error = anyhow::Error;
...
/// [`TryFrom<Rc<str>>`] for [`DigestAlgorithm`] routes through
...
/// [`<DigestAlgorithm as TryFrom<&str>>::try_from`] so a downstream consumer
/// holding a single-thread shared-owned UTF-8 label buffer at a
/// refcount-headered frontier (a serde container that opts into
/// `#[serde(try_from = "Rc<str>")]` on a wrapper field to consume a
/// thread-local shared-owned canonical label without a per-consumer
/// allocation, a validated-input newtype builder whose canonical parse
/// contract accepts a caller-supplied [`Rc<str>`] label slot at the
/// shared-owned frontier for single-thread cheap-clone semantics on the
/// input, a single-thread `HashMap<Rc<str>, _>` registry cache whose
/// algorithm-label key slot arrives as a shared-owned label from an
/// upstream table build refcounted across peer inspectors within one
/// thread, a `!Send` per-task lookaside whose `Vec<Rc<str>>` per-attempt
/// algorithm-label list is handed cheaply through [`Rc::clone`] to each
/// parse worker within one thread) recovers a [`DigestAlgorithm`] variant
/// from its canonical lowercase label through the same one-oracle grammar
/// the direct `.parse::<DigestAlgorithm>()` string call sites and the
/// sibling [`TryFrom<Box<str>>`] / [`TryFrom<Arc<str>>`] peers already read
/// — WITHOUT a per-consumer `shared.parse::<DigestAlgorithm>()`
...
/// a borrowed `&str` view of the shared allocation's UTF-8 payload without
/// allocating and without incrementing or decrementing the
/// non-atomic-refcount header preceding the label bytes, so the parse-side
/// receiver pays the by-reference [`DigestAlgorithm::parse`] cost only,
/// not the integer-op cost of an
...
/// paying [`Arc`]'s atomic-refcount header on every clone of a label that
/// never crosses a thread boundary — routes through this [`Rc<str>`] peer
/// instead and keeps the non-atomic-refcount cost on the input side by
/// construction. The single-thread parse-side cost of a `Vec<Rc<str>>`
/// algorithm-label list drained one-by-one through this impl collapses to
/// `n * (Rc::deref + DigestAlgorithm::parse)`, matching the [`Arc<str>`]
...
/// [`TryFrom<Box<str>> for DigestAlgorithm`] (commit 32659e5), and
/// [`TryFrom<Arc<str>> for DigestAlgorithm`] (commit 6d7085a) on the
/// canonical-label family — the parse surface widens across the
/// borrowed-string, owned-string, borrowed-or-owned-string,
...
/// input frontiers so a downstream site that receives its algorithm token
/// at any of those frontiers routes through the same
/// [`DigestAlgorithm::parse`] canonical-label oracle without a
/// per-consumer bridge — the pattern this impl absorbs at one site so no
/// downstream site restates it.
///
/// Closes the shrunk / cross-thread-shared / thread-local-shared UTF-8
/// parse trio at [`DigestAlgorithm`]: [`TryFrom<Box<str>>`]
/// (commit 32659e5, shrunk-owned), [`TryFrom<Arc<str>>`] (commit 6d7085a,
/// cross-thread shared-owned), this [`TryFrom<Rc<str>>`] (thread-local
/// shared-owned) — every owned-shape UTF-8 receiver frontier the sibling
/// composite digest type [`ContentDigest`] already carries at this module
/// now has a matching parse surface on the algorithm-axis typed sum. The
/// algorithm axis and the composite digest reference-grammar type grow in
/// lockstep so a consumer that types both at their thread-local
/// shared-owned frontiers reads the same one-oracle grammar at both
/// layers.
///
/// Structural mirror of
/// [`impl TryFrom<Rc<str>> for ContentDigest`] on the composite digest
/// reference-grammar type in this same module (commit 4d0783e): the
/// digest reference-grammar family already carries the complete
/// shrunk / cross-thread-shared / thread-local-shared UTF-8 parse-peer
/// surface ([`TryFrom<Box<str>>`], [`TryFrom<Arc<str>>`],
/// [`TryFrom<Rc<str>>`]), and the algorithm-axis typed sum this impl
/// widens is the underlying label oracle the reference-grammar surface
/// routes through — this impl closes the thread-local shared-owned
/// string leg on the algorithm-axis typed sum so the two typed primitives
/// carry the same shape at their shared-owned frontier.
///
/// The error type is [`anyhow::Error`] — the exact shape the
/// [`FromStr`] impl and every sibling by-reference and by-value parse
/// peer carry. Rejection wording is inherited from [`FromStr`]: an
/// unknown label surfaces an [`anyhow::Error`] naming the offending
/// input and the canonical three-label set — no per-peer rejection-
/// message drift across the thread-local shared-owned receiver frontier.
/// The UTF-8-invalid failure mode the byte-frontier peers
/// ([`TryFrom<&[u8]>`], [`TryFrom<Vec<u8>>`]) enforce is unreachable
/// here by construction: an [`std::rc::Rc<str>`] carries a
/// UTF-8-validated backing slice at the type level, so the
/// [`String::from_utf8`] gate is not run on this peer.
///
/// The identity
/// `DigestAlgorithm::try_from(Rc::<str>::from(algo.as_str())).unwrap()
/// == algo` at every [`DigestAlgorithm::ALL`] variant is pinned by
/// [`tests::test_digest_algorithm_try_from_rc_str_agrees_with_from_str`];
/// the identity carried through a generic `impl TryFrom<Rc<str>>`
/// consumer at every variant is pinned by
/// [`tests::test_digest_algorithm_try_from_rc_str_carries_through_generic_consumer`];
/// the strict-rejection contract on non-canonical input is pinned by
/// [`tests::test_digest_algorithm_try_from_rc_str_rejects_non_canonical_input`].
///
/// THEORY.md §III.1 typescape: the by-value thread-local shared-owned
/// UTF-8 try-conversion surface is a typed-primitive site on
/// [`DigestAlgorithm`] itself (one [`TryFrom<Rc<str>>`] impl routing
/// through [`AsRef::as_ref`] then [`TryFrom<&str>`]), not a per-consumer
/// `shared.parse::<DigestAlgorithm>()` restatement at every downstream
/// site that owns a thread-local shared UTF-8 label buffer. THEORY.md
/// §VI.1 one-oracle: the canonical-label grammar is named at one site
/// ([`DigestAlgorithm::as_str`]), inverted at one site
/// ([`DigestAlgorithm::parse`]), and every parse surface —
/// [`std::str::FromStr`], [`TryFrom<&str>`], [`TryFrom<String>`],
...
/// the [`ContentDigest::parse`] grammar oracle's algorithm arm — reads
/// through it.
impl TryFrom<std::rc::Rc<str>> for DigestAlgorithm {
    type Error = anyhow::Error;
...
/// [`TryFrom<Arc<[u8]>>`] for [`DigestAlgorithm`] routes through
/// [`<std::sync::Arc<[u8]> as AsRef<[u8]>>::as_ref`] (a zero-copy borrow of
/// the shared allocation's raw-byte payload that does NOT touch the
/// atomic-refcount header) then delegates to
/// [`<DigestAlgorithm as TryFrom<&[u8]>>::try_from`] so a downstream
/// consumer holding a cross-thread shared-owned raw-byte label buffer at a
/// refcount-headered byte-slice frontier (a serde container that opts into
/// `#[serde(try_from = "Arc<[u8]>")]` on a byte-frontier wrapper field to
/// consume a cross-thread shared-owned raw-byte label without a per-consumer
/// allocation, a validated-input newtype builder whose parse contract
/// accepts a caller-supplied [`Arc<[u8]>`] label slot at the shared-owned
/// raw-byte frontier for cross-thread cheap-clone semantics on the input,
/// a `bytes::Bytes::from(Arc<[u8]>)` intake path whose upstream table
/// build refcounted the raw-byte record across worker threads before
/// landing on the parse pass, a `Vec<Arc<[u8]>>` per-attempt algorithm-label
/// byte-list handed cheaply through [`Arc::clone`] to each parse worker)
/// recovers a [`DigestAlgorithm`] variant from its canonical lowercase-ASCII
/// cross-thread shared-owned raw-byte serialization through the same
/// one-oracle grammar the `.parse::<DigestAlgorithm>()` string call sites
/// and the sibling [`TryFrom<&[u8]>`] / [`TryFrom<Vec<u8>>`] /
/// [`TryFrom<Box<[u8]>>`] peers already read — WITHOUT a per-consumer
/// `shared.as_ref().try_into()` restatement at every downstream site,
/// WITHOUT an [`std::sync::Arc::try_unwrap`]-fallback-clone-then-parse
/// cascade that would negotiate the refcount at every intake, and WITHOUT
/// an `Arc::<[u8]>::to_vec()`-then-parse round trip that would allocate a
/// fresh owned [`Vec<u8>`] off the shared borrow for each parse pass.
///
/// Zero-touch on the atomic refcount during the parse: the receiver-side
/// [`<std::sync::Arc<[u8]> as AsRef<[u8]>>::as_ref`] call yields a borrowed
...
/// refcount header preceding the raw bytes, so the parse-side receiver
/// pays the by-reference [`TryFrom<&[u8]>`] cost only (a
/// [`std::str::from_utf8`] UTF-8 gate then the [`DigestAlgorithm::parse`]
/// string oracle), not the atomic-op cost of an
/// [`std::sync::Arc::try_unwrap`]-fallback-clone-then-parse composition
/// nor the allocation cost of an `Arc::<[u8]>::to_vec()`-then-parse round
/// trip. The [`std::sync::Arc<[u8]>`] input is dropped at end of scope,
/// releasing the shared allocation exactly when the last outstanding
/// [`Arc::clone`] refcount hits zero — the standard cross-thread
/// shared-owned drop semantics carry through unchanged.
///
/// A cross-thread caller that would otherwise widen its parse frontier to
/// [`Vec<u8>`] purely to satisfy an owned-byte parse peer's type signature
/// — paying a per-worker deep-copy [`Vec::from(slice)`] on every fan-out
/// of a raw-byte label shared across a worker pool — routes through this
/// [`Arc<[u8]>`] peer instead and keeps the cheap-clone atomic-refcount
/// cost on the input side by construction. The cross-thread parse-side
/// cost of a `Vec<Arc<[u8]>>` raw-byte label list drained one-by-one
/// through this impl collapses to
/// `n * (Arc::deref + std::str::from_utf8 + DigestAlgorithm::parse)`,
/// matching the [`Vec<u8>`] peer's parse cost minus the per-worker
/// deep-copy allocation the [`Arc<[u8]>`] frontier avoids by construction.
///
...
/// [`TryFrom<Vec<u8>> for DigestAlgorithm`] (commit 0689fde),
/// [`TryFrom<Box<str>> for DigestAlgorithm`] (commit 32659e5),
/// [`TryFrom<Arc<str>> for DigestAlgorithm`] (commit 6d7085a),
/// [`TryFrom<Rc<str>> for DigestAlgorithm`] (commit 8b8742d), and
/// [`TryFrom<Box<[u8]>> for DigestAlgorithm`] (commit a37ed68) on the
/// canonical-label family — the parse surface widens across the
/// borrowed-string, owned-string, borrowed-or-owned-string,
/// borrowed-byte-slice, owned-byte-slice, shrunk-owned-string,
/// cross-thread shared-owned string, thread-local shared-owned string,
/// shrunk-owned-byte-slice, AND cross-thread shared-owned-byte-slice
/// input frontiers so a downstream site that receives its input at any
/// of those frontiers routes through the same [`DigestAlgorithm::parse`]
/// canonical-label oracle without a per-consumer bridge — the pattern
/// this impl absorbs at one site so no downstream site restates it.
/// Structural mirror of the sibling
/// [`TryFrom<Arc<[u8]>> for ContentDigest`] impl below (commit d2ccc5d)
/// on the composite digest reference-grammar type: the digest
/// reference-grammar family already carries the complete shrunk /
/// cross-thread-shared / thread-local-shared byte-slice parse-peer
/// surface ([`TryFrom<Box<[u8]>>`], [`TryFrom<Arc<[u8]>>`],
/// [`TryFrom<Rc<[u8]>>`]); the algorithm-axis typed sum this impl widens
/// is the underlying label oracle that reference-grammar surface routes
/// through — the two typed primitives grow in lockstep on the middle
/// (cross-thread shared-owned) leg so a consumer that types both the
/// label axis and the composite digest at their cross-thread shared-owned
/// byte-slice frontiers reads the same one-oracle grammar at both layers.
///
/// Mid-trio in the owned-shape byte-slice family the shrunk-owned peer
/// (commit a37ed68) named as the next natural step: [`TryFrom<Box<[u8]>>`]
/// (commit a37ed68, shrunk-owned) opened the trio; this
/// [`TryFrom<Arc<[u8]>>`] closes the cross-thread shared-owned middle;
/// the thread-local shared-owned [`TryFrom<Rc<[u8]>>`] peer remains open
/// behind this one, mirroring the UTF-8-string trio (commits 32659e5,
/// 6d7085a, 8b8742d) already closed on the sibling axis.
///
/// The error type is [`anyhow::Error`] — the exact shape the [`FromStr`]
/// impl and every sibling by-reference and by-value parse peer carry.
/// Two rejection modes carry through unchanged from the underlying
/// [`TryFrom<&[u8]>`] peer: a UTF-8-invalid byte input (a stray non-UTF-8
/// sequence in a cross-thread shared-owned wire capture, a partial-write
/// byte tail that clips a UTF-8 continuation and later landed in an
/// [`Arc<[u8]>`] slot handed across threads) surfaces an [`anyhow::Error`]
/// naming the offending bytes' lossy-decoded rendering so a caller can
/// still attach the offending input to a failure record; a UTF-8-valid
/// but non-canonical label (uppercase, hyphenated, unknown, empty,
/// edge-whitespace) surfaces the same [`anyhow::Error`] the underlying
/// [`FromStr`] impl emits — no per-peer rejection-message drift across
/// the cross-thread shared-owned byte-slice receiver frontier.
///
/// The identity
/// `DigestAlgorithm::try_from(Arc::<[u8]>::from(algo.as_str().as_bytes())).unwrap()
/// == algo` at every [`DigestAlgorithm::ALL`] variant is pinned by
/// [`tests::test_digest_algorithm_try_from_arc_bytes_agrees_with_from_str`];
/// the identity carried through a generic `impl TryFrom<Arc<[u8]>>`
/// consumer at every variant is pinned by
/// [`tests::test_digest_algorithm_try_from_arc_bytes_carries_through_generic_consumer`];
/// the strict-rejection contract on non-canonical UTF-8-valid input is
/// pinned by
/// [`tests::test_digest_algorithm_try_from_arc_bytes_rejects_non_canonical_input`].
///
/// THEORY.md §III.1 typescape: the by-value cross-thread shared-owned
/// byte-slice try-conversion surface is a typed-primitive site on
/// [`DigestAlgorithm`] itself (one [`TryFrom<Arc<[u8]>>`] impl routing
/// through [`AsRef::as_ref`] then [`TryFrom<&[u8]>`]), not a per-consumer
/// `shared.as_ref().try_into()` restatement at every downstream site
/// that owns a cross-thread shared raw-byte label buffer.
/// THEORY.md §VI.1 generation over composition: the canonical-label
/// grammar is named at one site ([`DigestAlgorithm::as_str`]), inverted
/// at one site ([`DigestAlgorithm::parse`]), and every parse surface —
/// [`std::str::FromStr`], [`TryFrom<&str>`], [`TryFrom<String>`],
...
/// [`TryFrom<Box<[u8]>>`], this [`TryFrom<Arc<[u8]>>`], the
/// [`ContentDigest::parse`] grammar oracle's algorithm arm — reads
/// through it.
impl TryFrom<std::sync::Arc<[u8]>> for DigestAlgorithm {
    type Error = anyhow::Error;
...
/// [`TryFrom<Rc<[u8]>>`] for [`DigestAlgorithm`] routes through
/// [`<Rc<[u8]> as AsRef<[u8]>>::as_ref`] (a zero-copy borrow of the
/// shared allocation's raw-byte payload that does NOT touch the
/// non-atomic-refcount header) then delegates to
/// [`<DigestAlgorithm as TryFrom<&[u8]>>::try_from`] so a downstream
/// consumer holding a thread-local shared-owned raw-byte label buffer at
/// an [`std::rc::Rc<[u8]>`] frontier (a serde container that opts into
/// `#[serde(try_from = "Rc<[u8]>")]` on a byte-frontier wrapper field to
/// consume a thread-local shared-owned raw-byte label without a
/// per-consumer allocation, a validated-input newtype builder whose parse
/// contract accepts a caller-supplied [`Rc<[u8]>`] label slot at the
/// thread-local shared-owned raw-byte frontier for cheap-clone semantics
/// on the input inside a single-threaded arena, a `Vec<Rc<[u8]>>`
/// per-attempt algorithm-label byte-list handed cheaply through
/// [`Rc::clone`] to each parse pass inside a thread-local table build
/// pipeline) recovers a [`DigestAlgorithm`] variant from its canonical
/// lowercase-ASCII shared-owned-byte serialization through the same
/// one-oracle grammar the direct `.parse::<DigestAlgorithm>()` string
/// call sites and the sibling [`TryFrom<&[u8]>`], [`TryFrom<Vec<u8>>`],
/// [`TryFrom<Box<[u8]>>`], and [`TryFrom<Arc<[u8]>>`] byte-frontier peers
/// already read — WITHOUT a per-consumer `shared.as_ref().try_into()`
/// restatement at every downstream site, WITHOUT an [`Rc::try_unwrap`]-
/// fallback-clone-then-parse cascade that would negotiate the refcount at
/// every intake, and WITHOUT an [`Rc::<[u8]>::to_vec`]-then-parse round
/// trip that would allocate a fresh owned [`Vec<u8>`] off the shared
/// borrow for each parse pass.
///
/// Closes the shrunk / cross-thread-shared / thread-local-shared
/// byte-slice parse trio at [`DigestAlgorithm`]: [`TryFrom<Box<[u8]>>`]
/// (commit a37ed68, shrunk-owned) opened the trio,
/// [`TryFrom<Arc<[u8]>>`] (commit 1cd856a, cross-thread shared-owned)
/// held the middle, and this [`TryFrom<Rc<[u8]>>`] (thread-local
/// shared-owned) closes the third leg — mirroring the shrunk / shared
/// UTF-8-string trio (commits 32659e5, 6d7085a, 8b8742d) already closed
/// on the sibling axis. Every owned-shape receiver frontier — shrunk,
/// cross-thread shared, thread-local shared, across both the UTF-8
/// string surface and the raw-byte-slice surface — now carries a
/// matching parse peer on the algorithm-axis typed sum, so a downstream
/// consumer that receives its input at any of those six frontiers routes
/// through the same [`DigestAlgorithm::parse`] canonical-label oracle
/// without a per-consumer bridge.
///
/// Structural mirror of the sibling
/// [`TryFrom<Rc<[u8]>> for ContentDigest`] impl above (commit 0eeac6d)
/// on the composite digest reference-grammar type: the digest
/// reference-grammar family already carries the complete shrunk /
/// cross-thread-shared / thread-local-shared byte-slice parse-peer
/// surface ([`TryFrom<Box<[u8]>>`], [`TryFrom<Arc<[u8]>>`],
/// [`TryFrom<Rc<[u8]>>`]); the algorithm-axis typed sum this impl widens
/// is the underlying label oracle that reference-grammar surface routes
/// through — the two typed primitives now grow in lockstep on the
/// thread-local shared-owned leg so a consumer that types both the
/// label axis and the composite digest at their thread-local
/// shared-owned byte-slice frontiers reads the same one-oracle grammar
/// at both layers.
///
/// The error type is [`anyhow::Error`] — the exact shape the [`FromStr`]
/// impl and every sibling by-reference and by-value parse peer carry.
/// Two rejection modes carry through unchanged from the underlying
/// [`TryFrom<&[u8]>`] peer: a UTF-8-invalid byte input (a stray non-UTF-8
/// sequence in a thread-local shared-owned wire capture, a partial-write
/// byte tail that clips a UTF-8 continuation and later landed in an
/// [`Rc<[u8]>`] slot handed inside a single-threaded arena) surfaces an
/// [`anyhow::Error`] naming the offending bytes' lossy-decoded rendering
/// so a caller can still attach the offending input to a failure record;
/// a UTF-8-valid but non-canonical label (uppercase, hyphenated,
/// unknown, empty, edge-whitespace) surfaces the same [`anyhow::Error`]
/// the underlying [`FromStr`] impl emits — no per-peer rejection-message
/// drift across the thread-local shared-owned byte-slice receiver
/// frontier.
///
/// The identity
/// `DigestAlgorithm::try_from(Rc::<[u8]>::from(algo.as_str().as_bytes())).unwrap()
/// == algo` at every [`DigestAlgorithm::ALL`] variant is pinned by
/// [`tests::test_digest_algorithm_try_from_rc_bytes_agrees_with_from_str`];
/// the identity carried through a generic `impl TryFrom<Rc<[u8]>>`
/// consumer at every variant is pinned by
/// [`tests::test_digest_algorithm_try_from_rc_bytes_carries_through_generic_consumer`];
/// the strict-rejection contract on non-canonical UTF-8-valid input is
/// pinned by
/// [`tests::test_digest_algorithm_try_from_rc_bytes_rejects_non_canonical_input`].
///
/// THEORY.md §III.1 typescape: the by-value thread-local shared-owned
/// byte-slice try-conversion surface is a typed-primitive site on
/// [`DigestAlgorithm`] itself (one [`TryFrom<Rc<[u8]>>`] impl routing
/// through [`AsRef::as_ref`] then [`TryFrom<&[u8]>`]), not a per-consumer
/// `shared.as_ref().try_into()` restatement at every downstream site
/// that owns a thread-local shared raw-byte label buffer.
/// THEORY.md §VI.1 generation over composition: the canonical-label
/// grammar is named at one site ([`DigestAlgorithm::as_str`]), inverted
/// at one site ([`DigestAlgorithm::parse`]), and every parse surface —
/// [`std::str::FromStr`], [`TryFrom<&str>`], [`TryFrom<String>`],
...
/// [`TryFrom<Rc<[u8]>>`], the [`ContentDigest::parse`] grammar oracle's
/// algorithm arm — reads through it.
impl TryFrom<std::rc::Rc<[u8]>> for DigestAlgorithm {
    type Error = anyhow::Error;
...
/// [`TryFrom<Cow<'_, [u8]>>`] for [`DigestAlgorithm`] dispatches on the
/// [`Cow`] arm and routes each branch through the matching by-shape
/// byte-slice parse peer: [`Cow::Borrowed`] flows into the borrowed-byte
/// oracle [`TryFrom<&[u8]> for DigestAlgorithm`] (commit a403501) and
/// [`Cow::Owned`] flows into the owned-byte oracle
/// [`TryFrom<Vec<u8>> for DigestAlgorithm`] (commit 0689fde) — so a
/// downstream consumer holding a borrowed-or-owned byte payload at a
/// [`Cow<'_, [u8]>`] frontier (a `serde_bytes` container that opts into
/// `#[serde(try_from = "Cow<'_, [u8]>")]` on a byte-frontier field, a
/// codec sink that yields either a zero-copy borrowed sub-slice into a
/// larger buffer or an owned [`Vec<u8>`] depending on framing, a generic
/// try-conversion helper that defers the ownership decision on the
/// caller-supplied byte payload to the caller) recovers a
/// [`DigestAlgorithm`] variant from its canonical lowercase-ASCII byte
/// serialization through the same one-oracle grammar the direct
/// `.parse::<DigestAlgorithm>()` string call sites and the sibling
/// by-shape byte-frontier peers already read — WITHOUT the by-reference
/// bridge (`DigestAlgorithm::try_from(cow.as_ref())`) that leaks the
/// owned [`Cow::Owned`] allocation past the parse gate for the delegated
/// oracle to ignore.
///
/// Zero-copy on the borrowed arm: the [`Cow::Borrowed`] payload routes
/// through [`TryFrom<&[u8]>`] with no intermediate allocation.
/// Move-through on the owned arm: the [`Cow::Owned`] payload is consumed
/// and its [`Vec<u8>`] backing flowed into [`TryFrom<Vec<u8>>`] whose
/// [`String::from_utf8`] happy path reuses the same allocation as the
/// intermediate [`String`] backing storage — one allocation, consumed at
/// intake, carried through the byte-frontier UTF-8 gate and the string
/// oracle, landed on the [`Copy`]-returned typed variant.
///
/// The borrowed-or-owned-byte-slice parse peer of [`TryFrom<&[u8]>`] and
/// [`TryFrom<Vec<u8>>`] — the parse surface at the digest-algorithm axis
/// now closes the borrowed-or-owned byte-slice frontier alongside the
/// borrowed-or-owned UTF-8 frontier [`TryFrom<Cow<'_, str>>`] (commit
/// 66533a6) so a downstream site that receives its input at a
/// [`Cow<'_, [u8]>`] frontier routes through the same
/// [`DigestAlgorithm::parse`] oracle without a per-consumer
/// match-and-dispatch across the two [`Cow`] arms. Structural mirror of
/// [`TryFrom<Cow<'_, [u8]>> for ContentDigest`] (commit cc9fcb3) at the
/// parallel canonical-string typed primitive on the same module — the
/// same lift at the borrowed-or-owned-byte-slice frontier of the
/// reference-grammar family, delegating through the underlying label
/// sum's by-shape byte-slice parse peers.
///
/// The error type is [`anyhow::Error`] — the exact shape the [`FromStr`]
/// impl and every sibling by-reference and by-value parse peer carry.
/// Rejection modes on both arms track the underlying per-shape peer
/// byte-for-byte: a UTF-8-invalid byte payload surfaces an
/// [`anyhow::Error`] naming the offending bytes' lossy-decoded
/// rendering; a UTF-8-valid but non-canonical label (uppercase,
/// hyphenated, unknown, empty, edge-whitespace) surfaces the same
/// [`anyhow::Error`] the underlying [`FromStr`] impl emits — no per-arm
/// rejection-message drift across the borrowed-or-owned byte receiver
/// frontier.
///
/// The identity
/// `DigestAlgorithm::try_from(Cow::Borrowed(algo.as_str().as_bytes()))
/// .unwrap() == algo` on the borrowed arm and
/// `DigestAlgorithm::try_from(Cow::Owned(algo.as_str().as_bytes()
/// .to_vec())).unwrap() == algo` on the owned arm at every
/// [`DigestAlgorithm::ALL`] variant is pinned by
/// [`tests::test_digest_algorithm_try_from_cow_bytes_agrees_with_from_str`];
/// the identity carried through a generic
/// `impl for<'a> TryFrom<Cow<'a, [u8]>>` consumer at every variant is
/// pinned by
/// [`tests::test_digest_algorithm_try_from_cow_bytes_carries_through_generic_consumer`];
/// the strict-rejection contract on non-canonical UTF-8-valid input on
/// both arms is pinned by
/// [`tests::test_digest_algorithm_try_from_cow_bytes_rejects_non_canonical_input`];
/// the UTF-8-invalid rejection contract on both arms is pinned by
/// [`tests::test_digest_algorithm_try_from_cow_bytes_rejects_invalid_utf8`].
///
/// THEORY.md §III.1 typescape: the by-value borrowed-or-owned-frontier
/// byte-slice try-conversion surface is a typed-primitive site on
/// [`DigestAlgorithm`] itself (one [`TryFrom<Cow<'_, [u8]>>`] impl
/// dispatching to the two by-shape peers), not a per-consumer
/// `match cow { Cow::Borrowed(s) => s.try_into(), Cow::Owned(v) =>
/// v.try_into() }` bridge at every downstream site that owns a
/// borrowed-or-owned byte payload. THEORY.md §VI.1 generation over
/// composition: the canonical-label grammar is named at one site
/// ([`DigestAlgorithm::as_str`]), inverted at one site
/// ([`DigestAlgorithm::parse`]), and every parse surface —
/// [`std::str::FromStr`], [`TryFrom<&str>`], [`TryFrom<String>`],
...
/// this [`TryFrom<Cow<'_, [u8]>>`], the [`ContentDigest::parse`]
/// grammar oracle's algorithm arm — reads through it.
impl TryFrom<std::borrow::Cow<'_, [u8]>> for DigestAlgorithm {
    type Error = anyhow::Error;
...
mod tests {
    ...
    /// The [`TryFrom<Box<str>> for DigestAlgorithm`] impl round-trips
    /// every canonical label emitted by [`DigestAlgorithm::as_str`]
    /// through a shrunk-owned [`Box<str>`] receiver and agrees with
    /// [`std::str::FromStr`] AND the sibling [`TryFrom<String>`] peer
    /// byte-for-byte at every [`DigestAlgorithm::ALL`] variant. Pins the
    /// delegate-through-[`TryFrom<String>`] discipline at the shrunk-
    /// owned frontier: a regression that drifted the [`TryFrom<Box<str>>`]
    /// impl body (routing through a divergent oracle, cloning the boxed
    /// slice into a fresh [`String`] and re-parsing through a case-folded
    /// branch, admitting an empty label through a short-circuit) fails
    /// here rather than at every downstream `TryFrom<Box<str>>` call
    /// site.
    #[test]
    fn test_digest_algorithm_try_from_box_str_agrees_with_from_str() {
        for algo in DigestAlgorithm::ALL {
            let label = algo.as_str();
            ...
            assert_eq!(
                via_box, algo,
                "TryFrom<Box<str>> must round-trip through as_str at {algo:?}",
            );
            ...
            assert_eq!(
                ...
                "TryFrom<Box<str>> and FromStr must agree at {algo:?}",
            );
            ...
            assert_eq!(
                via_box, via_string,
                "TryFrom<Box<str>> and TryFrom<String> must agree at {algo:?}",
            );
        }
    }

    /// The [`TryFrom<Box<str>> for DigestAlgorithm`] identity carries
    /// through a generic `impl TryFrom<Box<str>>` consumer at every
    /// [`DigestAlgorithm::ALL`] variant. A tiny generic function
    /// `fn parse<T>(boxed: Box<str>) -> T where T: TryFrom<Box<str>>,
    /// T::Error: Debug` — the shape of an actual downstream consumer
    /// (validated-input newtype builder whose canonical intake contract
    /// is pinned as [`Box<str>`] to shed the growth-header word before
    /// the parse gate, serde `try_from = "Box<str>"` wrapper on a
    /// shrunk-owned label field, generic try-conversion helper that
    /// opts into the shrunk-owned frontier at the receiver-shape layer)
    /// — recovers the canonical variant from the shrunk-owned canonical
    /// lowercase label at every variant. The structural witness that a
    /// [`DigestAlgorithm`] is genuinely usable at
    /// `impl TryFrom<Box<str>>` call sites — a regression that drifted
    /// the [`TryFrom<Box<str>>`] impl signature (accepting only
    /// borrowed str at some coercion site, returning a different variant
    /// than the sibling peers would) fails here at compile time or at
    /// the assertion instead of at every downstream generic call site.
    #[test]
    fn test_digest_algorithm_try_from_box_str_carries_through_generic_consumer() {
        fn parse<T>(boxed: Box<str>) -> T
        where
            T: std::convert::TryFrom<Box<str>>,
            <T as std::convert::TryFrom<Box<str>>>::Error: std::fmt::Debug,
        {
            <T as std::convert::TryFrom<Box<str>>>::try_from(boxed).expect(
                "canonical label shrunk-owned str must parse through generic TryFrom<Box<str>>",
            )
        }

        for algo in DigestAlgorithm::ALL {
            assert_eq!(
                parse::<DigestAlgorithm>(Box::<str>::from(algo.as_str())),
                algo,
                "generic TryFrom<Box<str>> consumer must recover canonical variant at {algo:?}",
            );
        }
    }

    /// [`TryFrom<Box<str>> for DigestAlgorithm`] rejects non-canonical
    /// input with the same strictness the sibling [`std::str::FromStr`],
    /// [`TryFrom<&str>`], [`TryFrom<String>`], [`TryFrom<Cow<'_, str>>`],
    /// [`TryFrom<&[u8]>`], and [`TryFrom<Vec<u8>>`] peers enforce —
    /// empty input, uppercase, hyphenated, unknown labels, and edge-
    /// whitespace variants all reject. Pins the strict-rejection
    /// contract at the by-value shrunk-owned frontier try-conversion
    /// surface so a downstream consumer bound by [`TryFrom<Box<str>>`]
    /// (a serde `try_from = "Box<str>"` container on a shrunk-owned
    /// label field, a validated-builder frontier pinned at the boxed-
    /// slice intake) inherits the same canonical-only grammar the
    /// direct `.parse::<DigestAlgorithm>()` call sites already read.
    /// Also pins the delegate-through-[`TryFrom<String>`] discipline:
    /// rejection at [`TryFrom<Box<str>>`] tracks rejection at
    /// [`TryFrom<String>`] byte-for-byte at every reject-set element,
    /// so a future permissive-parse regression at the underlying
    /// [`FromStr`] impl lights up here rather than drifting silently
    /// through the shrunk-owned frontier try-conversion surface.
    #[test]
    fn test_digest_algorithm_try_from_box_str_rejects_non_canonical_input() {
        for bad in ["SHA256", "sha-256", "md5", "", "sha256 ", " sha256"] {
    ...
    /// [`TryFrom<Rc<str>> for DigestAlgorithm`] round-trips
    /// `algo.as_str()` at every [`DigestAlgorithm::ALL`] variant AND
    /// cross-agrees byte-for-byte with the sibling
    /// [`std::str::FromStr`] and [`TryFrom<Arc<str>>`] parse peers at
    /// every variant. Pins the one-oracle discipline at the by-value
    /// thread-local shared-owned frontier try-conversion surface: the
    /// receiver-side [`<std::rc::Rc<str> as AsRef<str>>::as_ref`]
    /// borrow flows into the same one-oracle grammar the by-reference
    /// / by-value / by-cross-thread-shared-owned string peers already
    /// read, so a downstream consumer bound by [`TryFrom<Rc<str>>`]
    /// (a serde `try_from = "Rc<str>"` container on a `!Send`
    /// per-task lookaside label field, a validated-builder frontier
    /// pinned at the thread-local shared-slice intake) recovers the
    /// exact canonical variant the paired sibling peers recover — no
    /// per-peer variant drift across the shared-owned receiver
    /// frontier.
    #[test]
    fn test_digest_algorithm_try_from_rc_str_agrees_with_from_str() {
        for algo in DigestAlgorithm::ALL {
            let label = algo.as_str();

            let shared: std::rc::Rc<str> = std::rc::Rc::from(label);
            let via_rc = <DigestAlgorithm as std::convert::TryFrom<std::rc::Rc<str>>>::try_from(
                shared,
            )
            .expect(
                "canonical label thread-local shared-owned str must parse through TryFrom<Rc<str>>",
            );
            assert_eq!(
                via_rc, algo,
                "TryFrom<Rc<str>> must round-trip through as_str at {algo:?}",
            );
            ...
            assert_eq!(
                via_rc, via_from_str,
                "TryFrom<Rc<str>> and FromStr must agree at {algo:?}",
            );

            let arc_shared: std::sync::Arc<str> = std::sync::Arc::from(label);
            let via_arc = <DigestAlgorithm as std::convert::TryFrom<
                std::sync::Arc<str>,
            >>::try_from(arc_shared)
            .expect(
                "canonical label cross-thread shared-owned str must parse through TryFrom<Arc<str>>",
            ...
            assert_eq!(
                via_rc, via_arc,
                "TryFrom<Rc<str>> and TryFrom<Arc<str>> must agree at {algo:?}",
            );
        }
    }

    /// The [`TryFrom<Rc<str>> for DigestAlgorithm`] identity carries
    /// through a generic `impl TryFrom<Rc<str>>` consumer at every
    /// [`DigestAlgorithm::ALL`] variant. A tiny generic function
    /// `fn parse<T>(shared: Rc<str>) -> T where T: TryFrom<Rc<str>>,
    /// T::Error: Debug` — the shape of an actual downstream consumer
    /// (validated-input newtype builder whose canonical intake
    /// contract is pinned as [`Rc<str>`] for single-thread cheap-clone
    /// semantics, serde `try_from = "Rc<str>"` wrapper on a
    /// thread-local shared-owned label field, generic try-conversion
    /// helper that opts into the shared-owned frontier at the
    /// receiver-shape layer within one thread) — recovers the
    /// canonical variant from the thread-local shared-owned canonical
    /// lowercase label at every variant. The structural witness that a
    /// [`DigestAlgorithm`] is genuinely usable at
    /// `impl TryFrom<Rc<str>>` call sites — a regression that drifted
    /// the [`TryFrom<Rc<str>>`] impl signature (accepting only
    /// borrowed str at some coercion site, returning a different
    /// variant than the sibling peers would) fails here at compile
    /// time or at the assertion instead of at every downstream
    /// generic call site.
    #[test]
    fn test_digest_algorithm_try_from_rc_str_carries_through_generic_consumer() {
        fn parse<T>(shared: std::rc::Rc<str>) -> T
        where
            T: std::convert::TryFrom<std::rc::Rc<str>>,
            <T as std::convert::TryFrom<std::rc::Rc<str>>>::Error: std::fmt::Debug,
        {
            <T as std::convert::TryFrom<std::rc::Rc<str>>>::try_from(shared).expect(
                "canonical label thread-local shared-owned str must parse through generic TryFrom<Rc<str>>",
            )
        }

        for algo in DigestAlgorithm::ALL {
            assert_eq!(
                parse::<DigestAlgorithm>(std::rc::Rc::<str>::from(algo.as_str())),
                algo,
                "generic TryFrom<Rc<str>> consumer must recover canonical variant at {algo:?}",
            );
        }
    }

    /// [`TryFrom<Rc<str>> for DigestAlgorithm`] rejects non-canonical
    /// input with the same strictness the sibling
    /// [`std::str::FromStr`], [`TryFrom<&str>`], [`TryFrom<String>`],
    /// [`TryFrom<Cow<'_, str>>`], [`TryFrom<&[u8]>`],
    /// [`TryFrom<Vec<u8>>`], [`TryFrom<Box<str>>`], and
    /// [`TryFrom<Arc<str>>`] peers enforce — empty input, uppercase,
    /// hyphenated, unknown labels, and edge-whitespace variants all
    /// reject. Pins the strict-rejection contract at the by-value
    /// thread-local shared-owned frontier try-conversion surface so a
    /// downstream consumer bound by [`TryFrom<Rc<str>>`] (a serde
    /// `try_from = "Rc<str>"` container on a shared-owned label
    /// field, a validated-builder frontier pinned at the shared-slice
    /// intake) inherits the same canonical-only grammar the direct
    /// `.parse::<DigestAlgorithm>()` call sites already read. Also
    /// pins the delegate-through-[`TryFrom<&str>`] discipline:
    /// rejection at [`TryFrom<Rc<str>>`] tracks rejection at
    /// [`TryFrom<Arc<str>>`] byte-for-byte at every reject-set
    /// element, so a future permissive-parse regression at the
    /// underlying [`FromStr`] impl lights up here rather than drifting
    /// silently through the thread-local shared-owned frontier
    /// try-conversion surface.
    #[test]
    fn test_digest_algorithm_try_from_rc_str_rejects_non_canonical_input() {
        for bad in ["SHA256", "sha-256", "md5", "", "sha256 ", " sha256"] {
            let shared: std::rc::Rc<str> = std::rc::Rc::from(bad);
            let via_rc =
                <DigestAlgorithm as std::convert::TryFrom<std::rc::Rc<str>>>::try_from(shared).ok();
            let arc_shared: std::sync::Arc<str> = std::sync::Arc::from(bad);
            let via_arc =
                <DigestAlgorithm as std::convert::TryFrom<std::sync::Arc<str>>>::try_from(
                    arc_shared,
                )
                .ok();
            assert!(
    ...
    /// The [`TryFrom<Arc<[u8]>> for DigestAlgorithm`] impl round-trips
    /// every canonical label emitted by [`DigestAlgorithm::as_str`]
    /// through a cross-thread shared-owned [`std::sync::Arc<[u8]>`]
    /// receiver and agrees with [`std::str::FromStr`] AND the sibling
    /// [`TryFrom<&[u8]>`] peer byte-for-byte at every
    /// [`DigestAlgorithm::ALL`] variant. Pins the delegate-through-
    /// [`TryFrom<&[u8]>`] discipline at the cross-thread shared-owned
    /// byte-slice frontier: a regression that drifted the
    /// [`TryFrom<Arc<[u8]>>`] impl body (routing through a divergent
    /// oracle, cloning the shared bytes into a fresh [`Vec<u8>`] and
    /// re-parsing through a case-folded branch, negotiating the
    /// atomic refcount through an [`std::sync::Arc::try_unwrap`]-
    /// fallback-clone-then-parse cascade, coercing a UTF-8-invalid tail
    /// through a lossy decode instead of the strict
    /// [`std::str::from_utf8`] gate) fails here rather than at every
    /// downstream [`TryFrom<Arc<[u8]>>`] call site. Also pins the
    /// cross-agreement with the sibling [`TryFrom<&[u8]>`] peer so a
    /// downstream consumer that receives its byte-slice label at either
    /// the borrowed [`&[u8]`] frontier or the cross-thread shared-owned
    /// [`std::sync::Arc<[u8]>`] frontier (a serde container that opts
    /// into `#[serde(try_from = "Arc<[u8]>")]` on a byte-frontier
    /// wrapper field, a validated-builder frontier pinned at the
    /// shared-owned raw-byte intake) recovers the exact canonical
    /// variant the paired sibling peer recovers — no per-peer variant
    /// drift across the cross-thread shared-owned byte-slice receiver
    /// frontier.
    #[test]
    fn test_digest_algorithm_try_from_arc_bytes_agrees_with_from_str() {
        for algo in DigestAlgorithm::ALL {
            let label = algo.as_str();

            let shared: std::sync::Arc<[u8]> = std::sync::Arc::from(label.as_bytes());
            let via_arc = <DigestAlgorithm as std::convert::TryFrom<std::sync::Arc<[u8]>>>::try_from(
                shared,
            )
            .expect("canonical label cross-thread shared-owned bytes must parse through TryFrom<Arc<[u8]>>");
            assert_eq!(
                via_arc, algo,
                "TryFrom<Arc<[u8]>> must round-trip through as_str at {algo:?}",
            );
            ...
            assert_eq!(
                via_arc, via_from_str,
                "TryFrom<Arc<[u8]>> and FromStr must agree at {algo:?}",
            );

            let via_slice =
                <DigestAlgorithm as std::convert::TryFrom<&[u8]>>::try_from(label.as_bytes())
                    .expect("canonical label borrowed bytes must parse through TryFrom<&[u8]>");
            assert_eq!(
                via_arc, via_slice,
                "TryFrom<Arc<[u8]>> and TryFrom<&[u8]> must agree at {algo:?}",
            );
        }
    }

    /// The [`TryFrom<Arc<[u8]>> for DigestAlgorithm`] identity carries
    /// through a generic `impl TryFrom<Arc<[u8]>>` consumer at every
    /// [`DigestAlgorithm::ALL`] variant. A tiny generic function
    /// `fn parse<T>(shared: Arc<[u8]>) -> T where T: TryFrom<Arc<[u8]>>,
    /// T::Error: Debug` — the shape of an actual downstream consumer
    /// (validated-input newtype builder whose canonical intake contract
    /// is pinned as [`Arc<[u8]>`] for cross-thread cheap-clone semantics
    /// on the input, serde `try_from = "Arc<[u8]>"` wrapper on a
    /// cross-thread shared-owned byte-frontier label field, generic
    /// try-conversion helper that opts into the cross-thread shared-owned
    /// byte-slice frontier at the receiver-shape layer) — recovers the
    /// canonical variant from the cross-thread shared-owned canonical
    /// lowercase-ASCII label bytes at every variant. The structural
    /// witness that a [`DigestAlgorithm`] is genuinely usable at
    /// `impl TryFrom<Arc<[u8]>>` call sites — a regression that drifted
    /// the [`TryFrom<Arc<[u8]>>`] impl signature (accepting only borrowed
    /// byte slices at some coercion site, returning a different variant
    /// than the sibling peers would) fails here at compile time or at
    /// the assertion instead of at every downstream generic call site.
    #[test]
    fn test_digest_algorithm_try_from_arc_bytes_carries_through_generic_consumer() {
        fn parse<T>(shared: std::sync::Arc<[u8]>) -> T
        where
            T: std::convert::TryFrom<std::sync::Arc<[u8]>>,
            <T as std::convert::TryFrom<std::sync::Arc<[u8]>>>::Error: std::fmt::Debug,
        {
            <T as std::convert::TryFrom<std::sync::Arc<[u8]>>>::try_from(shared).expect(
                "canonical label cross-thread shared-owned bytes must parse through generic TryFrom<Arc<[u8]>>",
            )
        }

        for algo in DigestAlgorithm::ALL {
            assert_eq!(
                parse::<DigestAlgorithm>(std::sync::Arc::<[u8]>::from(algo.as_str().as_bytes())),
                algo,
                "generic TryFrom<Arc<[u8]>> consumer must recover canonical variant at {algo:?}",
            );
        }
    }

    /// [`TryFrom<Arc<[u8]>> for DigestAlgorithm`] rejects UTF-8-valid
    /// but non-canonical input with the same strictness the sibling
    /// [`std::str::FromStr`], [`TryFrom<&str>`], [`TryFrom<String>`],
    /// [`TryFrom<Cow<'_, str>>`], [`TryFrom<&[u8]>`],
    /// [`TryFrom<Vec<u8>>`], [`TryFrom<Box<str>>`],
    /// [`TryFrom<Arc<str>>`], [`TryFrom<Rc<str>>`], and
    /// [`TryFrom<Box<[u8]>>`] peers enforce — empty input, uppercase,
    /// hyphenated, unknown labels, and edge-whitespace variants all
    /// reject. Pins the strict-rejection contract at the by-value
    /// cross-thread shared-owned byte-slice frontier try-conversion
    /// surface so a downstream consumer bound by [`TryFrom<Arc<[u8]>>`]
    /// (a serde `try_from = "Arc<[u8]>"` container on a cross-thread
    /// shared-owned byte-frontier field, a validated-builder frontier
    /// pinned at the shared-owned raw-byte intake) inherits the same
    /// canonical-only grammar the direct `.parse::<DigestAlgorithm>()`
    /// call sites already read. Also pins the delegate-through-
    /// [`TryFrom<&[u8]>`] discipline: rejection at [`TryFrom<Arc<[u8]>>`]
    /// tracks rejection at [`TryFrom<&[u8]>`] byte-for-byte at every
    /// reject-set element, so a future permissive-parse regression at
    /// the underlying [`FromStr`] impl lights up here rather than
    /// drifting silently through the cross-thread shared-owned byte-slice
    /// frontier try-conversion surface.
    #[test]
    fn test_digest_algorithm_try_from_arc_bytes_rejects_non_canonical_input() {
        for bad in ["SHA256", "sha-256", "md5", "", "sha256 ", " sha256"] {
            let shared: std::sync::Arc<[u8]> = std::sync::Arc::from(bad.as_bytes());
            let via_arc =
                <DigestAlgorithm as std::convert::TryFrom<std::sync::Arc<[u8]>>>::try_from(shared)
                    .ok();
            let via_slice =
                <DigestAlgorithm as std::convert::TryFrom<&[u8]>>::try_from(bad.as_bytes()).ok();
            assert!(
                via_arc.is_none(),
                "TryFrom<Arc<[u8]>> must reject non-canonical UTF-8-valid input {bad:?}",
            );
            assert_eq!(
                via_arc, via_slice,
                "TryFrom<Arc<[u8]>> and TryFrom<&[u8]> must agree on rejection at {bad:?}",
            );
        }
    }

    /// The [`TryFrom<Rc<[u8]>> for DigestAlgorithm`] impl round-trips
    /// every canonical label emitted by [`DigestAlgorithm::as_str`]
    /// through a thread-local shared-owned [`std::rc::Rc<[u8]>`]
    /// receiver and agrees with [`std::str::FromStr`] AND the sibling
    /// [`TryFrom<&[u8]>`] peer byte-for-byte at every
    /// [`DigestAlgorithm::ALL`] variant. Pins the delegate-through-
    /// [`TryFrom<&[u8]>`] discipline at the thread-local shared-owned
    /// byte-slice frontier: a regression that drifted the
    /// [`TryFrom<Rc<[u8]>>`] impl body (routing through a divergent
    /// oracle, cloning the shared bytes into a fresh [`Vec<u8>`] and
    /// re-parsing through a case-folded branch, negotiating the
    /// non-atomic refcount through an [`std::rc::Rc::try_unwrap`]-
    /// fallback-clone-then-parse cascade, coercing a UTF-8-invalid tail
    /// through a lossy decode instead of the strict
    /// [`std::str::from_utf8`] gate) fails here rather than at every
    /// downstream [`TryFrom<Rc<[u8]>>`] call site. Also pins the
    /// cross-agreement with the sibling [`TryFrom<&[u8]>`] and
    /// [`TryFrom<Arc<[u8]>>`] peers so a downstream consumer that
    /// receives its byte-slice label at either the borrowed [`&[u8]`]
    /// frontier, the cross-thread shared-owned [`std::sync::Arc<[u8]>`]
    /// frontier, or the thread-local shared-owned [`std::rc::Rc<[u8]>`]
    /// frontier (a serde container that opts into
    /// `#[serde(try_from = "Rc<[u8]>")]` on a byte-frontier wrapper
    /// field, a validated-builder frontier pinned at the shared-owned
    /// raw-byte intake inside a single-threaded arena) recovers the
    /// exact canonical variant the paired sibling peers recover — no
    /// per-peer variant drift across the thread-local shared-owned
    /// byte-slice receiver frontier.
    #[test]
    fn test_digest_algorithm_try_from_rc_bytes_agrees_with_from_str() {
        for algo in DigestAlgorithm::ALL {
            let label = algo.as_str();

            let shared: std::rc::Rc<[u8]> = std::rc::Rc::from(label.as_bytes());
            let via_rc = <DigestAlgorithm as std::convert::TryFrom<std::rc::Rc<[u8]>>>::try_from(
                shared,
            )
            .expect("canonical label thread-local shared-owned bytes must parse through TryFrom<Rc<[u8]>>");
            assert_eq!(
                via_rc, algo,
                "TryFrom<Rc<[u8]>> must round-trip through as_str at {algo:?}",
            );
            ...
            assert_eq!(
                via_rc, via_from_str,
                "TryFrom<Rc<[u8]>> and FromStr must agree at {algo:?}",
            );
            ...
            assert_eq!(
                via_rc, via_slice,
                "TryFrom<Rc<[u8]>> and TryFrom<&[u8]> must agree at {algo:?}",
            );

            let via_arc = <DigestAlgorithm as std::convert::TryFrom<std::sync::Arc<[u8]>>>::try_from(
                std::sync::Arc::<[u8]>::from(label.as_bytes()),
            )
            .expect("canonical label cross-thread shared-owned bytes must parse through TryFrom<Arc<[u8]>>");
            assert_eq!(
                via_rc, via_arc,
                "TryFrom<Rc<[u8]>> and TryFrom<Arc<[u8]>> must agree at {algo:?}",
            );
        }
    }

    /// The [`TryFrom<Rc<[u8]>> for DigestAlgorithm`] identity carries
    /// through a generic `impl TryFrom<Rc<[u8]>>` consumer at every
    /// [`DigestAlgorithm::ALL`] variant. A tiny generic function
    /// `fn parse<T>(shared: Rc<[u8]>) -> T where T: TryFrom<Rc<[u8]>>,
    /// T::Error: Debug` — the shape of an actual downstream consumer
    /// (validated-input newtype builder whose canonical intake contract
    /// is pinned as [`Rc<[u8]>`] for thread-local cheap-clone semantics
    /// on the input, serde `try_from = "Rc<[u8]>"` wrapper on a
    /// thread-local shared-owned byte-frontier label field, generic
    /// try-conversion helper that opts into the thread-local shared-owned
    /// byte-slice frontier at the receiver-shape layer) — recovers the
    /// canonical variant from the thread-local shared-owned canonical
    /// lowercase-ASCII label bytes at every variant. The structural
    /// witness that a [`DigestAlgorithm`] is genuinely usable at
    /// `impl TryFrom<Rc<[u8]>>` call sites — a regression that drifted
    /// the [`TryFrom<Rc<[u8]>>`] impl signature (accepting only borrowed
    /// byte slices at some coercion site, returning a different variant
    /// than the sibling peers would) fails here at compile time or at
    /// the assertion instead of at every downstream generic call site.
    #[test]
    fn test_digest_algorithm_try_from_rc_bytes_carries_through_generic_consumer() {
        fn parse<T>(shared: std::rc::Rc<[u8]>) -> T
        where
            T: std::convert::TryFrom<std::rc::Rc<[u8]>>,
            <T as std::convert::TryFrom<std::rc::Rc<[u8]>>>::Error: std::fmt::Debug,
        {
            <T as std::convert::TryFrom<std::rc::Rc<[u8]>>>::try_from(shared).expect(
                "canonical label thread-local shared-owned bytes must parse through generic TryFrom<Rc<[u8]>>",
            )
        }

        for algo in DigestAlgorithm::ALL {
            assert_eq!(
                parse::<DigestAlgorithm>(std::rc::Rc::<[u8]>::from(algo.as_str().as_bytes())),
                algo,
                "generic TryFrom<Rc<[u8]>> consumer must recover canonical variant at {algo:?}",
            );
        }
    }

    /// [`TryFrom<Rc<[u8]>> for DigestAlgorithm`] rejects UTF-8-valid
    /// but non-canonical input with the same strictness the sibling
    /// [`std::str::FromStr`], [`TryFrom<&str>`], [`TryFrom<String>`],
    /// [`TryFrom<Cow<'_, str>>`], [`TryFrom<&[u8]>`],
    /// [`TryFrom<Vec<u8>>`], [`TryFrom<Box<str>>`],
    /// [`TryFrom<Arc<str>>`], [`TryFrom<Rc<str>>`],
    /// [`TryFrom<Box<[u8]>>`], and [`TryFrom<Arc<[u8]>>`] peers enforce —
    /// empty input, uppercase, hyphenated, unknown labels, and
    /// edge-whitespace variants all reject. Pins the strict-rejection
    /// contract at the by-value thread-local shared-owned byte-slice
    /// frontier try-conversion surface so a downstream consumer bound
    /// by [`TryFrom<Rc<[u8]>>`] (a serde `try_from = "Rc<[u8]>"`
    /// container on a thread-local shared-owned byte-frontier field, a
    /// validated-builder frontier pinned at the shared-owned raw-byte
    /// intake inside a single-threaded arena) inherits the same
    /// canonical-only grammar the direct `.parse::<DigestAlgorithm>()`
    /// call sites already read. Also pins the delegate-through-
    /// [`TryFrom<&[u8]>`] discipline: rejection at [`TryFrom<Rc<[u8]>>`]
    /// tracks rejection at [`TryFrom<&[u8]>`] byte-for-byte at every
    /// reject-set element, so a future permissive-parse regression at
    /// the underlying [`FromStr`] impl lights up here rather than
    /// drifting silently through the thread-local shared-owned byte-slice
    /// frontier try-conversion surface.
    #[test]
    fn test_digest_algorithm_try_from_rc_bytes_rejects_non_canonical_input() {
        for bad in ["SHA256", "sha-256", "md5", "", "sha256 ", " sha256"] {
            let shared: std::rc::Rc<[u8]> = std::rc::Rc::from(bad.as_bytes());
            let via_rc =
                <DigestAlgorithm as std::convert::TryFrom<std::rc::Rc<[u8]>>>::try_from(shared)
                    .ok();
            let via_slice =
                <DigestAlgorithm as std::convert::TryFrom<&[u8]>>::try_from(bad.as_bytes()).ok();
            assert!(
                via_rc.is_none(),
                "TryFrom<Rc<[u8]>> must reject non-canonical UTF-8-valid input {bad:?}",
            );
            assert_eq!(
                via_rc, via_slice,
                "TryFrom<Rc<[u8]>> and TryFrom<&[u8]> must agree on rejection at {bad:?}",
            );
        }
    }

    /// [`TryFrom<Cow<'_, [u8]>> for DigestAlgorithm`] agrees with
    /// [`std::str::FromStr`] on every [`DigestAlgorithm::ALL`] variant on
    /// BOTH the [`Cow::Borrowed`] and [`Cow::Owned`] arms when the byte
    /// payload is the canonical lowercase label's UTF-8 bytes. Pins the
    /// borrowed-or-owned-byte-slice parse peer against the shared
    /// canonical-label oracle and against the by-shape byte-slice peers
    /// it delegates to ([`TryFrom<&[u8]>`] on the borrowed arm,
    /// [`TryFrom<Vec<u8>>`] on the owned arm) — a regression that drifted
    /// the [`Cow<'_, [u8]>`] arm dispatch (routing both arms through the
    /// borrowed peer and losing the owned-arm move-through, cloning the
    /// [`Cow::Owned`] payload into a fresh [`Vec`] and re-parsing through
    /// a case-folded branch, admitting an empty label on the
    /// [`Cow::Borrowed`] arm through a short-circuit) fails here rather
    /// than at every downstream `TryFrom<Cow<'_, [u8]>>` call site.
    #[test]
    fn test_digest_algorithm_try_from_cow_bytes_agrees_with_from_str() {
        use std::borrow::Cow;
        for algo in DigestAlgorithm::ALL {
            let label = algo.as_str();

            let via_borrowed = <DigestAlgorithm as std::convert::TryFrom<Cow<'_, [u8]>>>::try_from(
                Cow::Borrowed(label.as_bytes()),
            )
            .expect("canonical label bytes must parse through TryFrom<Cow::Borrowed>");
            assert_eq!(
                via_borrowed, algo,
                "TryFrom<Cow::Borrowed<[u8]>> must round-trip through as_str at {algo:?}",
            );

            let via_owned = <DigestAlgorithm as std::convert::TryFrom<Cow<'_, [u8]>>>::try_from(
                Cow::Owned(label.as_bytes().to_vec()),
            )
            .expect("canonical label owned bytes must parse through TryFrom<Cow::Owned>");
            assert_eq!(
                via_owned, algo,
                "TryFrom<Cow::Owned<[u8]>> must round-trip through as_str at {algo:?}",
            );
            ...
            assert_eq!(
                via_borrowed, via_from_str,
                "TryFrom<Cow::Borrowed<[u8]>> and FromStr must agree at {algo:?}",
            );
            assert_eq!(
                via_owned, via_from_str,
                "TryFrom<Cow::Owned<[u8]>> and FromStr must agree at {algo:?}",
            );
            ...
            assert_eq!(
                ...
                "TryFrom<Cow::Borrowed<[u8]>> and TryFrom<&[u8]> must agree at {algo:?}",
            );
            ...
            assert_eq!(
                via_owned, via_vec,
                "TryFrom<Cow::Owned<[u8]>> and TryFrom<Vec<u8>> must agree at {algo:?}",
            );
        }
    }

    /// The [`TryFrom<Cow<'_, [u8]>> for DigestAlgorithm`] identity carries
    /// through a generic `impl for<'a> TryFrom<Cow<'a, [u8]>>` consumer at
    /// every [`DigestAlgorithm::ALL`] variant on BOTH [`Cow`] arms. A tiny
    /// generic function
    /// `fn parse<'a, T>(bytes: Cow<'a, [u8]>) -> T
    /// where T: TryFrom<Cow<'a, [u8]>>, T::Error: Debug` — the shape of
    /// an actual downstream consumer (borrowed-or-owned byte pipeline
    /// that consumes [`Cow<'_, [u8]>`] to defer the ownership decision to
    /// its caller, serde `try_from = "Cow<'_, [u8]>"` wrapper on a
    /// `serde_bytes` field, generic try-conversion helper that opts into
    /// the borrowed-or-owned byte frontier at the receiver-shape layer)
    /// — recovers the canonical variant from the canonical lowercase
    /// label's UTF-8 bytes on both arms at every variant. The structural
    /// witness that a [`DigestAlgorithm`] is genuinely usable at
    /// `impl for<'a> TryFrom<Cow<'a, [u8]>>` call sites — a regression
    /// that drifted the [`TryFrom<Cow<'_, [u8]>>`] impl signature (e.g.,
    /// binding the [`Cow`] lifetime to `'static` and rejecting borrowed
    /// non-static payloads, returning a different variant than the
    /// by-shape peers would) fails here at compile time or at the
    /// assertion instead of at every downstream generic call site.
    #[test]
    fn test_digest_algorithm_try_from_cow_bytes_carries_through_generic_consumer() {
        use std::borrow::Cow;
        fn parse<'a, T>(bytes: Cow<'a, [u8]>) -> T
        where
            T: std::convert::TryFrom<Cow<'a, [u8]>>,
            <T as std::convert::TryFrom<Cow<'a, [u8]>>>::Error: std::fmt::Debug,
        {
            <T as std::convert::TryFrom<Cow<'a, [u8]>>>::try_from(bytes)
                .expect("canonical label bytes must parse through generic TryFrom<Cow<'_, [u8]>>")
        }

        for algo in DigestAlgorithm::ALL {
            assert_eq!(
                parse::<DigestAlgorithm>(Cow::Borrowed(algo.as_str().as_bytes())),
                algo,
                "generic TryFrom<Cow::Borrowed<[u8]>> consumer must recover canonical variant at {algo:?}",
            );
            assert_eq!(
                parse::<DigestAlgorithm>(Cow::Owned(algo.as_str().as_bytes().to_vec())),
                algo,
                "generic TryFrom<Cow::Owned<[u8]>> consumer must recover canonical variant at {algo:?}",
            );
        }
    }

    /// [`TryFrom<Cow<'_, [u8]>> for DigestAlgorithm`] rejects UTF-8-valid
    /// but non-canonical input with the same strictness the sibling
    /// [`std::str::FromStr`], [`TryFrom<&[u8]>`], and [`TryFrom<Vec<u8>>`]
    /// peers enforce on BOTH the [`Cow::Borrowed`] and [`Cow::Owned`]
    /// arms — empty input, uppercase, hyphenated, unknown labels, and
    /// edge-whitespace variants all reject. Pins the strict-rejection
    /// contract at the borrowed-or-owned-byte frontier try-conversion
    /// surface so a downstream consumer bound by
    /// [`TryFrom<Cow<'_, [u8]>>`] (a serde `try_from = "Cow<'_, [u8]>"`
    /// container on a `serde_bytes` field, a codec sink that yields
    /// either a zero-copy borrowed sub-slice or an owned [`Vec<u8>`])
    /// inherits the same canonical-only grammar the direct
    /// `.parse::<DigestAlgorithm>()` call sites already read. Also pins
    /// the delegate-through-by-shape-peer discipline: rejection on the
    /// [`Cow::Borrowed`] arm tracks [`TryFrom<&[u8]>`] rejection
    /// byte-for-byte and rejection on the [`Cow::Owned`] arm tracks
    /// [`TryFrom<Vec<u8>>`] rejection byte-for-byte, so a future
    /// permissive-parse regression at the underlying [`FromStr`] impl
    /// lights up here rather than drifting silently through the
    /// borrowed-or-owned-byte-slice frontier surface.
    #[test]
    fn test_digest_algorithm_try_from_cow_bytes_rejects_non_canonical_input() {
        use std::borrow::Cow;
        for bad in ["SHA256", "sha-256", "md5", "", "sha256 ", " sha256"] {
            let via_borrowed = <DigestAlgorithm as std::convert::TryFrom<Cow<'_, [u8]>>>::try_from(
                Cow::Borrowed(bad.as_bytes()),
            )
            .ok();
            let via_owned = <DigestAlgorithm as std::convert::TryFrom<Cow<'_, [u8]>>>::try_from(
                Cow::Owned(bad.as_bytes().to_vec()),
            )
            .ok();
            let via_slice =
                <DigestAlgorithm as std::convert::TryFrom<&[u8]>>::try_from(bad.as_bytes()).ok();
            ...
            assert!(
                via_borrowed.is_none(),
                "TryFrom<Cow::Borrowed<[u8]>> must reject non-canonical UTF-8-valid input {bad:?}",
            );
            assert!(
                via_owned.is_none(),
                "TryFrom<Cow::Owned<[u8]>> must reject non-canonical UTF-8-valid input {bad:?}",
            );
            assert_eq!(
                via_borrowed, via_slice,
                "TryFrom<Cow::Borrowed<[u8]>> and TryFrom<&[u8]> must agree on rejection at {bad:?}",
            );
            assert_eq!(
                via_owned, via_vec,
                "TryFrom<Cow::Owned<[u8]>> and TryFrom<Vec<u8>> must agree on rejection at {bad:?}",
            );
        }
    }

    /// [`TryFrom<Cow<'_, [u8]>> for DigestAlgorithm`] rejects UTF-8-invalid
    /// byte payloads on BOTH the [`Cow::Borrowed`] and [`Cow::Owned`]
    /// arms — a stray non-UTF-8 sequence, a partial-write byte tail that
    /// clips a UTF-8 continuation — before the input reaches the string
    /// oracle. Pins the two-stage validation contract inherited from the
    /// by-shape byte peers on both arms: UTF-8 validation runs first,
    /// then the canonical-label grammar. The rejection message names the
    /// failure mode so a downstream consumer pinning a per-shape handling
    /// policy at its frontier can distinguish UTF-8-invalid input from
    /// non-canonical labels off the returned error string.
    #[test]
    fn test_digest_algorithm_try_from_cow_bytes_rejects_invalid_utf8() {
        use std::borrow::Cow;
        for bad in [
            &b"\xffsha256"[..],
            &b"sha256\xff"[..],
            ...
            let via_owned = <DigestAlgorithm as std::convert::TryFrom<Cow<'_, [u8]>>>::try_from(
                Cow::Owned(bad.to_vec()),
            );
            assert!(
                via_borrowed.is_err(),
                "TryFrom<Cow::Borrowed<[u8]>> must reject UTF-8-invalid input {bad:?}",
            );
            assert!(
                via_owned.is_err(),
                "TryFrom<Cow::Owned<[u8]>> must reject UTF-8-invalid input {bad:?}",
            );
            let borrowed_msg = via_borrowed.unwrap_err().to_string();
            let owned_msg = via_owned.unwrap_err().to_string();
            assert!(
                borrowed_msg.contains("not valid UTF-8"),
                "UTF-8-invalid rejection on Cow::Borrowed must name the failure mode, got {borrowed_msg:?}",
            );
            assert!(
