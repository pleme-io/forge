...
/// Palette + width pairing every pre-lift bare heavy-rule site fed to
/// [`println!("{}", "━".repeat(<width>).<color>())`]. Closed to the
/// exact two `(color, width)` pairings the pre-lift 7-site sibling
/// class carried and nothing more — a new palette entry is a
/// deliberate additive edit, not an open call-site choice.
///
/// # Compounding
///
/// Pre-lift each of the 7 sibling sites in `commands/rust_service.rs`
/// nailed the color and width into two literal arguments at the call
/// site (`"━".repeat(80).bright_blue()`, `"━".repeat(60).bright_green()`);
/// a rename that misspells `.bright_blue` as `.brith_blue` fails to
/// compile, but a drift from width 80 to width 78 at one site alone
/// silently lines two adjacent report sections against visibly
/// different rules, and a swap of `.bright_blue()` for `.blue()` on
/// one rule silently loses the visual contrast against the headline.
/// Post-lift the mapping [`HeavyRuleStyle`] → `(color, width)` lives
/// in ONE match arm; the enum is closed, so a future variant added
/// without an arm fails the exhaustiveness check at build time.
///
/// # Distinct from [`write_success_banner`], [`write_release_stage_banner`]
///
/// [`write_success_banner`] carries a THREE-line `━`-rule + message +
/// `━`-rule stanza (`bright_green` throughout) fused as one primitive
/// because the message is load-bearing to the primitive's identity.
/// [`write_release_stage_banner`] carries the same three-line shape
/// under a `=`-rule + composite headline grammar. This primitive
/// carries the **single-line** bare-rule shape the 7 pre-lift
/// consumer sites used to open or close a headline whose composition
/// varies (SUMMARY vs. DEPLOYMENT REPORT vs. PUSH COMPLETE with
/// arch-suffix branching) enough that a fused three-line primitive
/// would need to accept a `Display` middle-line and lose the
/// arch-branch println!s that live between the two rules. Coexists
/// with the fused banners rather than subsuming them because the
/// pre-lift consumers of those banners emit exactly one message
/// between the rules, while these seven consumers emit multiple
/// heterogeneous lines.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum HeavyRuleStyle {
    /// Summary-section rule (`"━".repeat(80).bright_blue()`). 5
    /// pre-lift sites in `commands/rust_service.rs`: opening +
    /// closing rule around the `📋 SUMMARY` headline (:2278, :2280),
    /// opening + closing rule around the `📊 DEPLOYMENT REPORT`
    /// headline (:2660, :2662), and the lone trailing rule below the
    /// `🎉 Deployment workflow complete!` headline (:2290).
    SummaryBlue80,
    /// Push-complete milestone rule (`"━".repeat(60).bright_green()`).
    /// 2 pre-lift sites in `commands/rust_service.rs`: opening +
    /// closing rule around the `PUSH COMPLETE` headline whose middle
...
impl HeavyRuleStyle {
    ...
    ///
    /// Kept as a match on the closed enum (rather than a `width`
    /// field on each variant) so a fleet-wide sweep for the literal
    /// widths (`60`, `80`) finds this arm, not a runtime value nobody
    /// can grep for.
...
/// Prints the bare heavy-rule line every pre-lift consumer in
/// `commands/rust_service.rs` spelled inline as
/// `println!("{}", "━".repeat(<width>).<color>())`. Marks a
/// section-boundary rule (SUMMARY opening/closing, DEPLOYMENT REPORT
/// opening/closing, trailing deployment-complete rule, PUSH COMPLETE
/// opening/closing) at 7 sibling call sites — the pre-lift one-line
/// stanza — so a future palette shift against rule-char, rule-width,
/// or color-token happens at ONE site rather than seven.
///
/// Delegates to [`write_heavy_rule`] against `std::io::stdout()`; the
/// writer split exists so the fail-before-pass test can pin the
/// rule-glyph, width, and palette contract by inspecting emitted
/// bytes rather than shelling out and grepping stdout.
pub fn print_heavy_rule(style: HeavyRuleStyle) {
    let _ = write_heavy_rule(&mut std::io::stdout().lock(), style);
}

/// Writer-taking sibling to [`print_heavy_rule`]. Emits the single
/// heavy-rule line (`"━".repeat(<width>).<color>()` per
/// [`HeavyRuleStyle`]) via [`writeln!`] against the supplied writer.
/// [`print_heavy_rule`] is the stdout adapter; this variant exists
/// so tests can pin the rule-glyph, width, and palette contract
/// without capturing stdout.
pub fn write_heavy_rule<W: std::io::Write>(
    w: &mut W,
    style: HeavyRuleStyle,
) -> std::io::Result<()> {
...
/// Prints the one-line `"   ℹ️  <message>"` (three-space indent + `ℹ️`
/// glyph + two-space gap + plain message) DEEPER-nested in-body info
/// grammar 3 pre-lift consumer sites spelled inline as `println!("   ℹ️
/// <literal>")` in `commands/migrations.rs` (envFrom "No secrets
/// configured" arm at :466, "No existing migration jobs to clean up"
/// arm at :919, "No orphaned pods to clean up" arm at :959). Marks an
/// informational note inside a sub-listing whose siblings already carry
/// a three-space indent (a `print_step_pass` / `print_plain_step_warn`
/// / `print_red_x_step_fail` line above it in the same block) — the
/// deeper-nested twin of [`print_step_info`] under the same `ℹ️  `
/// glyph and identical uncolored-line contract, with the sole
/// difference being a three-space leading indent that visually
/// registers this line as a peer of the surrounding indented items
/// rather than a top-level step.
///
/// # Distinct from [`print_step_info`]
///
/// [`print_step_info`] emits the same `ℹ️  <message>` grammar at ZERO
/// leading indent — its 15 pre-lift consumer sites all sat at the
/// top-level of a command's readout (a milestone-level note, not
/// inside a sub-listing). The three `commands/migrations.rs` sites
/// this primitive owns each sit inside a check-loop's "no-work"
/// branch whose sibling lines carry a three-space indent
/// (`print_step_pass` "Deleted orphaned pods", `print_plain_step_warn`
/// "Configured secret not found: …") — the indent is not decorative,
/// it aligns the informational note with those siblings so a reader
/// scanning the block sees a single indented column. The two grammars
/// coexist deliberately: [`print_step_info`]'s existing caller-shield
/// test explicitly names this 3-space-indented variant as a distinct
/// class NOT enrolled in its own shield, so a lift of these 3 sites
/// onto [`print_step_info`] would drop the indent and drift the visual
/// grammar of the surrounding block; a fresh primitive is the only
/// safe migration path.
///
/// # Uncolored — no palette to preserve
///
/// Pre-lift every consumer spelled the whole line plain — no `.color()`
/// chain on the glyph, no `format!(...).color()` wrap on the composed
/// line. The primitive preserves that: the writer receives `   ℹ️
/// <msg>\n` with no ANSI escape sequences anywhere, so a terminal
/// without color renders exactly what a color-capable one does. A
/// future promotion to `.bright_cyan()` on the whole line would
/// collide with the sibling milestone-level [`print_info`] grammar.
///
/// # Compounding
///
/// Pre-lift 3 sibling sites in `commands/migrations.rs` each restated
/// the `println!("   ℹ️  <literal>")` grammar verbatim, with the
/// three-space indent, the `ℹ️` glyph, the two-space gap after the
/// glyph, and the plain-message tail all spelled inline. A future
/// palette adjustment (a promotion to `.bright_cyan()` on the whole
/// line, an OTLP `sub_check_info` observability event wired alongside
/// the print, a shift of the three-space indent to two- or
/// four-space under a standardized body-indent, a swap of `ℹ️  ` for
/// `[i] ` under a CI-log-friendly grammar) had to hit 3 sites in
/// lockstep or drift the visual grammar; post-lift it hits ONE typed
/// body. Delegates to [`write_step_info_indented`] against
/// [`std::io::stdout()`]; the writer split exists so the fail-before-
/// pass test can pin the one-line body, the three-space indent
/// OUTSIDE the glyph span, the `ℹ️  ` glyph + two-space gap, and the
/// absence-of-any-ANSI-escape contract by inspecting emitted bytes
/// rather than shelling out and grepping stdout.
pub fn print_step_info_indented(message: &str) {
    let _ = write_step_info_indented(&mut std::io::stdout().lock(), message);
}

/// Writer-taking sibling to [`print_step_info_indented`]. Emits the
/// single `   ℹ️  <message>` line via [`writeln!`] against the supplied
/// writer. [`print_step_info_indented`] is the stdout adapter; this
/// variant exists so tests can pin the one-line body, the three-space
/// leading indent (OUTSIDE the glyph — pre-lift consumers spelled
/// `"   ℹ️  <fmt>"` verbatim so the indent is not part of the glyph
/// span), the `ℹ️  ` glyph + two-space gap, and the absence of any
/// ANSI escape sequence without capturing stdout.
pub fn write_step_info_indented<W: std::io::Write>(
    w: &mut W,
    message: &str,
) -> std::io::Result<()> {
    writeln!(w, "   ℹ️  {}", message)
...
/// Prints the three-line fused `--- DRY RUN SQL --- / <sql> / --- END DRY
/// RUN ---` zero-indent open+body+close SQL-dump stanza the two pre-lift
/// consumer sites in `commands/seed.rs` (`seed`'s `dry_run` guard around
/// `generate_seed_sql`; `unseed`'s `dry_run` guard around
/// `generate_unseed_sql`) each spelled inline as
/// `println!("--- DRY RUN SQL ---"); println!("{}", sql); println!("---
/// END DRY RUN ---");`. Marks the dry-run SQL dump an operator sees when
/// they pass `--dry-run` to `forge seed` / `forge unseed` — the header
/// fence + literal SQL payload + closing fence framing a copy-and-paste
/// runnable block above the point where the wet-run path would have
/// executed `exec_psql` — where the ASCII triple-hyphen fences (`--- `
/// ... ` ---`) frame the payload distinctly from the U+2500 `── ...
/// ──.dimmed()` diagnostic-stream markers [`print_dimmed_dashed_marker`]
/// carries (that primitive lives inside `commands/prerelease.rs`'s cargo
/// test stdout/stderr fenced blocks under the G11 and G14 gates) and
/// from the U+2500 `└` ... `┘` thin-box closing row
/// [`print_thin_box_bottom`] emits.
///
/// # Distinct from every peer `ui::print_*` primitive
///
/// [`print_dimmed_dashed_marker`] emits a single-line `── <label>
/// ──.dimmed()` U+2500 dash-fence with the whole span `.dimmed()` — this
/// primitive emits THREE lines with plain (uncolored) ASCII triple-
/// hyphen fences and a caller-supplied SQL payload between them. The
/// dashed-marker peer is a boundary above ONE diagnostic block that the
/// caller then dumps into separately; this primitive OWNS the whole
/// open+body+close as a fused unit so a re-lift cannot drift the payload
/// away from its fences.
///
/// [`print_diagnostic_line`] emits a single `   <line>` three-space-
/// indented plain diagnostic row inside a fenced block; this primitive
/// emits the fences AND their SQL payload with ZERO indent (the payload
/// is a copy-and-paste runnable multi-line SQL blob, so a leading indent
/// on every line would break the operator's paste). The payload reaches
/// the writer as-is via a single `writeln!("{}", sql)` matching the
/// pre-lift `println!("{}", sql)` byte-for-byte.
///
/// # zero indent + plain (uncolored) + ASCII triple-hyphen fences
///
/// Pre-lift both consumers spelled the fences as
/// `println!("--- DRY RUN SQL ---")` and `println!("--- END DRY RUN
/// ---")` — ASCII `-` (U+002D) triple-hyphens, not U+2500 `─` box-
/// drawing horizontals — with zero indent and no `.dimmed()` /
/// `.yellow()` / `.cyan()` coloring. The primitive preserves that shape:
/// zero indent OUTSIDE any coloring span, plain fence bytes reaching the
/// writer, and the SQL payload emitted through a single `writeln!` so an
/// operator piping `forge seed --dry-run | psql` gets a captured SQL
/// blob that psql itself parses (psql treats the `---` fences as SQL
/// line-comment leaders, ignoring them harmlessly). A re-lift that
/// wrapped the fences in `.dimmed()` would drop the `\x1b[0m` reset into
/// the operator's pipe and break psql's parse of the following payload
/// bytes.
///
/// # stdout, not stderr
///
/// Both pre-lift consumers routed the three lines through `println!`
/// (stdout), not `eprintln!` (stderr). The primitive preserves that
/// routing: an operator running `forge seed --dry-run > migration.sql`
/// sees the fenced SQL captured to a file; an operator watching stderr
/// for warnings sees only warnings.
///
/// # Compounding
///
/// Pre-lift 2 sibling sites in one command module
/// (`commands/seed.rs::{seed (×1), unseed (×1)}`) each restated the
/// three-line `--- DRY RUN SQL --- / <sql> / --- END DRY RUN ---`
/// grammar verbatim, with the ASCII triple-hyphen open fence, the
/// literal `DRY RUN SQL` open label, the `{}"..., sql` body interpolation,
/// the ASCII triple-hyphen close fence, and the literal `END DRY RUN`
/// close label all spelled inline. A future palette adjustment (a swap
/// of the ASCII `---` fence for a U+2500 `───` box-drawing fence under a
/// unified diagnostic grammar, a promotion of the fences to `.dimmed()`
/// under a themed dry-run readout matching the sibling
/// [`print_dimmed_dashed_marker`] palette, a shift of the open-fence
/// label to `--- DRY RUN (BEGIN) ---` with a matching `--- DRY RUN
/// (END) ---` close for a stricter operator grammar, a fusion with a
/// `writeln!("-- generated by forge seed --dry-run on {ts}", ...)`
/// provenance banner between the open fence and the body, an OTLP
/// `dry_run_sql_dumped` observability event wired alongside the print
/// carrying the exact `sql.len()` byte count) had to hit 2 sites in
/// lockstep or drift the visual grammar; post-lift it hits ONE typed
/// body. Delegates to [`write_dry_run_sql_dump`] against
/// [`std::io::stdout()`]; the writer split exists so the fail-before-
/// pass test can pin the exact three-line body, the ASCII triple-hyphen
/// fences, the `DRY RUN SQL` open label, the `END DRY RUN` close label,
/// the payload interpolation shape, and the trailing newline after the
/// close fence by inspecting emitted bytes rather than shelling out and
/// grepping stdout.
pub fn print_dry_run_sql_dump(sql: &str) {
    let _ = write_dry_run_sql_dump(&mut std::io::stdout().lock(), sql);
}

/// Writer-taking sibling to [`print_dry_run_sql_dump`]. Emits the three
/// fused lines — `--- DRY RUN SQL ---` open fence, the caller-supplied
/// SQL payload, `--- END DRY RUN ---` close fence — via three
/// [`writeln!`] calls against the supplied writer.
/// [`print_dry_run_sql_dump`] is the stdout adapter; this variant exists
/// so tests can pin the exact three-line body, the ASCII triple-hyphen
/// fences (not U+2500 box-drawing), the `DRY RUN SQL` open label, the
/// `END DRY RUN` close label, the byte-for-byte payload reproduction
/// between the fences, and the trailing newline after the close fence by
/// inspecting emitted bytes rather than capturing stdout.
pub fn write_dry_run_sql_dump<W: std::io::Write>(w: &mut W, sql: &str) -> std::io::Result<()> {
    writeln!(w, "--- DRY RUN SQL ---")?;
    writeln!(w, "{}", sql)?;
...
mod tests {
    ...
    /// Fail-before-pass envelope for
    /// [`super::write_step_info_indented`]. Pins the one-line body
    /// every pre-lift consumer spelled verbatim (`println!("   ℹ️
    /// <literal>")`): a three-space leading indent OUTSIDE the glyph
    /// (pre-lift consumers spelled the indent inside the format
    /// string, not as a computed prefix), the `ℹ️` glyph followed by
    /// TWO spaces (matching the sibling [`super::write_step_info`]
    /// legibility hedge against the emoji's variable rendered width),
    /// then the plain (uncolored) message. A silent contract drift a
    /// future rewrite might introduce — dropping the three-space
    /// indent (silently collapsing this primitive onto
    /// [`super::write_step_info`]), promoting the line to
    /// `.bright_cyan()` (`\x1b[96m` — the milestone-level
    /// `print_info` palette), collapsing the double-space gap after
    /// the glyph to a single space, slipping a trailing blank line
    /// into the primitive body — flips this assertion rather than
    /// compiling and silently diverging the 3 consumer sites' visual
    /// grammar.
    #[test]
    fn write_step_info_indented_emits_exactly_one_indented_info_line() {
        // Force ANSI emission (colored auto-drops sequences on a
        // non-tty stdout) and serialize against peer banner tests via
        // [`AnsiOverrideForTest`] — pre-lift stanza carries NO ANSI
        // escapes, so the absence-of-color contract this test pins
        // holds under either colored-mode.
        let _override_guard = AnsiOverrideForTest::acquire();

        let mut buf: Vec<u8> = Vec::new();
        super::write_step_info_indented(&mut buf, "No orphaned pods to clean up")
            .expect("write_step_info_indented against a Vec<u8> writer must succeed");

        let out = String::from_utf8(buf)
            .expect("write_step_info_indented must emit valid UTF-8 (the pre-lift println!s did)");
        ...
        assert_eq!(
            lines.len(),
            1,
            "write_step_info_indented must emit exactly one line — \
        ...
        // glyph — a fusion that dropped the indent (silently
        // collapsing this primitive onto [`super::write_step_info`])
        // fails here.
        assert!(
            lines[0].starts_with("   ℹ"),
            "line 0 must begin with three spaces followed by the `ℹ️` \
             info glyph — every pre-lift consumer spelled `\"   ℹ️  \
             <fmt>\"` verbatim; the three-space indent is the one \
             visual difference from the sibling [`super::write_step_info`] \
             and dropping it collapses the two grammars silently; \
             got {:?}",
            lines[0]
        );

        // The `ℹ️` glyph followed by TWO spaces reaches the rendered
        // line — a legibility hedge against the emoji's variable
        // rendered width, shared with the sibling
        // [`super::write_step_info`]. A fusion that collapses it to a
        // single space fails here.
        assert!(
            lines[0].contains("ℹ\u{fe0f}  "),
            "line 0 must contain the `ℹ️` glyph followed by TWO \
             spaces — every pre-lift consumer spelled `\"   ℹ️  \"` \
             (three-space indent + glyph + double space) verbatim as \
             a legibility hedge against the emoji's variable rendered \
             width; got {:?}",
            lines[0]
        );
        ...
        // The message text reaches the rendered line verbatim.
        assert!(
            lines[0].contains("No orphaned pods to clean up"),
            "line 0 must carry the message verbatim; got {:?}",
            lines[0]
        );
        ...
        // NO ANSI escape sequence reaches the rendered line — every
        // pre-lift consumer spelled the whole line plain (no
        // `.color()` chain, no `format!(...).color()` wrap). A future
        // promotion to `.bright_cyan()` (`\x1b[96m` — the
        // milestone-level [`super::print_info`] palette) collapses
        // the distinction between this uncolored in-body primitive
        // and the heavier milestone-level info sigil.
        assert!(
            !out.contains('\x1b'),
            "write_step_info_indented must emit ZERO ANSI escape \
             sequences — every pre-lift consumer spelled the whole \
             line plain. Got: {:?}",
            out
        );
        ...
        assert!(
            out.ends_with('\n'),
            "write_step_info_indented must emit a trailing `\\n` (the \
             pre-lift `println!` did); got {:?}",
            out
        );
    }

    /// Post-lift the 3 callers migrated onto
    /// [`super::print_step_info_indented`] no longer spell the
    /// `println!("   ℹ️  <literal>")` shape inline. Structural
    /// regression shield — the exact-shape needle is `"   ℹ` (opening
    /// quote immediately followed by three spaces and the info
    /// glyph's leading BMP code point, `\u{2139}`) appearing anywhere
    /// in the module body. The sibling zero-indent
    /// [`super::print_step_info`] shape spelled `"ℹ` right after the
    /// quote (no spaces), so the needle does not match it.
    #[test]
    fn print_step_info_indented_callers_delegate_through_primitive() {
        const CALLERS: &[(&str, &str)] = &[(
            include_str!("commands/migrations.rs"),
            "commands/migrations.rs",
        )];
        for (source, module_path) in CALLERS {
            ...
            for (i, line) in body.lines().enumerate() {
                assert!(
                    !line.contains("\"   ℹ"),
                    "{module_path}:{lineno} spells the pre-lift inline \
                     `\"   ℹ️  <literal>\"` three-space-indented \
                     step-info marker — that shape was lifted onto \
                     `crate::ui::print_step_info_indented`. A re-inline \
                     would silently reopen the 3-site duplication \
                     class this shield exists to close. Offending \
                     line: {line:?}",
                    lineno = i + 1
                );
            ...
            assert!(
                body.contains("crate::ui::print_step_info_indented("),
                "{module_path} body must forward to \
                 `crate::ui::print_step_info_indented(\"<MSG>\")` — \
                 the primitive body every three-space-indented uncolored \
                 `ℹ️  ` in-body sub-listing info line in the crate now \
                 delegates through."
            );
        }
    }
    ...
    /// the [`super::HeavyRuleStyle::SummaryBlue80`] variant. Pins the
    /// single-line body every pre-lift consumer spelled verbatim
    /// (`"━".repeat(80).bright_blue()`): exactly one line, exactly
    /// 80 `━` glyphs, the `bright_blue` ANSI sequence (`\x1b[94m`),
    /// and a trailing `\n`. A silent contract drift a future rewrite
    /// might introduce — dropping a glyph to width 78, promoting the
    /// rule color to `blue` and losing the visual-contrast contract
    /// against the `bright_blue().bold()` headline, dropping the
    /// trailing newline, hoisting the rule glyph off `━` under a
    /// leaner ASCII-only readout — flips this assertion rather than
    /// compiling and silently diverging the five consumer sites'
    /// visual grammar.
    #[test]
    fn write_heavy_rule_summary_blue_80_emits_one_bright_blue_bar_line_of_80_glyphs() {
        // Force ANSI emission (colored auto-drops sequences on a
        // non-tty stdout) and serialize against peer banner tests
        // via [`AnsiOverrideForTest`]; its Drop restores colored's
        // auto-detection on scope exit AFTER releasing the shared
        ...
        let mut buf: Vec<u8> = Vec::new();
        super::write_heavy_rule(&mut buf, super::HeavyRuleStyle::SummaryBlue80)
        ...
        assert_eq!(
            lines.len(),
            1,
            "write_heavy_rule must emit exactly one line — the \
             pre-lift stanza is one `println!` carrying no framing \
             blank; got {}:\n{}",
            lines.len(),
            out
        ...
        // 80 — a fusion that hoists the width off the variant and
        // pins it to a fresh literal (78, 76) fails here.
        let bar_glyph_count = lines[0].chars().filter(|c| *c == '━').count();
        assert_eq!(
            bar_glyph_count, 80,
            "line 0 must contain exactly 80 `━` glyphs; got {}",
            bar_glyph_count
        );
        ...
        // loses the contrast against the `bright_blue().bold()`
        // headline the 5 consumer sites paint between the rules.
        assert!(
    ...
    /// Fail-before-pass envelope for [`super::write_heavy_rule`] under
    /// the [`super::HeavyRuleStyle::PushCompleteGreen60`] variant.
    /// Pins the single-line body every pre-lift consumer spelled
    /// verbatim (`"━".repeat(60).bright_green()`): exactly one line,
    /// exactly 60 `━` glyphs, the `bright_green` ANSI sequence
    /// (`\x1b[92m`), and a trailing `\n`. Peer to
    /// [`write_heavy_rule_summary_blue_80_emits_one_bright_blue_bar_line_of_80_glyphs`]
    /// — pins the OTHER pre-lift `(color, width)` pairing so a fusion
    /// that collapsed both variants onto one arm (or dropped the
    /// PushComplete variant on the assumption "callers can pass
    /// SummaryBlue80") silently drifts the PUSH COMPLETE grammar off
    /// its 60-glyph green rule onto an 80-glyph blue one.
    #[test]
    fn write_heavy_rule_push_complete_green_60_emits_one_bright_green_bar_line_of_60_glyphs() {
        let _override_guard = AnsiOverrideForTest::acquire();

        let mut buf: Vec<u8> = Vec::new();
        super::write_heavy_rule(&mut buf, super::HeavyRuleStyle::PushCompleteGreen60)
            .expect("write_heavy_rule against a Vec<u8> writer must succeed");
        let out = String::from_utf8(buf)
            .expect("write_heavy_rule must emit valid UTF-8 (the pre-lift println!s did)");
        ...
        assert_eq!(
            lines.len(),
            1,
            "write_heavy_rule must emit exactly one line; got {}:\n{}",
            lines.len(),
            out
        );
        ...
        assert_eq!(
            bar_glyph_count, 60,
            "line 0 must contain exactly 60 `━` glyphs; got {}",
            bar_glyph_count
        );

        // The palette contract: the rule carries the `bright_green`
        // ANSI sequence (`\x1b[92m`). A silent swap of
        // `.bright_green()` for `.green()` (`\x1b[32m`) on the rule
        // loses the contrast against the `PUSH COMPLETE.green().bold()`
        // headline the 2 consumer sites paint between the rules.
        assert!(
            lines[0].contains("\x1b[92m"),
            "line 0 must carry the `bright_green` ANSI sequence \
             (`\\x1b[92m`); got {:?}",
    ...
    /// no longer spell the `println!("{}", "━".repeat(<width>).<color>())`
    /// shape inline in `commands/rust_service.rs`. Structural
    /// regression shield — without it, a future refactor could
    /// silently re-inline the one-liner (e.g. a "just call `println!`
    /// directly, it's shorter" cleanup) and reopen the 7-site
    ...
    /// shield.
    ///
    /// The exact-shape needle is `println!("{}", "━".repeat(` on one
    /// source line — the macro invocation with the exact `{}` value
    /// slot and the `"━".repeat(` literal opener. A sibling shape
    /// carrying a different glyph (`"═".repeat`, `"─".repeat`) or a
    /// runtime-composed rule (`bar_string.bright_blue()`) is OUT of
    /// scope by construction — the needle's `"━".repeat(` character
    /// sequence rejects both.
    ///
    /// The positive count is pinned at the pre-lift 7-site census
    /// (`rust_service.rs` ×7). A fusion that folded two rule sites
    /// into one call or dropped one of the rules silently fails
    /// here — the negative half above would still pass, but the
    /// positive count would fall below the pre-lift census.
    #[test]
    fn print_heavy_rule_callers_delegate_through_primitive() {
        const CALLERS: &[(&str, &str, usize)] = &[(
            include_str!("commands/rust_service.rs"),
            "commands/rust_service.rs",
            7,
        ...
        for (source, module_path, expected_forwards) in CALLERS {
            let body = crate::test_support::module_body_before_first_cfg_test(source, module_path);
            for (i, line) in body.lines().enumerate() {
                if !line.contains("println!(\"{}\", \"\u{2501}\".repeat(") {
                    continue;
                }
                panic!(
                    "{module_path}:{lineno} spells the pre-lift inline \
                     `println!(\"{{}}\", \"\u{2501}\".repeat(<width>).<color>())` \
                     bare heavy-rule stanza — that shape was lifted \
                     onto `crate::ui::print_heavy_rule`. A re-inline \
                     would silently reopen the 7-site duplication \
                     class this shield exists to close. Offending \
                     line: {line:?}",
                    lineno = i + 1
            ...
            let forward_hits = body.matches("crate::ui::print_heavy_rule(").count();
            assert_eq!(
                forward_hits, *expected_forwards,
                "{module_path} body must forward to \
                 `crate::ui::print_heavy_rule(...)` at exactly \
                 {expected_forwards} site(s) — one per pre-lift \
                 consumer in this module. A fusion that folded two \
                 consumer sites into one call or dropped one of the \
                 rule lines silently fails here. Found \
    ...
    /// [`super::write_dry_run_sql_dump`] emits the exact three-line
    /// `--- DRY RUN SQL --- / <sql> / --- END DRY RUN ---` fused
    /// open+body+close stanza both `commands/seed.rs::seed` and
    /// `commands/seed.rs::unseed` pre-lift sites each spelled inline as
    /// `println!("--- DRY RUN SQL ---"); println!("{}", sql);
    /// println!("--- END DRY RUN ---");`. Pins the line count (exactly
    /// three), the ASCII triple-hyphen fences (`---`, U+002D, NOT
    /// U+2500 `─` box-drawing horizontals or U+2014 `—` em-dashes), the
    /// literal `DRY RUN SQL` open label, the literal `END DRY RUN`
    /// close label, the byte-for-byte payload reproduction between the
    /// fences (so an operator's `psql` receiving the piped bytes sees
    /// exactly the SQL that a wet-run would have executed), the
    /// no-ANSI-escape contract (a `.dimmed()` re-lift would break psql
    /// on the receiving end of a `--dry-run | psql` pipe), and the
    /// trailing newline after the close fence so the caller's next
    /// output lands on a fresh row. Dropping either fence, reordering
    /// the fences around the payload, swapping the ASCII `---` for
    /// U+2500 `───` under a "unified diagnostic" refactor, painting the
    /// fences `.dimmed()` under a themed dry-run readout, or dropping
    /// the trailing newline flips this assertion rather than compiling
    /// and silently diverging the two `dry_run` guards' operator
    /// output.
    #[test]
    fn write_dry_run_sql_dump_emits_exact_three_line_fenced_block() {
        let mut buf: Vec<u8> = Vec::new();
        super::write_dry_run_sql_dump(&mut buf, "SELECT 1;\nSELECT 2;")
            .expect("write_dry_run_sql_dump against a Vec<u8> writer must succeed");
        let out = String::from_utf8(buf).expect(
            "write_dry_run_sql_dump must emit valid UTF-8 \
             (the pre-lift println!s did)",
        );
        assert_eq!(
            out, "--- DRY RUN SQL ---\nSELECT 1;\nSELECT 2;\n--- END DRY RUN ---\n",
            "byte-oracle: exact three fused lines — ASCII triple-hyphen \
             open fence with `DRY RUN SQL` label, sql payload (which may \
             itself carry embedded newlines), ASCII triple-hyphen close \
             fence with `END DRY RUN` label, trailing newline. Any drift \
             from this exact byte sequence flips this assertion. Actual: \
             {out:?}"
        );

        // ANSI-free contract: a re-lift that wrapped the fences in
        // `.dimmed()` / `.yellow()` / any colored span would leak
        // `\x1b[...m` bytes into the operator's `forge seed --dry-run |
        // psql` pipe and break psql's parse of the SQL payload.
        assert!(
            !out.contains('\x1b'),
            "write_dry_run_sql_dump must not emit any ANSI escape \
             sequences — the fences are plain ASCII so a piped dry-run \
             stream stays parse-clean for `psql`. Got: {out:?}"
        );

        // Fence-glyph contract: the fences must be ASCII `-` (U+002D)
        // hyphens, not U+2500 `─` box-drawing horizontals (the sibling
        // grammar `print_dimmed_dashed_marker` carries) and not U+2014
        // `—` em-dashes. A silent glyph-swap under a "unified
        // diagnostic grammar" refactor would still visually resemble
        // the pre-lift fences on the operator's terminal while
        // breaking downstream byte-oracle comparators.
        assert!(
            out.contains("--- DRY RUN SQL ---"),
            "open fence must be the exact ASCII `--- DRY RUN SQL ---` \
             triple-hyphen shape; got {out:?}"
        );
        assert!(
            out.contains("--- END DRY RUN ---"),
            "close fence must be the exact ASCII `--- END DRY RUN ---` \
             triple-hyphen shape; got {out:?}"
        );
        assert!(
            !out.contains('\u{2500}'),
            "fences must be ASCII `-`, not U+2500 `─` box-drawing \
             horizontals — that glyph is reserved for the sibling \
             `print_dimmed_dashed_marker` diagnostic-boundary grammar. \
             Got: {out:?}"
        );
        assert!(
            !out.contains('\u{2014}'),
            "fences must be ASCII `-`, not U+2014 `—` em-dashes. Got: \
             {out:?}"
        );

        // Line-count contract: exactly three lines (open, payload,
        // close) when the payload itself carries a single logical row.
        let mut single: Vec<u8> = Vec::new();
        super::write_dry_run_sql_dump(&mut single, "SELECT 1;")
            .expect("single-line payload rendering must succeed");
        let single_out = String::from_utf8(single).expect("valid UTF-8");
        assert_eq!(
            single_out.lines().count(),
            3,
            "with a single-line payload the primitive emits exactly \
             three lines — open, payload, close. Got: {single_out:?}"
        );
        assert!(
            single_out.ends_with('\n'),
            "close fence must end with a newline so the caller's next \
             output lands on a fresh row. Got tail bytes: {:?}",
            single_out
                .as_bytes()
                .iter()
                .rev()
    ...
    /// Both pre-lift consumer sites in `commands/seed.rs` (`seed`'s
    /// `dry_run` guard and `unseed`'s `dry_run` guard) must forward to
    /// the fusion primitive rather than respell the three-line stanza
    /// inline. Pinning the delegation shape means a re-inline of the
    /// three `println!` lines at either site silently re-opens the
    /// two-site duplication class this primitive exists to close.
    #[test]
    fn dry_run_sql_dump_callers_delegate_through_primitive() {
        let source = include_str!("commands/seed.rs");
        let body =
            crate::test_support::module_body_before_first_cfg_test(source, "commands/seed.rs");

        // Negative shield: no re-inline of either fence in the module
        // body. A "one-off" restatement of `println!("--- DRY RUN SQL
        // ---")` or `println!("--- END DRY RUN ---")` at either
        // consumer site flips this assertion.
        assert!(
            !body.contains("println!(\"--- DRY RUN SQL ---\")"),
            "commands/seed.rs must not contain the pre-lift inline \
             `println!(\"--- DRY RUN SQL ---\")` open fence — the \
             three-line dry-run SQL dump was lifted onto \
             `crate::ui::print_dry_run_sql_dump`. A re-inline reopens \
             the two-site duplication class this shield exists to \
             close."
        );
        assert!(
            !body.contains("println!(\"--- END DRY RUN ---\")"),
            "commands/seed.rs must not contain the pre-lift inline \
             `println!(\"--- END DRY RUN ---\")` close fence — the \
             three-line dry-run SQL dump was lifted onto \
             `crate::ui::print_dry_run_sql_dump`. A re-inline reopens \
             the two-site duplication class this shield exists to \
             close."
        ...
        // Positive shield: exactly two forwarding hits (one per
        // pre-lift consumer — `seed`'s `dry_run` guard and `unseed`'s
        // `dry_run` guard). A fusion that collapsed both `dry_run`
        // arms into a single call site or dropped one of the arms
        // silently fails here.
        let forward_hits = body.matches("crate::ui::print_dry_run_sql_dump(").count();
        assert_eq!(
            forward_hits, 2,
            "commands/seed.rs body must forward to \
             `crate::ui::print_dry_run_sql_dump(...)` at exactly two \
             sites — one per pre-lift consumer (`seed` + `unseed`). A \
             fusion that folded the two consumer sites into one call \
             or dropped one of the `dry_run` arms silently fails here. \
             Found {forward_hits} forwarding hits."
        );
    }
