...
mod tests {
    ...
    // `synth_output_for_git` so the four pre-existing call sites in this
    // test module read verbatim while the underlying construction routes
    ...
    // local `fn synth_output_for_git` this module carried paid
    // `Command::new("true"/"false").status().expect(...)` per
    // fake-`Output` construction (see the sibling shield below for the
    // enforcement).
    use crate::test_support::synthetic_output as synth_output_for_git;
    ...
    /// Consolidation shield: the four `GitError::from_capture` shields
    /// in this test module MUST route fake-`Output` construction through
    /// the shared [`crate::test_support::synthetic_output`] helper
    /// (aliased locally as `synth_output_for_git` at the `mod tests`
    /// head), NOT a locally-redefined fork-based builder.
    /// Pre-consolidation this module carried a `fn synth_output_for_git`
    /// body that forked `Command::new("true"/"false").status().expect(
    /// ...)` per fake-`Output` construction — the same fork-per-call
    /// cost the sibling `commands/status.rs::tests` and
    /// `commands/helm.rs::capture_tests` shields already close against
    /// their `fn make_output` copies (c8a3f55). PRIME DIRECTIVE:
    /// duplication budget is zero (THEORY §VI.1). All four sites (this
    /// file, `retry::tests`, `commands/status.rs::tests`,
    /// `commands/helm.rs::capture_tests`) now delegate to the ONE
    /// `test_support::synthetic_output` body, so a future reshape
    /// (adding a captured signal detail, widening to a cross-platform
    /// builder, tracking the fork-elapsed instant) lands at one body.
    ///
    ...
    fn tests_use_shared_synthetic_output_from_test_support_not_local_redef() {
        ...
        assert!(
            ...
            "error.rs::tests must import the shared fake-`Output` \
        ...
        let deprecated_signature = format!(
            "fn synth_{}_for_git(success: bool, stdout: &[u8], stderr: &[u8]) -> std::process::Output",
            "output"
        );
        assert!(
            !SOURCE.contains(&deprecated_signature),
            "error.rs must NOT redefine the deprecated local fork-based \
