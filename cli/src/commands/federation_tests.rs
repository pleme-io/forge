...
#[allow(dead_code)]
const FEDERATION_JOB_POLL_BACKOFF: RetryPolicy =
...
mod tests {
    ...
    fn test_wait_for_job_completion_consumes_typed_poll_delay_not_bare_fixed_sleep() {
        ...
        let delegation_hits =
            crate::test_support::code_line_hits(module_body, "federation_job_poll_delay");
        assert!(
            delegation_hits.len() >= 2,
            "federation_tests.rs must consume the typed poll-delay \
             helper at the poll loop's sleep site — post-lift the \
             `federation_job_poll_delay` function pointer is passed to \
             `crate::poll_backoff_advance::advance_poll_backoff_tokio` \
